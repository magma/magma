...
TEST_F(Service303Test, test_counters) {
  magma::service303::increment_counter("test_counter", 3, NO_LABELS);
...
TEST_F(Service303Test, test_gauges) {
  ...
  magma::service303::increment_gauge(
  ...
  magma::service303::increment_gauge("test_gauge", 3, NO_LABELS);
  ...
  magma::service303::decrement_gauge("test_gauge", 3, NO_LABELS);
  status = service303_client->GetMetrics(&metrics_container);
  family = &Service303Test::findFamily(metrics_container, "test_gauge");
  EXPECT_EQ(family->name(), "test_gauge");
  ...
  // Set the gauge to 10
  magma::service303::set_gauge("test_gauge", 10, NO_LABELS);
  status = service303_client->GetMetrics(&metrics_container);
  family = &Service303Test::findFamily(metrics_container, "test_gauge");
  EXPECT_EQ(family->name(), "test_gauge");
  gauge = &family->metric().Get(0).gauge();
...
TEST_F(Service303Test, test_histograms) {
  ...
  magma::service303::observe_histogram(
      "test_hist", 3, NO_LABELS, NO_BOUNDARIES);
  MetricsContainer metrics_container;
  int status = service303_client->GetMetrics(&metrics_container);
  const MetricFamily* family =
  ...
  magma::service303::observe_histogram(
      "test_hist", 7, NO_LABELS, 3, 1, 10, 100);
  status = service303_client->GetMetrics(&metrics_container);
...
TEST_F(Service303Test, test_enum_conversions) {
  ...
  magma::service303::increment_counter(
