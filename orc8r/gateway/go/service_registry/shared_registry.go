...
func NewDefaultRegistry() GatewayRegistry {
	...
	if _, err := reg.GetServiceAddress(platform_registry.ControlProxyServiceName, protos2.ServiceType_SOUTHBOUND); err != nil {
...
func GetServiceAddress(service string) (string, error) {
	return Get().GetServiceAddress(service, protos2.ServiceType_SOUTHBOUND)
}
