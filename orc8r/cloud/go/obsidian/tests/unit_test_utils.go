...
func RunUnitTest(t *testing.T, e *echo.Echo, test Test) {
	...
	if test.ExpectedError != "" {
		...
		if httpErr, ok := handlerErr.(*echo.HTTPError); ok {
			...
			test_util.AssertErrorsEqual(t, test.ExpectedError, fmt.Sprintf("%s", httpErr.Message), "<>")
		} else {
		...
		if handlerErr == nil {
			...
			test_util.AssertErrorsEqual(t, test.ExpectedErrorSubstring, handlerErr.Error(), "<>")
		}
	} else {
