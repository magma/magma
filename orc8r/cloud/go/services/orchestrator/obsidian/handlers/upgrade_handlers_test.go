...
func Test_Tiers(t *testing.T) {
	...
	tc = tests.Test{
		...
		Handler:        createTier,
		ExpectedStatus: 500,
		ExpectedError:  "rpc error: code = Unknown desc = could not find entities matching [type:\"magmad_gateway\" key:\"g1\" ]",
	}
	tests.RunUnitTest(t, e, tc)
...
func TestPartialTierUpdates(t *testing.T) {
	...
	tc = tests.Test{
		...
		ParamValues:    []string{"n1", "tier1"},
		ExpectedStatus: 500,
		ExpectedError:  "rpc error: code = Unknown desc = could not find entities matching [type:\"magmad_gateway\" key:\"g4\" ]",
	}
	tests.RunUnitTest(t, e, tc)
