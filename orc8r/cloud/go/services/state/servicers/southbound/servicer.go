...
func (srv *stateServicer) ReportStates(ctx context.Context, req *protos.ReportStatesRequest) (*protos.ReportStatesResponse, error) {
	...
	if err != nil {
		return nil, common.InternalErr(err, "ReportStates convert to blobs")
	}

	store, err := srv.factory.StartTransaction(nil)
	if err != nil {
		return nil, common.InternalErr(err, "ReportStates blobstore start transaction")
	}
	err = store.Write(networkID, states)
	if err != nil {
		_ = store.Rollback()
		return nil, common.InternalErr(err, "ReportStates blobstore create or update")
	}
	err = store.Commit()
	if err != nil {
		return nil, common.InternalErr(err, "ReportStates blobstore commit transaction")
	}

	byID, err := state_types.MakeSerializedStatesByID(req.States)
	if err != nil {
		return nil, common.InternalErr(err, "ReportStates make states by ID")
	}
...
func (srv *stateServicer) DeleteStates(ctx context.Context, req *protos.DeleteStatesRequest) (*protos.Void, error) {
	...
	stateRequest := &internal_protos.GetStatesRequest{NetworkID: networkID, Ids: req.Ids}
	...
	store, err := srv.factory.StartTransaction(nil)
	if err != nil {
		return nil, common.InternalErr(err, "DeleteStates blobstore start transaction")
	}
	err = store.Delete(networkID, ids)
	if err != nil {
		_ = store.Rollback()
		return nil, common.InternalErr(err, "DeleteStates blobstore delete")
	}
	err = store.Commit()
	if err != nil {
		return nil, common.InternalErr(err, "DeleteStates blobstore commit transaction")
	}
...
func (srv *stateServicer) SyncStates(ctx context.Context, req *protos.SyncStatesRequest) (*protos.SyncStatesResponse, error) {
	...
	store, err := srv.factory.StartTransaction(nil)
	if err != nil {
		return nil, common.InternalErr(err, "SyncStates blobstore start transaction")
	}
	blobs, err := store.GetMany(networkID, tkIds)
	if err != nil {
		_ = store.Rollback()
		return nil, common.InternalErr(err, "SyncStates blobstore get many")
	}
	...
	err = store.Commit()
	if err != nil {
		return nil, common.InternalErr(err, "SyncStates blobstore commit transaction")
	}

	return &protos.SyncStatesResponse{UnsyncedStates: unsyncedStates}, nil
}

func (srv *stateServicer) getStates(_ context.Context, req *internal_protos.GetStatesRequest) (*internal_protos.GetStatesResponse, error) {
	store, err := srv.factory.StartTransaction(nil)
	if err != nil {
	...
	return &internal_protos.GetStatesResponse{States: blobsToStates(blobs)}, nil
}
