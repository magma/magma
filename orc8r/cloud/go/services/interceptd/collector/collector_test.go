...
func TestMultiStreamsQuery(t *testing.T) {
	...
	expectedQuery := elastic.NewBoolQuery().
		Filter(elastic.NewTermsQuery("stream_name.keyword", stringsToInterfaces(interceptd.GetESStreams())...)).
		Filter(elastic.NewTermsQuery("event_type.keyword", stringsToInterfaces(interceptd.GetESEventTypes())...)).
