...
type Store interface {
	...
	GetTokenInfoFromNonce(networkID string, nonce string) (*protos.TokenInfo, error)
}
...
func (b *blobstoreStore) SetTokenInfo(oldNonce string, tokenInfo protos.TokenInfo) error {
	...
	err = store.Write(n, blobstore.Blobs{nBlob})
	if err != nil {
		return err
	}
...
func (b *blobstoreStore) GetTokenInfoFromNonce(networkID string, nonce string) (*protos.TokenInfo, error) {
	store, err := b.factory.StartTransaction(nil)
	if err != nil {
		return nil, err
	}
	...
	lBlob, err := store.Get(networkID, nTK)
	if err != nil {
		return nil, err
	}
