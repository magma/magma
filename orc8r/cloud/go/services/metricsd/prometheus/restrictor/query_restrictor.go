...
func (q *QueryRestrictor) AddMatcher(key string, values ...string) *QueryRestrictor {
	...
	matchString := strings.Builder{}
	for i, val := range values {
		matchString.WriteString(val)
		if i < len(values)-1 {
			matchString.WriteString("|")
		}
	}

	q.matchers = append(q.matchers, labels.Matcher{Type: labels.MatchRegexp, Name: key, Value: matchString.String()})
	return q
}
