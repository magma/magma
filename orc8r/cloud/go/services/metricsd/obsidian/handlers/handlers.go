...
func GetObsidianHandlers(configMap *config.ConfigMap) []obsidian.Handler {
	...
	if err != nil {
		...
		ret = append(ret,
			...
			obsidian.Handler{Path: promH.TenantV1QueryURL, Methods: obsidian.GET, HandlerFunc: promH.TenantQueryHandlerProvider(pAPI)},
			obsidian.Handler{Path: promH.TenantV1QueryRangeURL, Methods: obsidian.GET, HandlerFunc: promH.TenantQueryRangeHandlerProvider(pAPI)},
