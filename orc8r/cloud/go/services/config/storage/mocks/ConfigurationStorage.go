...
func (_m *ConfigurationStorage) GetConfigs(networkId string, criteria *storage.FilterCriteria) (map[storage.TypeAndKey]*storage.ConfigValue, error) {
	ret := _m.Called(networkId, criteria)

	var r0 map[storage.TypeAndKey]*storage.ConfigValue
	if rf, ok := ret.Get(0).(func(string, *storage.FilterCriteria) map[storage.TypeAndKey]*storage.ConfigValue); ok {
		r0 = rf(networkId, criteria)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(map[storage.TypeAndKey]*storage.ConfigValue)
		}
	}
