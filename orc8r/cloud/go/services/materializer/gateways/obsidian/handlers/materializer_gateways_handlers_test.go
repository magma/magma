...
import (
	...
	"magma/orc8r/cloud/go/services/materializer/gateways/obsidian/handlers"
	...
	"magma/orc8r/cloud/go/services/materializer/gateways/storage/mocks"
...
func TestGetViewsForNetwork(t *testing.T) {
	// Set up test
	mockStore := &mocks.GatewayViewStorage{}
	config.TLS = false
	...
	// Execute test
	err := handlers.ListGatewayViews(c, mockStore)
...
func TestGetViewsForNetworkEmptyResponse(t *testing.T) {
	...
	mockStore.On("GetGatewayViewsForNetwork", networkID).Return(map[string]*storage.GatewayState{}, nil)
