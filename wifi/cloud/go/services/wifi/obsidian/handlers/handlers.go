...
func getGateway(c echo.Context) error {
	...
	if err != nil {
		return obsidian.MakeHTTPError(errors.Wrap(err, "failed to load wifi gateway"), http.StatusInternalServerError)
	}
...
func deleteGateway(c echo.Context) error {
	...
	if gwEnt.PhysicalID != "" {
		...
		if err != nil {
			return obsidian.MakeHTTPError(errors.Wrap(err, "failed to delete device for gateway"))
		}
	}
