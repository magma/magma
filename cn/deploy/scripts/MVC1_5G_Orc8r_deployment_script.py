...
def run_services():
    ...
    time.sleep(3)
    execute_cmd("helm install prometheus stable/prometheus-operator --namespace default")
    myprint("FAIL", "change type(key) value from 'ClusterIP' to 'NodePort' and save it") 
    time.sleep(3)
    execute_cmd("kubectl edit service/prometheus-prometheus-oper-alertmanager -n default")
    myprint("FAIL", "change type(key) value from 'ClusterIP' to 'NodePort' and save it") 
    time.sleep(3)
    execute_cmd("kubectl edit service/prometheus-grafana -n default")
    myprint("FAIL", "change type(key) value from 'ClusterIP' to 'NodePort' and save it") 
    time.sleep(3)
    execute_cmd("kubectl edit service/prometheus-prometheus-oper-prometheus -n default")
    myprint("GREEN", "+++++++++++++++++++++++++++++++++++++++++++++++")
...
def del_objects(file, type):
    k8s_obj = "kubectl get all -n default| grep %s > %s" % (type , file)
    execute_cmd(k8s_obj)
...
def un_install(pwd):
    ...
    execute_cmd("helm uninstall prometheus stable/prometheus-operator --namespace default")
