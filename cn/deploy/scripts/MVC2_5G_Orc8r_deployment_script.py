...
DEBIAN_QCOW2_FILE = os.path.join(
    TEMPLATES_DIR, 'debian-9-openstack-amd64.qcow2',
)
...
def del_iptables(pwd):
    myprint(
        "BLUE", "*****Trying to delete iptables rules added as part of installation*****",
    )
...
def del_dvs(pwd):
    ...
    time.sleep(10)
    myprint(
        "BLUE", "*****Deleting PersistantVolumes [PVs] which are created for upload the Debian Image*****",
    )
...
def del_cdi():
    myprint(
        "BLUE", "*****Deleting EFK [ElasticSearch Fluentd Kibana] configurations*****",
    )
    execute_cmd(
...
def check_status(obj, namespace):
    ...
    if obj == "pod":
        cmd = "kubectl get pods -n " + namespace + " | awk " + \
            "'{{if ($3 ~ " + '!"Running"' + " || $3 ~ " + \
            '!"STATUS"' + ") print $1,$3};}' > " + K8S_GET_OBJ
    elif obj == "deployment":
        cmd = "kubectl get deployment -n " + namespace + " | awk " + \
            "'{{if ($2 ~ " + '!"1"' + " || $2 ~ " + '!"READY"' + \
            ") print $1,$2};}' > " + K8S_GET_OBJ
    execute_cmd(cmd)
...
def install_cdi():
    myprint(
        "BLUE", "*****Installing COntainerized Data Importer[CDI] used to import VM images to create PVC*****",
    )
...
def create_datavolume(pwd):
    ...
    myprint(
        "BLUE", "*****Create DataVolume[dv] which will mount the debian qcow2 disk.img to directory under /mnt*****",
    )
    try:
...
def get_help(color):
    ...
    myprint(color, '    (OR)   ')
    myprint(
        color, './MVC2_5G_Orc8r_deployment_script.py --password <sudo-password> --install',
    )
