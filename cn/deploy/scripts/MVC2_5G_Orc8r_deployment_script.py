...
DEBIAN_QCOW2_FILE = os.path.join(
    TEMPLATES_DIR,
...
# Checking pre-requisites like kubeadm, helm should be installed before we
# run this script


def check_pre_requisite():
    # Setting logging basic configurations like severity level=DEBUG,
    # timestamp, function name, line numner
    logging.basicConfig(
...
# Un-installing all the k8s objects and deleting the temperary files in
# the path /tmp/Orc8r_temp/
...
def un_install(pwd):
    ...
    execute_cmd(
        "helm uninstall prometheus stable/prometheus-operator --namespace kubevirt")
    execute_cmd(
        "kubectl delete -f $PWD/../helm/templates/monitoring.coreos.com_thanosrulers.yaml -n kubevirt")
    execute_cmd(
        "kubectl delete -f $PWD/../helm/templates/monitoring.coreos.com_servicemonitors.yaml -n kubevirt")
    execute_cmd(
        "kubectl delete -f $PWD/../helm/templates/monitoring.coreos.com_prometheusrules.yaml -n kubevirt")
    execute_cmd(
        "kubectl delete -f $PWD/../helm/templates/monitoring.coreos.com_prometheuses.yaml -n kubevirt")
    execute_cmd(
        "kubectl delete -f $PWD/../helm/templates/monitoring.coreos.com_podmonitors.yaml -n kubevirt")
    execute_cmd(
        "kubectl delete -f $PWD/../helm/templates/monitoring.coreos.com_alertmanagers.yaml -n kubevirt")
    myprint(
        "BLUE",
        "*****Trying to Cleanup the temporay files & Directories created as part of installation*****")
...
def del_route(pwd):
    myprint(
        "WARNING",
        "*****Trying to Un-install all 3 magma Virtual Machines*****")
...
def del_iptables(pwd):
    myprint(
        "BLUE",
        "*****Trying to delete iptables rules added as part of installation*****")
...
def del_vms():
    ...
    myprint(
        "BLUE",
        "*****Revert the changes like remove magmadev VM IP from endpoint.yml, service.yml files*****")
...
def del_dvs(pwd):
    ...
    myprint(
        "BLUE",
        "*****Deleting PersistantVolumes [PVs] which are created for upload the Debian Image*****")
...
def del_cdi():
    myprint(
        "BLUE",
        "*****Deleting EFK [ElasticSearch Fluentd Kibana] configurations*****")
    execute_cmd(
        "kubectl delete -f $PWD/../helm/templates/kubevirt_efkchart.yaml -n kubevirt")
...
def del_kubevirt():
    ...
    execute_cmd(
        "kubectl delete -f $PWD/../helm/templates/kubevirt-operator.yaml")
...
def del_temp_files():
    myprint(
        "BLUE",
        "*****Deleting Temperary files and directories created as part of VM installation*****")
    del_files(INFRA_SOFTWARE_VER)
...
def check_status(obj, namespace):
    ...
    if obj == "pod":
        cmd = "kubectl get pods -n " + namespace + " | awk " + \
            "'{{if ($3 ~ " + '!"Running"' + " || $3 ~ " + '!"STATUS"' + ") print $1,$3};}' > " + K8S_GET_OBJ
    elif obj == "deployment":
        cmd = "kubectl get deployment -n " + namespace + " | awk " + \
            "'{{if ($2 ~ " + '!"1"' + " || $2 ~ " + '!"READY"' + ") print $1,$2};}' > " + K8S_GET_OBJ
    execute_cmd(cmd)
    ...
    with open(K8S_GET_OBJ) as fop:
        while True:
            ...
            myprint(
                "WARNING",
                obj +
                "is not yet Running, please wait for a while")
            time.sleep(5)
            check_status(obj, namespace)

# thread1 : Getting the status of k8s objects like deployment and updating
# the k8s_obj_dict dictionary
...
def get_status(lock):
    while True:
        ...
        for values in k8s_obj_dict.values():
            ...
            cmd = "kubectl get deployment -n kubevirt | awk " + \
                "'{{if ($2 ~ " + '!"1"' + " || $2 ~ " + '!"READY"' + ") print $1,$2};}' > " + K8S_GET_DEP
            execute_cmd(cmd)
            with open(K8S_GET_DEP) as fop1:
                while True:
                    ...
                    for key in k8s_obj_dict.keys():
                        # Checking whether any key matches with deployment
                        # which are not in Running state
                        if re.search(k8s_obj_name_list1[0], key):
                            myprint(
                                "WARNING",
                                "Few k8s Objects not Running YET!! Be patient, Please wait for a while")
                            # Get the latest status of all the deployments
                            cmd = "kubectl get deployment -n kubevirt | awk " + \
                                "'{{if (NR != 1) print $1,$2};}' > " + K8S_GET_SVC
                            execute_cmd(cmd)
                            with open(K8S_GET_SVC) as fop2:
                                while True:
                                    ...
                                    k8s_obj_name_list2 = k8s_obj_file2_line.split(
                                        ' ')
                                    # Update the latest status of deployment
                                    # into the k8s_obj_dict dictionary
                                    if re.search(
                                            k8s_obj_name_list1[0], k8s_obj_name_list2[0]):
                                        lock.acquire()
...
def configure_alert_manager():
    myprint(
        "BLUE",
        "*****Get the magmadev VM IP and update in service.yml, endpoint.yml to get the alerts from magmadev VM*****")
    MAGMA_DEV_VM_IP = get_magmadev_vm_ip()
    os.chdir(TEMPLATES_DIR)
    for line in fileinput.input("endpoint.yml", inplace=True):
        if "ip" in line:
    ...
    myprint(
        "BLUE",
        "*****Applying the yaml files required to get the alerts from magmadev VM*****")
...
# From the k8s services updating k8s_obj_dict dictionary and creating
# get_status, get_ports threads


def start_to_run():
    cmd = "kubectl get services -n kubevirt | awk " + \
        "'{{if ($5 ~ " + '"TCP"' + " || $5 ~ " + '"UDP"' + ") print $1, $5};}' > " + K8S_GET_SVC
    execute_cmd(cmd)
    # Initializing the k8s_obj_dict with default values list[0, 0] for each
    # key:k8s_obj_name
...
def run_services():
    ...
    execute_cmd(
        "kubectl apply -f $PWD/../helm/templates/monitoring.coreos.com_alertmanagers.yaml -n kubevirt")
    execute_cmd(
        "kubectl apply -f $PWD/../helm/templates/monitoring.coreos.com_podmonitors.yaml -n kubevirt")
    execute_cmd(
        "kubectl apply -f $PWD/../helm/templates/monitoring.coreos.com_prometheuses.yaml -n kubevirt")
    execute_cmd(
        "kubectl apply -f $PWD/../helm/templates/monitoring.coreos.com_prometheusrules.yaml -n kubevirt")
    execute_cmd(
        "kubectl apply -f $PWD/../helm/templates/monitoring.coreos.com_servicemonitors.yaml -n kubevirt")
    execute_cmd(
        "kubectl apply -f $PWD/../helm/templates/monitoring.coreos.com_thanosrulers.yaml -n kubevirt")
    execute_cmd(
        "kubectl apply -f $PWD/../helm/templates/kubevirt_efkchart.yaml -n kubevirt")
    time.sleep(3)
    execute_cmd(
        "helm install prometheus stable/prometheus-operator --namespace kubevirt")
    myprint(
        "FAIL",
        "change type(key) value from 'ClusterIP' to 'NodePort' and save it")
    time.sleep(3)
    execute_cmd(
        "kubectl edit service/prometheus-prometheus-oper-alertmanager -n kubevirt")
    myprint(
        "FAIL",
        "change type(key) value from 'ClusterIP' to 'NodePort' and save it")
    time.sleep(3)
    ...
    time.sleep(3)
    execute_cmd(
        "kubectl edit service/prometheus-prometheus-oper-prometheus -n kubevirt")
...
# Install multus plugin which will be used for creating multiple
# interfaces in VM in addition to the default interfaces
...
# Install kubevirt which allows to run virtual machines alongside your
# containers on a k8s platform


def install_kubevirt():
    myprint(
        "BLUE",
        '*****Installing KubeVirt which allows to run virtual machines along with containers in k8s platform*****')
...
# Install Containerized Data Importer [CDI] used to import VM images to
# crate and control PVC


def install_cdi():
    myprint(
        "BLUE",
        "*****Installing COntainerized Data Importer[CDI] used to import VM images to create PVC*****")
...
# Install virtctl which is used to create DV,PVC to upload disk.img also
# used to connect and control VM via CLI


def install_virtctl(pwd):
    myprint(
        "BLUE",
        '*****Installing virtctl which is used to create DV,PVC to upload disk.img*****')
    os.chdir(TEMPLATES_DIR)
    cmd = "{ sleep 0.1; echo '%s'; } | script -q -c 'sudo install virtctl /usr/local/bin' /dev/null" % pwd
    execute_cmd(cmd)
    os.chdir(CWD)

# Create Bridges which are required to communicate between Host to VM and
# VM to VM
...
def create_bridges(pwd):
    myprint(
        "BLUE",
        "*****Creating Bridges required to communicate between Host to VM and VM to VM*****")
...
# Creating NetworkAttachmentDefinition to configure Network Attachment
# with a L2 Bridge and Vlan


def create_network_attachment_definition():
    myprint(
        "BLUE",
        "*****Creating NetworkAttachmentDefinition to configure Network Attachment with a L2 Bridge*****")
    execute_cmd("kubectl create -f $PWD/../helm/templates/net_attach_def.yml")

# Generate ssh-key and inject to debian qcow2 image to make use of
# passwordless authentication via root user
...
def generate_ssh_public_key(pwd):
    ...
    if not os.path.exists(DEBIAN_QCOW2_FILE):
        ...
        myprint(
            "BLUE",
            "*****debian-9-openstack-amd64.qcow2 image is already present under magma/cn/deploy/helm/templates/ directory so skipping download!!*****")
    myprint(
        "BLUE",
        "*****Generating password-less SSH key and inject to debian qcow2 image*****")
...
# Creating DataVolumes for magmadev, magmatest, magmatraffic VMs, These
# DataVolumes will mount corresponding PVC


def create_datavolume(pwd):
    myprint("BLUE", "*****Creating DataVolumes to mount debian qcow2 image *****")
    # Get the cdi_uplodproxy service IP address which is used to frame URL to
    # image-upload
    ...
    myprint(
        "BLUE",
        "*****Create directories under /mnt to store disk.img under /mnt from the mounted PVC *****")
    ...
    # Create DataVolume[dv] which will mount the debian qcow2 disk.img to
    # corresponding mounted path under /mnt
    myprint(
        "BLUE",
        "*****Create DataVolume[dv] which will mount the debian qcow2 disk.img to directory under /mnt*****")
    try:
...
def get_help(color):
    ...
    myprint(
        color,
        './MVC2_5G_Orc8r_deployment_script.py --password <sudo-password> --install')
    myprint(
        color,
        './MVC2_5G_Orc8r_deployment_script.py --password <sudo-password> --uninstall')
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(
            argv, "hiup:", [
                "help", "install", "uninstall", "password="])
