    ...
    try:
        ...
        if out == 0:
            ...
            with open(INFRA_SOFTWARE_VER) as fop2:
                ...
                logging.debug(
                    "%s %s %s" % (
                        kubeadm_version[2].split(
                            '{',
                        )[1], kubeadm_version[3], kubeadm_version[4],
                    ),
                )
        else:
            raise NotInstalled
...
def create_docker_images(pwd):
    ...
    shutil.copyfile(
        os.path.join(MAGMA_DOCKER, 'amf_dockerfile'),
        os.path.join(BUILD_CONTEXT_AMF, 'Dockerfile'),
    )
    ...
    shutil.copyfile(
        os.path.join(MAGMA_DOCKER, 'smf_dockerfile'),
        os.path.join(BUILD_CONTEXT_SMF, 'Dockerfile'),
    )
    ...
    shutil.copyfile(
        os.path.join(MAGMA_DOCKER, 'upf_dockerfile'),
        os.path.join(BUILD_CONTEXT_UPF, 'Dockerfile'),
    )
                ...
                while True:
                    ...
                    for key in k8s_obj_dict.keys():
                        ...
                        if re.search(k8s_obj_name_list1[0], key):
                            myprint(
                                "WARNING", "Few k8s Objects not Running YET!! Be patient, Please wait for a while",
                            )
            ...
            if len(ports_list[0].split(':')) > 1:
                for key in k8s_obj_dict.keys():
                    if re.search(k8s_obj_name_list[0], key):
                        k8s_obj_dict.setdefault(key, []).append(
...
def get_help(color):
    ...
    myprint(color, '    (OR)   ')
    myprint(
        color, './mvc1_5g_orc8r_deployment_script.py --password <sudo-password> --install',
    )
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(
            argv, "hiup:", ["help", "install", "uninstall", "password="],
        )
