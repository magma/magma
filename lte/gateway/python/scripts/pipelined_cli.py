...
def set_smf_session(client, args):
    cls_sess = CreateSessionUtil(
        args.subscriber_id, args.session_id, args.version,
    )
...
def create_ng_services_parser(apps):
    ...
    subcmd.add_argument(
        '--subscriber_id',
        help='Subscriber Identity', default='IMSI12345',
    )
    subcmd.add_argument(
        '--session_id', help='Session Identity', type=int, default=100,
    )
    subcmd.add_argument(
    ...
    subcmd.add_argument(
        '--allow', help='YES/NO for allow and deny', default='YES',
    )
    subcmd.add_argument(
...
def create_pg_services_parser(apps):
    ...
    subcmd.add_argument(
        '--imsi', help='Subscriber Identity', default='IMSI12345',
    )
    subcmd.add_argument(
        '--priority', help='priority for rule',
        type=int, default=10,
    ...
    subcmd.add_argument(
        '--flow_dl', help='ENABLE/DISABLE flow dl', default='DISABLE',
    )
...
def create_enforcement_parser(apps):
    ...
    subcmd = subparsers.add_parser(
        'pull_stats_grpc', help='Have RuleRecords returned based on matching cookie and cookie mask',
    )
