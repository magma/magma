...
from feg.protos.mock_core_pb2_grpc import MockPCRFStub
from integ_tests.gateway.rpc import get_pcrf_rpc_channel
...
class PCRFGrpcClient(Exception):
    def __init__(self, grpc_stub):
        """ Init the gRPC stub.  """
        self._mock_pcrf_stub = grpc_stub

    def clear_subscribers(self):
        print("Clearing account in PCRF")
        self._mock_pcrf_stub.ClearSubscribers(Void())

    def add_subscriber(self, imsi):
        imsi = imsi.replace("IMSI", "")
        print("Creating account in PCRF for IMSI -", imsi)
