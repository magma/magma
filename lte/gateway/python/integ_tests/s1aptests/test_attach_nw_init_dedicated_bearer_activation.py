...
class TestDedicatedBearer(unittest.TestCase):
    ...
    def test_dedicated_bearer_activation(self):
        ...
        time.sleep(0.2)
        # Wait on Activate Dedicated Bearer Request from MME
        response = self._s1ap_wrapper.s1_util.get_response()
        self.assertTrue(response, s1ap_types.tfwCmd.UE_ACT_DED_BER_REQ.value)
        print("************************* Rcvd Activate Dedicated bearer Req")
        ...
        print("************************* Sent Activate Dedicated bearer Rsp")
        time.sleep(1)
