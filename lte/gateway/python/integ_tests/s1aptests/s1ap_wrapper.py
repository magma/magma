...
class TestWrapper(object):
    ...
    def _enBConfig(self):
        ...
        assert (
            self._s1_util.issue_cmd(s1ap_types.tfwCmd.ENB_CONFIG, req) == 0)
    ...
    def _s1setup(self):
        ...
        while (retry < TestWrapper.MSX_S1_RETRY):
            if res.res == s1ap_types.S1_setp_Result.S1_SETUP_FAILED.value and \
                            res.waitIe.pres == 1:
                print("Received time to wait in S1-Setup-Failure"
        ...
        assert (res.res ==
                s1ap_types.S1_setp_Result.S1_SETUP_SUCCESS.value)
    ...
    def _configUEApp(self):
        ...
        assert (self._s1_util.issue_cmd(
            s1ap_types.tfwCmd.UE_APPL_CONFIG, req) == 0)
        response = self._s1_util.get_response()
