...
from orc8r.protos.magmad_pb2 import ServiceState
...
class MagmadUtil(object):
    ...
    def is_service_active(self, service) -> bool:
        ...
        """
        service_status = self._magmad_client.get_service_status(service)
        return service_status.state == ServiceState.ACTIVE
    ...
    def restart_service_and_wait(self, service: str, timeout: int = 30, retry: bool = True):
        print("Restarting %s service on gateway" % service)
        self.restart_services([service])
        print("Waiting for %s to restart. %s sec" % (service, timeout))
        for j in range(timeout):
            print("Waiting for", j, "seconds")
            time.sleep(1)
        if retry and not self.is_service_active(service):
            print('Retrying since service is not yet active')
            for j in range(timeout):
                print("Waiting for", j, "seconds")
                time.sleep(1)
...
class SessionManagerUtil(object):
    ...
    def _print_directoryd_content(self):
        try:
            ...
            print("error: couldnt print directoryd content. gRPC failed with %s: %s" % (
                e.code(), e.details()))
            return
