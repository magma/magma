...
class TestSecondaryPdnConnWithDedBearerLooped(unittest.TestCase):
    ...
    def test_seconday_pdn_conn_ded_bearer_looped(self):
        ...
        for i in range(num_ues):
            ...
            for j in range(loop):
                ...
                self.assertEqual(
                    response.msg_type, s1ap_types.tfwCmd.UE_PDN_CONN_RSP_IND.
                    value)
                act_def_bearer_req = response.cast(s1ap_types.uePdnConRsp_t)

                print("********************** Sending Activate default EPS "
                      "bearer context accept for UE id ", ue_id)
                ...
                response = self._s1ap_wrapper.s1_util.get_response()
                self.assertTrue(response, s1ap_types.tfwCmd.
