...
class TestSecondaryPdnDisConnRej(unittest.TestCase):
    ...
    def test_seconday_pdn_disconn_rej(self):
        """ Attach a single UE +  add secondary pdn + pdn disconnect reject """
        ...
        act_def_bearer_acc.bearerId = act_def_bearer_req.m.pdnInfo.epsBearerId
        self._s1ap_wrapper._s1_util.issue_cmd(
                s1ap_types.tfwCmd.UE_ACTV_DEFAULT_EPS_BEARER_CNTXT_ACCEPT,
                act_def_bearer_acc)

        time.sleep(5)
        # Send PDN Disconnect with wrong default bearer id - 10
        pdn_disconnect_req = s1ap_types.uepdnDisconnectReq_t()
        pdn_disconnect_req.ue_Id = ue_id
        pdn_disconnect_req.bearerId = 10
        self._s1ap_wrapper._s1_util.issue_cmd(s1ap_types.tfwCmd.
                                              UE_PDN_DISCONNECT_REQ,
                                              pdn_disconnect_req)

        # Receive UE_PDN_DISCONNECT_REJ
        response = self._s1ap_wrapper.s1_util.get_response()
        self.assertTrue(response, s1ap_types.tfwCmd.UE_PDN_DISCONNECT_REJ.
                        value)
        print("******************* Received UE_PDN_DISCONNECT_REJ from NW")
