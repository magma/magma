...
class TestS1HandoverTimerExpiry(unittest.TestCase):
    ...
    def test_s1_handover_timer_expiry(self):
        """S1 Handover Unsuccessful Scenario(S1 HO Cancel due to Timer Expiry):
        ...
        3) Receive and handle the S1 HO Request message in the target ENB
        4) Drop the incoming S1 HO Command msg in source ENB for timer expiry
        5) After S1 Reloc Timer Expiry, source ENB sends S1 HO Cancel to MME
        6) Handle the received S1 HO Cancel Acknowledgement in Source ENB
        7) S1 HO has failed. UE is still attached to ENB 1. Detach the UE
        ...
        # After receiving S1 Handover Command, source ENB drops the Handover
        ...
        print(
            ...
            "Indication (UeId: "
            + str(s1ho_cmd_drp_ind.ueId)
            + ", Connected EnbId: "
            + str(s1ho_cmd_drp_ind.currEnbId)
            + ") (HO SrcEnbId: "
            + str(s1ho_cmd_drp_ind.hoSrcEnbId)
            + ", HO TgtEnbId: "
            + str(s1ho_cmd_drp_ind.hoTgtEnbId)
            + ")",
        )
