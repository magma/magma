...
class TestAttachIcsDropWithMmeRestart(unittest.TestCase):
    ...
    def test_attach_ics_drop_with_mme_restart(self):
        ...
        print("*** Setting Initial Context Setup Req Drop falg ***")
        ...
        print("*** Received Initial Context Setup Drop Indication ***")

        print("************************* Restarting MME service on gateway")
        self._s1ap_wrapper.magmad_util.restart_services(["mme"])

        print("*** Resetting Initial Context Setup Req Drop falg ***")
        drop_init_ctxt_setup_req = s1ap_types.UeDropInitCtxtSetup()
        drop_init_ctxt_setup_req.ue_Id = req.ue_id
        ...
        print(
            "*** Waiting for NW initiated Detach Req or "
        ...
        while True:
            ...
            if (
                ...
                print(
                    "**************** Received NW initiated Detach Req with "
                    "detach type set to ",
                ...
                if resp_count == 1:
                    ...
                    print(
                        "**************** Sending UE triggered Detach Accept"
                    )
                    self._s1ap_wrapper._s1_util.issue_cmd(
                    ...
                    print(
                        "**************** Ignoring repeated NW initiated "
                        "detach req message"
                    )
            else:
                break
