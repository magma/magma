...
class Test3485TimerForDedicatedBearerWithMmeRestart(unittest.TestCase):
    ...
    def test_3485_timer_for_dedicated_bearer_with_mme_restart(self):
        ...
        for i in range(num_ues):
            ...
            print("receved masg", s1ap_types.tfwCmd.UE_ERAB_DROP_IND.value)
            print("receved masg1", response.msg_type)
            self.assertEqual(
                response.msg_type, s1ap_types.tfwCmd.UE_ERAB_DROP_IND.value
            )

            print(
                "*******************Received ERAB_DROP_IND For dedicated ",
                "bearer request",
            )

            print(
                "*** Sending indication to disbale dropping of Erab Setup ",
                "Request ***",
            )
            drop_erab_setup_req = s1ap_types.UeDropErabSetupReq_t()
            drop_erab_setup_req.ue_Id = req.ue_id
            ...
            print(
                "*******************Received Activate dedicated bearer "
                "request"
            )
