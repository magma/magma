...
from magma.subscriberdb.store.base import SuciProfileNotFoundError
...
class M5GSUCIRegRpcServicer(subscriberdb_pb2_grpc.M5GSUCIRegistrationServicer):
    ...
    def __init__(self, lte_processor, suciprofile_db: dict, print_grpc_payload: bool = False):
        self.lte_processor = lte_processor
    ...
    def M5GDecryptImsiSUCIRegistration(self, request, context):
        ...
        try:
            ...
            msin_recv = home_network_info.unprotect(
                request.ue_pubkey, request.ue_ciphertext, request.ue_encrypted_mac)

            aia.ue_msin_recv = msin_recv[0:10]
