...
class RpcTestsSuciExt(unittest.TestCase):
    ...
    def test_add_delete_suciprofile(self):
        ...
        request = SuciProfile(home_net_public_key_id = home_net_public_key_id,
        ...
        # See if we can get the data for the subscriber
        self.assertEqual(len(self._stub.ListSuciProfile(Void()).sp), 1)
        self.assertEqual(self._stub.ListSuciProfile(Void()).sp[0].home_net_public_key_id, 2)
        self.assertEqual(self._stub.ListSuciProfile(Void()).sp[0].protection_scheme, SuciProfile.ProfileA)

        # Delete the subscriber
        request = SuciProfile(home_net_public_key_id = int(home_net_public_key_id),)
        self._stub.DeleteSuciProfile(request)
        self.assertEqual(len(self._stub.ListSuciProfile(Void()).sp), 0)
