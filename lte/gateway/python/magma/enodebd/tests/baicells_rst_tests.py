...
class BAICELLS_RTSHandlerTests(EnodebHandlerTestCase):
    def test_initial_enb_bootup(self) -> None:
        """
        BAICELLS_RTS does not support configuration during initial bootup of
        ...
        InformResponse.
        """
        acs_state_machine = EnodebAcsStateMachineBuilder.build_acs_state_machine(EnodebDeviceName.BAICELLS_RTS)
    ...
    def test_gps_coords(self) -> None:
        ...
        param_val_list = [
            Tr069MessageBuilder.get_parameter_value_struct(
                name='Device.X_BAICELLS_RTS_COM_GpsSyncEnable',
                val_type='boolean',
                data='true',
            ),
        ]
    ...
    def test_missing_param_during_provisioning(self) -> None:
        ...
        # Pretend that here the NumPLMNs was not sent because of a BAICELLS_RTS bug
        req = Tr069MessageBuilder.get_rts_regular_param_values_response(
            admin_state=False, earfcndl=39150,
    ...
    def test_provision_multi_without_invasive_changes(self) -> None:
        ...
        """
        acs_state_machine = EnodebAcsStateMachineBuilder.build_multi_enb_acs_state_machine(
            EnodebDeviceName.BAICELLS_RTS,
        )
        ...
        # Send back typical values for the regular parameters
        req = Tr069MessageBuilder.get_rts_regular_param_values_response(admin_state=False, earfcndl=39150)
        resp = acs_state_machine.handle_tr069_message(req)
    ...
    def test_provision_without_invasive_changes(self) -> None:
        ...
        inform_msg = Tr069MessageBuilder.get_inform()
        resp = acs_state_machine.handle_tr069_message(inform_msg)
        self.assertTrue(isinstance(resp, models.InformResponse), 'Should respond with an InformResponse')
    ...
    def test_reboot_after_invasive_changes(self) -> None:
        ...
        - This test does not wait the ten minutes to simulate REM process
          finishing on the BAICELLS_RTS eNodeB

        'Invasive' parameters are those which require special behavior to apply
        the changes for the eNodeB.

        In the case of the BAICELLS_RTS eNodeB, properly applying changes to
        ...
        # to the BAICELLS_RTS eNodeB, we need to reboot the device
        ...
        self.assertTrue(
            isinstance(resp, models.DummyInput),
            'After sending command to reboot the BAICELLS_RTS eNodeB, '
