...
from magma.enodebd.device_config.configuration_init import (
    _get_enb_config,
    _set_algorithm_x2_enable_disable,
    _set_bandwidth,
    ...
    _set_perf_mgmt,
    _set_plmnids_tac,
    _set_power_control_config,
    _set_s1_connection,
    _set_tdd_subframe_config,
)
from magma.enodebd.device_config.enodeb_configuration import EnodebConfiguration
from magma.enodebd.devices.baicells import BaicellsTrDataModel
from magma.enodebd.devices.baicells_qafb import BaicellsQAFBTrDataModel
from magma.enodebd.devices.device_utils import EnodebDeviceName
from magma.enodebd.exceptions import ConfigurationError
from magma.enodebd.tests.test_utils.config_builder import EnodebConfigBuilder


class qafbEnodebConfigurationFactoryTest(TestCase):

    def setUp(self):
        self.data_model = BaicellsQAFBTrDataModel()
        self.cfg = EnodebConfiguration(BaicellsQAFBTrDataModel())
        self.device_cfg = EnodebConfiguration(BaicellsQAFBTrDataModel())
    ...
    def test_get_enb_config(self):
        mconfig = EnodebConfigBuilder.get_mconfig(EnodebDeviceName.BAICELLS_QAFB)
        self.device_cfg.set_parameter(
        ...
        self.assertEqual(
            enb_config.x2_enable_disable, False,
            "Should give x2_enable_disable from default eNB config",
        )
        enb_config = _get_enb_config(mconfig, self.device_cfg)
        self.assertTrue(
            enb_config.power_control.reference_signal_power == -24,
            "Should give reference_signal_power from default eNB config",
        )
        enb_config = _get_enb_config(mconfig, self.device_cfg)
        self.assertTrue(
            enb_config.power_control.power_class == 22,
            "Should give power_class from default eNB config",
        )
        enb_config = _get_enb_config(mconfig, self.device_cfg)
        self.assertTrue(
            enb_config.power_control.pb == 1,
            "Should give pb from default eNB config",
        )
        enb_config = _get_enb_config(mconfig, self.device_cfg)
        self.assertTrue(
            enb_config.power_control.pa == 100,
            "Should give pa from default eNB config",
        )

    def test_set_algorithm_x2_enable_disable(self):
        # We only handle x2
        mconfig = EnodebConfigBuilder.get_mconfig(EnodebDeviceName.BAICELLS_QAFB)
        self.device_cfg.set_parameter(
            ParameterName.SERIAL_NUMBER,
        ...
        enb_config = _get_enb_config(mconfig, self.device_cfg)
        x2_config = enb_config.x2_enable_disable
        _set_algorithm_x2_enable_disable(self.cfg, self.data_model, x2_config)
        self.assertEqual(
            self.cfg.get_parameter(BaicellsParameterName.X2_ENABLE_DISABLE), False,
            'X2 Should be disabled ',
        )

    def test_set_power_control_config(self):
        mconfig = EnodebConfigBuilder.get_mconfig(EnodebDeviceName.BAICELLS_QAFB)
        self.device_cfg.set_parameter(
            ParameterName.SERIAL_NUMBER,
            '1202000181186TB0006',
        ...
        enb_config = _get_enb_config(mconfig, self.device_cfg)
        power_config = enb_config.power_control
        _set_power_control_config(self.cfg, power_config)
        self.assertEqual(
            self.cfg.get_parameter(BaicellsParameterName.REFERENCE_SIGNAL_POWER), -24,
            'Should support a REFERENCE_SIGNAL_POWER parameter',
        )
        self.assertEqual(
            self.cfg.get_parameter(BaicellsParameterName.POWER_CLASS), 22,
            'Should support a POWER_CLASS parameter',
        )
        self.assertEqual(
            self.cfg.get_parameter(BaicellsParameterName.PA), 100,
            'Should support a PA parameter',
        )
        self.assertEqual(
