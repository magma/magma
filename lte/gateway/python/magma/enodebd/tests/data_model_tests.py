...
class BaicellsTrDataModelTest(TestCase):
    ...
    def test_get_parameter_names(self):
        ...
        self.assertIn(ParameterName.MME_STATUS, name_list,
                      'Should have %s in parameter name list' %
                      ParameterName.MME_STATUS)
        self.assertIn(ParameterName.DUPLEX_MODE_CAPABILITY, name_list,
                      'Should have %s in parameter name list' %
                      ParameterName.DUPLEX_MODE_CAPABILITY)
        self.assertIn(ParameterName.EARFCNUL, name_list,
                      'Should have %s in parameter name list' %
    ...
    def test_get_numbered_param_names(self):
        ...
        self.assertNotIn(ParameterName.EARFCNDL, name_list,
                         'Should not have %s in parameter name list' %
                         ParameterName.EARFCNDL)
        self.assertNotIn(ParameterName.MME_PORT, name_list,
                         'Should not have %s in parameter name list' %
                         ParameterName.MME_PORT)
        self.assertNotIn(ParameterName.PERIODIC_INFORM_ENABLE, name_list,
                         'Should not have %s in parameter name list' %
                         ParameterName.PERIODIC_INFORM_ENABLE)

        # Check that some numbered parameters are present
        self.assertIn(ParameterName.PLMN_N % 1, name_list,
                      'Should have %s in parameter name list' %
                      ParameterName.PLMN_N % 1)
        self.assertIn(ParameterName.PLMN_N % 6, name_list,
                      'Should have %s in parameter name list' %
