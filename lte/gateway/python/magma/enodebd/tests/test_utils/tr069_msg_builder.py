...
class Tr069MessageBuilder:
    ...
    def get_cavium_object_param_values_response(
        ...
        for i in range(1, num_plmns + 1):
            param_val_list.append(
                cls.get_parameter_value_struct(
                    name='Device.Services.FAPService.1.CellConfig.LTE.EPC.PLMNList.%d.IsPrimary' %
                    i, val_type='boolean', data='true', ))
            param_val_list.append(
                cls.get_parameter_value_struct(
                    name='Device.Services.FAPService.1.CellConfig.LTE.EPC.PLMNList.%d.CellReservedForOperatorUse' %
                    i, val_type='boolean', data='false', ))
            param_val_list.append(
                cls.get_parameter_value_struct(
                    name='Device.Services.FAPService.1.CellConfig.LTE.EPC.PLMNList.%d.PLMNID' %
                    i, val_type='string', data='00101', ))
            param_val_list.append(
                cls.get_parameter_value_struct(
                    name='Device.Services.FAPService.1.CellConfig.LTE.EPC.PLMNList.%d.Enable' %
                    i, val_type='boolean', data='true', ))
        msg.ParameterList = models.ParameterValueList()
        msg.ParameterList.ParameterValueStruct = param_val_list
        return msg
