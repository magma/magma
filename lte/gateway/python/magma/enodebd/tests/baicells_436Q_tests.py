...
from dp.protos.enodebd_dp_pb2 import (
    CBSDStateRequest,
    CBSDStateResult,
    LteChannel,
)
...
from magma.enodebd.devices.baicells_436Q import (
    Baicells436NotifyDPState,
    Baicells436QQueuedEventsWaitState,
    Baicells436QTrDataModel,
    Baicells436QWaitInformRebootState,
    update_desired_config_based_on_sas_grant,
)
...
class SasToRfConfigTests(TestCase):
    def test_bandwidth_20MHz(self) -> None:
        config = EnodebConfiguration(Baicells436QTrDataModel())
        channel = LteChannel(
            low_frequency_hz=3550_000_000,
            high_frequency_hz=3570_000_000,
            max_eirp_dbm_mhz=34,
        ...
        update_desired_config_based_on_sas_grant(state, config)
...
class Baicells436QHandlerTests(EnodebHandlerTestCase):

    @mock.patch('magma.enodebd.devices.baicells_436Q.get_cbsd_state')
    def test_notify_dp_sets_values_received_by_dp_in_desired_config(self, mock_get_state) -> None:
        ...
        acs_state_machine = EnodebAcsStateMachineBuilder.build_acs_state_machine(EnodebDeviceName.BAICELLS_436Q)

        acs_state_machine.desired_cfg = EnodebConfiguration(Baicells436QTrDataModel())
        ...
        req = Tr069MessageBuilder.param_values_436q_response(
        ...
        mock_get_state.assert_called_with(
            CBSDStateRequest(user_id=test_user, fcc_id=test_fcc_id, serial_number=test_serial_number),
        )
    ...
    def test_manual_reboot(self) -> None:
        ...
        inform_msg = \
            Tr069MessageBuilder.get_436Q_inform(
                DEFAULT_INFORM_PARAMS,
                '48BF74',
                '1202000181186TB0006',
                ['2 PERIODIC'],
    ...
    def test_manual_reboot_during_provisioning(self) -> None:
        ...
        inform_msg = Tr069MessageBuilder.get_436Q_inform(
            DEFAULT_INFORM_PARAMS,
            '48BF74',
            '1202000181186TB0006',
            ['2 PERIODIC'],
...
class Baicells436QStatesTests(EnodebHandlerTestCase):
    """Testing Baicells 436Q specific states"""

    @mock.patch('magma.enodebd.devices.baicells_436Q.get_cbsd_state')
    def test_end_session_and_notify_dp_transition(self, mock_get_state):
        """Testing if SM steps in and out of Baicells436QWaitNotifyDPState as per state map"""

        mock_get_state.return_value = MOCK_CBSD_STATE

        acs_state_machine = provision_clean_sm_for_testing(
            state='check_wait_get_params',
        )

        msg = Tr069MessageBuilder.param_values_436q_response(
            GET_PARAMS_RESPONSE_PARAMS,
            models.GetParameterValuesResponse,
        )
    ...
    def test_wait_post_reboot_inform_transition(self):
        """Testing if SM steps in and out of Baicells436QWaitInformRebootState as per state map"""
        acs_state_machine = provision_clean_sm_for_testing(state='wait_reboot')
    ...
    @mock.patch.object(Baicells436QQueuedEventsWaitState, 'CONFIG_DELAY_AFTER_BOOT', new=0.5)
    def test_wait_queued_events_post_reboot_transition(self):
        """Testing if we step in and out of Baicells436QQueuedEventsWaitState as per state map"""
        acs_state_machine = provision_clean_sm_for_testing(
...
class Baicells436QConfigTests(EnodebHandlerTestCase):
    ...
    def test_device_and_desired_config_discrepancy_after_initial_configuration(self):
        ...
        acs_state_machine = provision_clean_sm_for_testing('wait_get_transient_params')
...
def provision_clean_sm_for_testing(state=None):
    acs_state_machine = EnodebAcsStateMachineBuilder.build_acs_state_machine(EnodebDeviceName.BAICELLS_436Q)
