...
from dp.protos.enodebd_dp_pb2 import CBSDStateRequest, CBSDStateResult
...
from magma.enodebd.logger import EnodebdLogger as logger  # noqa
...
class Baicells436QHandler(BasicEnodebAcsStateMachine):
    """
    Baicells436Q State Machine
    """
    ...
    def _init_state_map(self) -> None:
        self._state_map = {
            # RemWait state seems not needed for 436Q
            ...
            'end_session': Baicells436EndSessionState(self, when_done='notify_dp'),
            'notify_dp': Baicells436NotifyDPState(self, when_inform='wait_inform'),
            ...
            'wait_post_reboot_inform': Baicells436QWaitInformRebootState(
                self,
                when_done='wait_queued_events_post_reboot',
                when_timeout='wait_inform_post_reboot',
            ),
            "wait_queued_events_post_reboot": Baicells436QQueuedEventsWaitState(
                self,
    ...
    def data_model_class(self) -> Type[DataModel]:
        ...
        Returns:
            Type[DataModel]
        """
        return Baicells436QTrDataModel
    ...
    def config_postprocessor(self) -> EnodebConfigurationPostProcessor:
        ...
        Returns:
            EnodebConfigurationPostProcessor
        """
        return Baicells436QTrConfigurationInitializer()
...
class Baicells436EndSessionState(EnodebAcsState):
    """ To end a TR-069 session, send an empty HTTP response

    For Baicells 436Q we can expect an inform message on
...
BAND = 48
BAND_48_NOFFS_DL = 55240  # noqa
BAND_48_LOW_FREQ_MHZ = 3550  # noqa
SAS_BANDWIDTHS = (5, 10, 15, 20)
MIN_POWER_SPECTRAL_DENSITY = -137
MAX_POWER_SPECTRAL_DENSITY = 37


def update_desired_config_based_on_sas_grant(state: CBSDStateResult, config: EnodebConfiguration) -> None:
    """
    ...
    earfcn = _calc_earfcn(state.channel.low_frequency_hz, state.channel.high_frequency_hz)
    bandwidth_mhz = _calc_bandwidth_mhz(state.channel.low_frequency_hz, state.channel.high_frequency_hz)
    bandwidth_rbs = _calc_bandwidth_rbs(bandwidth_mhz)
    psd = _calc_psd(state.channel.max_eirp_dbm_mhz)

    config.set_parameter(ParameterName.BAND, BAND)
    config.set_parameter(ParameterName.DL_BANDWIDTH, bandwidth_rbs)
    config.set_parameter(ParameterName.UL_BANDWIDTH, bandwidth_rbs)
    config.set_parameter(ParameterName.EARFCNDL, earfcn)
    config.set_parameter(ParameterName.EARFCNUL, earfcn)
    config.set_parameter(ParameterName.POWER_SPECTRAL_DENSITY, psd)


def _calc_psd(eirp: float) -> int:
    psd = int(eirp)
    if not MIN_POWER_SPECTRAL_DENSITY <= psd <= MAX_POWER_SPECTRAL_DENSITY:  # noqa
        raise ConfigurationError('Power Spectral Density %d exceeds allowed range [%d, %d]' %
                                 (psd, MIN_POWER_SPECTRAL_DENSITY, MAX_POWER_SPECTRAL_DENSITY))
    return psd


def _calc_bandwidth_mhz(low_freq_hz: int, high_freq_hz: int) -> float:
...
def _calc_bandwidth_rbs(bandwidth_mhz: float) -> str:
    bandwidth_rbs = int(5 * bandwidth_mhz)
    return str(bandwidth_rbs)


def _calc_earfcn(low_freq_hz: int, high_freq_hz: int) -> int:
    mid_frequency_mhz = (high_freq_hz + low_freq_hz) / 2e6
    earfcn = 10 * (mid_frequency_mhz - BAND_48_LOW_FREQ_MHZ) + BAND_48_NOFFS_DL
    return int(earfcn)


class Baicells436NotifyDPState(EnodebAcsState):
    """ Notify DP ...
    ...
    def enter(self):
        ...
        """
        request = CBSDStateRequest(
            user_id=self.acs.device_cfg.get_parameter(ParameterName.SAS_USER_ID),
            fcc_id=self.acs.device_cfg.get_parameter(ParameterName.SAS_FCC_ID),
            serial_number=self.acs.device_cfg.get_parameter(ParameterName.SERIAL_NUMBER),
        )
        state = get_cbsd_state(request)
        update_desired_config_based_on_sas_grant(state, self.acs.desired_cfg)
...
class Baicells436QQueuedEventsWaitState(EnodebAcsState):
    """
...
class Baicells436QWaitInformRebootState(WaitInformMRebootState):
    """
    Baicells436Q WaitInformRebootState implementation
    """
    INFORM_EVENT_CODE = '1 BOOT'


class Baicells436QTrDataModel(DataModel):
    """
    Class to represent relevant data model parameters from TR-196/TR-098/TR-181.
    This class is effectively read-only

    This is for Baicells 436Q based on software BaiBS_QRTB_2.6.2.
    Tested on hw version E01
    """
    ...
    for i in range(1, NUM_PLMNS_IN_CONFIG + 1):  # noqa
...
class Baicells436QTrConfigurationInitializer(EnodebConfigurationPostProcessor):
    """
    ...
    def postprocess(self, mconfig: Any, service_cfg: Any, desired_cfg: EnodebConfiguration) -> None:
        ...
        desired_cfg.set_parameter_for_object(
            ParameterName.PLMN_N_CELL_RESERVED % 1, True,  # noqa
            ParameterName.PLMN_N % 1,  # noqa
        )
