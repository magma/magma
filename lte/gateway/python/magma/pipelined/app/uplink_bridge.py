...
class UplinkBridgeController(MagmaController):
    ...
    def _install_ip_v4_v6_flows(
            self, priority: int, flow_match: str, flow_action: str,
    ):
        if self.config.enable_nat is True:
            return
    ...
    def _set_sgi_ip_addr(self, if_name: str):
        ...
        if self.config.sgi_management_iface_ip_addr is None or \
            ...
            if if_name == self.config.uplink_bridge:
                ...
                if_addrs = netifaces.ifaddresses(
                    if_name,
                ).get(netifaces.AF_INET, [])
                if len(if_addrs) != 0:
                    self.logger.info(
                        "SGi has valid IP, skip reconfiguration %s", if_addrs,
                    )
                    return
