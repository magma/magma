...
class Classifier(MagmaController):
    ...
    def gtp_handler(self, pdr_state, precedence:int, local_f_teid:int,
                     o_teid:int, ue_ipv4_addr:str, gnb_ip_addr:str,
                     sid:int = None, controller_id:int = 0):
        ue_ip_addr = IPAddress(version=IPAddress.IPV4, address=ue_ipv4_addr.encode('utf-8'))

        if pdr_state == PdrState.Value('INSTALL'):
            self._remove_paging_flow(ue_ipv4_addr)
            ...
            self._install_paging_flow(ue_ipv4_addr, controller_id)
