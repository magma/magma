...
def get_app_id(app: str, service_type: str) -> str:
    ...
    if len(app_match) > 1:
        logging.warning("Found more than 1 app match in %s", app)
        return DEFAULT_DPI_ID

    if (len(app_match) == 1):
        app_id = APP_PROTOS[app_match[0]]
        logging.debug("Classified %s-%s as %d", app, service_type,
                            app_id)
        return app_id
    ...
    if len(parent_match) == 0:
        logging.debug("Didn't find a match for app name %s", app)
        return DEFAULT_DPI_ID
    if len(parent_match) > 1:
        logging.debug("Found more than 1 parent app match in %s", app)
        return DEFAULT_DPI_ID
    ...
    for serv in SERVICE_IDS:
        if serv in service_type.lower():
            ...
            break
    app_id += service_id
    logging.debug("Classified %s-%s as %d", app, service_type, app_id)
    return app_id
