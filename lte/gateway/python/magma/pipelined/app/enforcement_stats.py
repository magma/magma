...
class EnforcementStatsController(PolicyMixin, MagmaController):
    ...
    def _install_flow_for_rule(self, imsi, ip_addr, apn_ambr, rule, match_teid=0, action_teid=0):
        ...
        if match_teid or action_teid:
            msgs = self._get_ng_rule_match_flow_msgs(imsi, ip_addr, apn_ambr, rule, match_teid, action_teid)
        else:
    ...
    # pylint: disable=protected-access,unused-argument
    def _get_ng_rule_match_flow_msgs(self, imsi, ip_addr, ambr, rule,
        ...
        for flow in rule.flow_list:
            ...
            rule_match = _generate_rule_match(imsi, ip_addr, rule_num,
                                              version, direction, teid)#Direction.OUT, Direction.IN
    ...
    def _get_ng_default_flow_msgs_for_subscriber(self, imsi, ip_addr, match_teid=0, action_teid=0):
        flows_list = []

        if action_teid:
            match_in = _generate_rule_match(imsi, ip_addr, 0, 0, Direction.IN, action_teid)
            flows_list.append(flows.get_add_drop_flow_msg(self._datapath, self.tbl_num, match_in,
                                                          priority=Utils.DROP_PRIORITY))

        if match_teid:
            match_out = _generate_rule_match(imsi, ip_addr, 0, 0, Direction.OUT, match_teid)
            flows_list.append(flows.get_add_drop_flow_msg(self._datapath, self.tbl_num, match_out,
                                                          priority=Utils.DROP_PRIORITY))
    ...
    def _install_default_flow_for_subscriber(self, imsi, ip_addr,
        ...
        if match_teid or action_teid:
            msgs = self._get_ng_default_flow_msgs_for_subscriber(imsi, ip_addr)
        else:
        ...
        for flow_stats in stats_msgs:
            for stat in flow_stats:
                ...
                if stat.match[DIRECTION_REG] == Direction.OUT:
                    ...
                    if local_f_teid not in session_config_dict:
                        session_config_dict.update ({local_f_teid:
