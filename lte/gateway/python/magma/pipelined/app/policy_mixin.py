...
class PolicyMixin(metaclass=ABCMeta):
    ...
    def _get_classify_rule_flow_msgs(
        ...
        flow_match = flow_match_to_magma_match(flow.match, local_f_teid_ng)
    ...
    def _get_ue_specific_flow_msgs(self, requests: List[ActivateFlowsRequest]):
        ...
        for add_flow_req in requests:
            ...
            downlink_tunnel = add_flow_req.downlink_tunnel
            ...
            if self._setup_type == 'CWF' or add_flow_req.ip_addr:
                ...
                for policy in policies:
                    msg_list.extend(self._get_policy_flows(imsi, msisdn, uplink_tunnel, ipv4, apn_ambr, policy, shard_id, downlink_tunnel))
            if add_flow_req.ipv6_addr:
                ...
                for policy in policies:
                    msg_list.extend(self._get_policy_flows(imsi, msisdn, uplink_tunnel, ipv6, apn_ambr, policy, shard_id, downlink_tunnel))

        return {self.tbl_num: msg_list}

    def _get_policy_flows(
        self, imsi, msisdn, uplink_tunnel, ip_addr, apn_ambr,
        policy, shard_id, local_f_teid_ng
    ):
        msg_list = []
        ...
        try:
            if policy.rule.redirect.support == policy.rule.redirect.ENABLED:
                return msg_list
            flow_adds = self._get_rule_match_flow_msgs(imsi, msisdn, uplink_tunnel, ip_addr, apn_ambr, policy.rule, policy.version, shard_id, local_f_teid_ng)
