...
class NodeStateManager:
    ...
    def _send_association_request_message(self, assoc_message):
        ...
        if self._send_messsage_wrapper(node_message):
    ...
    def send_association_setup_message(self):
        ...
        self._assoc_mon_thread = hub.spawn(
            self._monitor_association, assoc_message)

    def _monitor_association(
            self,
            assoc_message: UPFAssociationState,
            poll_interval: int = 3):
        """
        ...
        while assoc_established == False:
            ...
            if not assoc_established:
                retry_count += 1

                if retry_count == self.ASSOC_MAX_RETRIES:
                    logging.info(
                        " Max Attempt to SMF connection failed. Reattempting..")
                    retry_count = 0
