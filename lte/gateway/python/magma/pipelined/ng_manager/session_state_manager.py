...
class SessionStateManager:
    ...
    MsgParseOutput = namedtuple(
        'MsgParseOutput',['cause', 'identfier', 'session_version'],
    )
    ...
    def __init__(self, loop):
        ...
        logging.info("NGServices : SessionStateManager launched ")

    # Creating the dict entries for the far group
    def _pdr_create_rule_group(self, new_session, pdr_rules):

        for pdr_entry in new_session.set_gr_pdr:
            # PDR Validation
            if pdr_entry.HasField('pdi') == False or pdr_entry.pdr_id == 0:
                return (self.MsgParseOutput(CauseIE.MANDATORY_IE_INCORRECT,
            ...
            if  len(pdr_entry.set_gr_far.ListFields()) == 0 and \
                ...
                return (self.MsgParseOutput(CauseIE.INVALID_FORWARDING_POLICY,
                                        pdr_entry.pdr_id, pdr_entry.pdr_version))

            pdr_rules.update({pdr_entry.pdr_id: pdr_create_rule_entry(pdr_entry)})

        return self.MsgParseOutput(CauseIE.REQUEST_ACCEPTED, 0, 0)

    def validate_session_msg(self, new_session):
        """
    ...
    def process_session_message(self, new_session, process_pdr_rules):
        ...
        context_response.cause_info.cause_ie = \
                  self.validate_session_msg(new_session)
        if context_response.cause_info.cause_ie != CauseIE.REQUEST_ACCEPTED:
            return context_response

        #Create PDR rules
        pdr_parse = self._pdr_create_rule_group(new_session, process_pdr_rules)

        if pdr_parse.cause != CauseIE.REQUEST_ACCEPTED:
            context_response.cause_info.cause_ie = pdr_parse.cause
            context_response.failure_rule_id.extend([
                         FailureRuleInformation(pdr=OffendingIE(identifier=pdr_parse.identfier,
                                                                version=pdr_parse.session_version))])

        return context_response

    def _send_session_messsage_wrapper(self, msg_type, msg):

        # Periodic message towards SMF about session information in UPF
        if (msg_type == SessionMessageType.MSG_TYPE_CONFIG_STATE):
            logging.info (msg)

            ret = send_periodic_session_update(msg, self._sessiond_chan)
