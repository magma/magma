...
class MessageHub(object):
    ...
    def _flow_matches_flowmsg(self, dp, flow, msg):
        ...
        for j in range(0, len(flow.instructions)):
            ...
            if not isinstance(
                    flow.instructions[j], dp.ofproto_parser.OFPInstructionActions,
            ):
                continue
            if not isinstance(
                    msg.instructions[j], dp.ofproto_parser.OFPInstructionActions,
            ):
                continue
            ...
            reg_loads_flow = {
                ...
                if isinstance(i, dp.ofproto_parser.NXActionRegLoad2)
            }
            reg_loads_msg = {
            ...
            resubmits_flow = [
                ...
                if isinstance(i, dp.ofproto_parser.NXActionResubmitTable)
            ]
            resubmits_msg = [
