...
class CreateSessionUtil:
    ...
    def __init__(
        self, subscriber_id: str, local_f_teid: int,
        session_version, node_id="192.168.220.1",
    ):
        """
        ...
        self._set_session = \
            SessionSet(
                ...
                node_id=NodeID(
                    node_id_type=NodeID.IPv4, node_id=node_id,
                ),
    ...
    def CreateFARinPDR(o_teid: int = 0, gnb_ip_addr: str = '') -> FARRuleEntry:
        ...
        if o_teid != 0:
            ...
            return SetGroupFAR(
                far_action_to_apply=[Action.Value('FORW')],
                fwd_parm=FwdParam(
                    dest_iface=0,
    ...
    @staticmethod
    def CreatePDR(
        pdr_id: int, pdr_version: int, pdr_state,
    ...
    def CreateSessionMsg(
        self, imsi_val: str, pdr_entry: SetGroupPDR, far_entry: SetGroupFAR = None,
    ...
    def CreateSession(
        ...
        if del_rule_id:
            del_qer_tuple = QoSEnforceRuleEntry(
                imsi_val, del_rule_id, None,
                None, None, None, None,
            )
        ...
        if add_rule_id:
            uplink_qer_tuple = QoSEnforceRuleEntry(
        ...
        if pdr_state == "ADD":
            uplink_pdr = CreateSessionUtil.CreatePDR(
                pdr_id, 1, PdrState.Value('INSTALL'), 32, in_teid, ue_ip_addr,
            )
            pdr_id = pdr_id + 1
            ...
            uplink_pdr = CreateSessionUtil.CreatePDR(
                pdr_id, 1, PdrState.Value('IDLE'), 32, in_teid, ue_ip_addr,
            )
            pdr_id = pdr_id + 1
            downlink_pdr = CreateSessionUtil.CreatePDR(
                pdr_id, 1, PdrState.Value('IDLE'), 32, 0, ue_ip_addr,
            )
        else:
            uplink_pdr = CreateSessionUtil.CreatePDR(
                pdr_id, 1, PdrState.Value('REMOVE'), 32, in_teid, ue_ip_addr,
            )
            pdr_id = pdr_id + 1
            downlink_pdr = CreateSessionUtil.CreatePDR(
