...
class PipelinedRpcServicer(pipelined_pb2_grpc.PipelinedServicer):
    ...
    def _activate_rules_in_enforcement_stats(self, imsi: str,
                                             ...
                                             is_ng: bool=False) -> ActivateFlowsResult:
        if not self._service_manager.is_app_enabled(
    ...
    def SetSMFSessions(self, request, context):
        ...
        """
        logging.info("=====SMF-UPF-INTEGRATE: REQUEST=====")
        ...
        if response.cause_info.cause_ie == CauseIE.REQUEST_ACCEPTED:
            for _, pdr_entries in process_pdr_rules.items():
                ...
                if ret == True:
                    # Install the Rules
                    failed_static_rule_results, failed_dynamic_rule_results =\
                            self._ng_qer_update(request.session_version, pdr_entries,
    ...
    def _ng_tunnel_update(self, pdr_entry: PDRRuleEntry, subscriber_id: str) -> bool:
        ...
        elif pdr_entry.pdr_state == PdrState.Value('REMOVE'):
            ret = self._classifier_app.delete_tunnel_flows(\
                           pdr_entry.local_f_teid, pdr_entry.ue_ip_addr)

        return ret

    def _ng_qer_update(self, session_version:int, pdr_entry: PDRRuleEntry,
                       subscriber_id: str) -> Tuple[List[RuleModResult],
                                   List[RuleModResult]]:
        enforcement_res = []
        ...
        if pdr_entry.pdr_state == PdrState.Value('REMOVE') or\
           pdr_entry.del_qos_enforce_rule:
            self._ng_deactivate_qer_flows(session_version, pdr_entry, subscriber_id)

        if pdr_entry.pdr_state == PdrState.Value('INSTALL'):
            enforcement_res = \
                self._ng_activate_qer_flow(session_version, pdr_entry, subscriber_id)
    ...
    def _ng_activate_qer_flow(self, session_version, pdr_entry, subscriber_id):

        qos_enforce_rule = pdr_entry.add_qos_enforce_rule

        ipv4 = convert_ipv4_str_to_ip_proto(pdr_entry.ue_ip_addr)
        self._ng_update_version(subscriber_id, session_version)
        ...
        enforcement_stats_res = self._activate_rules_in_enforcement_stats(
                                         ...
                                         qos_enforce_rule.dynamic_rules, True)
    ...
    def _ng_deactivate_qer_flows(self, session_version, pdr_entry, subscriber_id):
