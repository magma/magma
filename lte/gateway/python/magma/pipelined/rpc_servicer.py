...
class PipelinedRpcServicer(pipelined_pb2_grpc.PipelinedServicer):
    ...
    def __init__(self, loop, gy_app, enforcer_app, enforcement_stats, dpi_app,
                 ...
                 tunnel_learn_app, classifier_app, ng_servicer_app,
    ...
    def _activate_rules_in_enforcement_stats(self, imsi: str,
                                             ...
                                             match_teid=0,
                                             action_teid=0) -> ActivateFlowsResult:
        if not self._service_manager.is_app_enabled(
    ...
    def ng_update_session_flows(self, request: SessionSet,
        ...
        for _, pdr_entries in process_pdr_rules.items():
            if pdr_entries.pdr_state == PdrState.Value('INSTALL'):
                self._classifier_app.add_tunnel_flows(\
                    pdr_entries.precedence, pdr_entries.local_f_teid,\
                    pdr_entries.far_action.o_teid, pdr_entries.ue_ip_addr,\
                    pdr_entries.far_action.enodeb_ip_addr, encode_imsi(request.subscriber_id))
        
                #Install enforcement apps
                self._ng_qer_update(request.session_version, pdr_entries, response)
        
            elif pdr_entries.pdr_state == PdrState.Value('REMOVE'):
                self._classifier_app.delete_tunnel_flows(\
                    pdr_entries.local_f_teid, pdr_entries.ue_ip_addr)
    ...
    def _ng_qer_update(self, session_version:int, pdr_entry,
                       response: UPFSessionContextState):

        enforcement_res = []

        if pdr_entry.pdr_state == PdrState.Value('REMOVE') or\
           pdr_entry.del_qos_enforce_rule:
            self._ng_deactivate_qer_flows(session_version, pdr_entry)
            response.del_qer_results.result = DeactivateFlowsResult.SUCCESS

        if pdr_entry.pdr_state == PdrState.Value('INSTALL'):
            enforcement_res = \
                self._ng_activate_qer_flow(session_version, pdr_entry)

            response.qos_enforce_rule_results.extend([enforcement_res])

    def _ng_activate_qer_flow(self, session_version, pdr_entry):

        qos_enforce_rule = pdr_entry.add_qos_enforce_rule

        ipv4 = convert_ipv4_str_to_ip_proto(qos_enforce_rule.ip_addr)
        self._ng_update_version(qos_enforce_rule.sid.id, session_version)
        if qos_enforce_rule.ipv6_addr:
            self._update_ipv6_prefix_store(qos_enforce_rule.ipv6_addr)

        # Install rules in enforcement stats
        enforcement_stats_res = self._activate_rules_in_enforcement_stats(
             qos_enforce_rule.sid.id, ipv4, qos_enforce_rule.apn_ambr, qos_enforce_rule.rule_ids,
             qos_enforce_rule.dynamic_rules, pdr_entry.local_f_teid, pdr_entry.far_action.o_teid)
        ...
        enforcement_res = \
               self._activate_rules_in_enforcement(
                    qos_enforce_rule.sid.id, ipv4, qos_enforce_rule.apn_ambr, static_rule_ids,
                    dynamic_rules, pdr_entry.local_f_teid, pdr_entry.far_action.o_teid)
    ...
    def _ng_deactivate_qer_flows(self, session_version, pdr_entry):

        if  pdr_entry.del_qos_enforce_rule:
            self._ng_deactivate_qer_flow_entry(session_version, pdr_entry, \
                                               pdr_entry.del_qos_enforce_rule)

        if pdr_entry.pdr_state == PdrState.Value('REMOVE') and \
           pdr_entry.add_qos_enforce_rule:
            self._ng_deactivate_qer_flow_entry(session_version, pdr_entry, \
                                               pdr_entry.add_qos_enforce_rule)

    def _ng_deactivate_qer_flow_entry(self, session_version, pdr_entry, qos_enforce_rule):
        ...
        self._enforcer_app.deactivate_rules(qos_enforce_rule.sid.id, ipv4,
                                            qos_enforce_rule.rule_ids,
                                            pdr_entry.local_f_teid,
                                            pdr_entry.far_action.o_teid)

    def _ng_update_version(self, subscriber_id: str, session_version: int):
        """
        Update 5G version for a given subscriber as QOS is installed.
        """
        self._service_manager.session_rule_version_mapper\
                 .ng_update_rules_version(subscriber_id, session_version)
