...
TEST_F(SgwS8Config, create_context_on_cs_req) {
  ...
  EXPECT_TRUE(
      strcmp(sgw_pdn_session->pdn_connection.apn_in_use, "internet") == 0);
  EXPECT_EQ(
  ...
  EXPECT_TRUE(eps_bearer_ctxt_p->s_gw_teid_S1u_S12_S4_up > 0);
  EXPECT_TRUE(eps_bearer_ctxt_p->s_gw_teid_S5_S8_up > 0);
  sgw_state_exit();
}
...
TEST_F(SgwS8Config, update_pdn_session_on_cs_rsp) {
  ...
  EXPECT_TRUE(
      strcmp(sgw_pdn_session->pdn_connection.apn_in_use, "NO APN") == 0);
  EXPECT_TRUE(
