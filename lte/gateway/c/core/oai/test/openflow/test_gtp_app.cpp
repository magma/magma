...
TEST_F(GTPApplicationTest, TestAddTunnel) {
  ...
  int vlan = 0;
  AddGTPTunnelEvent add_tunnel(ue_ip, NULL, vlan, enb_ip, in_tei, out_tei, imsi,
                               0);
  // Uplink
...
TEST_F(GTPApplicationTest, TestAddTunnelIpv6) {
  ...
  AddGTPTunnelEvent add_tunnel(ue_ip, &ue_ipv6, vlan, enb_ip, in_tei, out_tei,
                               imsi, 0);
  // Uplink
  EXPECT_CALL(*messenger,
              send_of_msg(AllOf(CheckTableId(0), CheckInPort(TEST_GTP_PORT),
...
TEST_F(GTPApplicationTest, TestAddTunnelS8) {
  ...
  AddGTPTunnelEvent add_tunnel(ue_ip, NULL, vlan, enb_ip, pgw_ip, in_tei,
                               out_tei, pgw_in_tei, pgw_out_tei, imsi, enb_port,
                               pgw_port);
  // Uplink
  EXPECT_CALL(*messenger,
