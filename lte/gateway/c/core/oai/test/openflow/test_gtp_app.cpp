...
#include "lte/gateway/c/core/oai/tasks/gtpv1-u/gtpv1u.h"  // for ip_flow_dl, ip_flow_dl::...
 ...
 protected:
  virtual void SetUp() {
    ...
    controller->register_for_event(gtp_app,
                                   openflow::EVENT_DELETE_GTP_S8_TUNNEL);
  }
...
TEST_F(GTPApplicationTest, TestAddTunnel) {
  ...
  int vlan = 0;
  AddGTPTunnelEvent add_tunnel(ue_ip, NULL, vlan, enb_ip, NULL, in_tei, out_tei,
                               imsi, 0);
  // Uplink
  EXPECT_CALL(*messenger,
              send_of_msg(AllOf(CheckTableId(0), CheckInPort(TEST_GTP_PORT),
  ...
  // downlink
  EXPECT_CALL(*messenger,
              send_of_msg(AllOf(CheckTableId(0), CheckInPort(of13::OFPP_LOCAL),
                                CheckEthType(0x0800), CheckIPv4Dst(ue_ip),
                                CheckCommandType(of13::OFPFC_ADD)),
                          _))
      .Times(1);
  EXPECT_CALL(*messenger,
              send_of_msg(AllOf(CheckTableId(0), CheckInPort(TEST_MTR_PORT),
                                CheckEthType(0x0800), CheckIPv4Dst(ue_ip),
                                CheckCommandType(of13::OFPFC_ADD)),
                          _))
      .Times(1);
...
TEST_F(GTPApplicationTest, TestAddTunnelDlFlow) {
  ...
  AddGTPTunnelEvent add_tunnel(ue_ip, NULL, vlan, enb_ip, NULL, in_tei, out_tei,
                               imsi, &dl_flow, dl_flow_precedence, 0);
  // Uplink
  EXPECT_CALL(*messenger,
              send_of_msg(AllOf(CheckTableId(0), CheckInPort(TEST_GTP_PORT),
                                CheckTunnelId(in_tei),
  ...
  EXPECT_CALL(
      *messenger,
      send_of_msg(
          AllOf(CheckTableId(0), CheckInPort(of13::OFPP_LOCAL),
  ...
  EXPECT_CALL(
      *messenger,
      send_of_msg(
          AllOf(CheckTableId(0), CheckInPort(TEST_MTR_PORT),
                CheckEthType(0x0800), CheckIPv4Dst(dl_flow.dst_ip),
                CheckIPv4Src(dl_flow.src_ip), CheckIPv4Proto(dl_flow.ip_proto),
                CheckTcpDstPort(dl_flow.tcp_dst_port),
                CheckTcpSrcPort(dl_flow.tcp_src_port),
...
TEST_F(GTPApplicationTest, TestAddTunnelDlFlowGtpPort) {
  ...
  AddGTPTunnelEvent add_tunnel(ue_ip, NULL, vlan, enb_ip, NULL, in_tei, out_tei,
                               imsi, &dl_flow, dl_flow_precedence, 10);
  // Uplink
  EXPECT_CALL(*messenger, send_of_msg(AllOf(CheckTableId(0), CheckInPort(10),
                                            CheckTunnelId(in_tei),
                                            CheckCommandType(of13::OFPFC_ADD)),
                                      _))
      .Times(1);
...
TEST_F(GTPApplicationTest, TestDeleteTunnelDlFlowGtpPort) {
  ...
  EXPECT_CALL(
      *messenger,
      send_of_msg(AllOf(CheckTableId(0), CheckInPort(30), CheckTunnelId(in_tei),
                        CheckCommandType(of13::OFPFC_DELETE)),
                  _))
      .Times(1);
  // downlink
...
TEST_F(GTPApplicationTest, TestAddTunnelIpv6) {
  ...
  AddGTPTunnelEvent add_tunnel(ue_ip, &ue_ipv6, vlan, enb_ip, NULL, in_tei,
                               out_tei, imsi, 0);
  // Uplink
  EXPECT_CALL(*messenger,
              send_of_msg(AllOf(CheckTableId(0), CheckInPort(TEST_GTP_PORT),
...
TEST_F(GTPApplicationTest, TestAddTunnelS8) {
  ...
  AddGTPTunnelEvent add_tunnel(ue_ip, NULL, vlan, enb_ip, NULL, pgw_ip, NULL,
                               in_tei, out_tei, pgw_in_tei, pgw_out_tei, imsi,
                               enb_port, pgw_port);
  // Uplink
  EXPECT_CALL(*messenger,
              send_of_msg(AllOf(CheckTableId(0), CheckInPort(enb_port),
                                CheckTunnelId(in_tei),
                                CheckCommandType(of13::OFPFC_ADD)),
                          _))
      .Times(1);
  // downlink
  EXPECT_CALL(*messenger,
              send_of_msg(AllOf(CheckTableId(0), CheckInPort(pgw_port),
                                CheckTunnelId(pgw_in_tei),
                                CheckCommandType(of13::OFPFC_ADD)),
                          _))
      .Times(1);
...
TEST_F(GTPApplicationTest, TestDeleteTunnelS8) {
  ...
  DeleteGTPTunnelEvent del_tunnel(ue_ip, NULL, in_tei, pgw_in_tei, enb_port,
                                  pgw_port);
  // Uplink
  EXPECT_CALL(*messenger,
              send_of_msg(AllOf(CheckTableId(0), CheckInPort(enb_port),
