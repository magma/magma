...
std::string convert_to_ipv4_host_format(IPAddress req_ipv4_addr) {
  ...
  inet_ntop(AF_INET, (req_ipv4_addr.address().c_str()), req_ipv4_addr_str,
            INET_ADDRSTRLEN);
...
std::string convert_to_ipv6_host_format(IPAddress req_ipv6_addr) {
  ...
  inet_ntop(AF_INET6, (req_ipv6_addr.address().c_str()), req_ipv6_addr_str,
            INET6_ADDRSTRLEN);
...
TEST(test_classifier_rpc, test_utility_update_request_ipv4) {
  ...
  request = create_update_request_ipv4(enb_ipv4_addr, ue_ipv4_addr, in_teid,
                                       out_teid, vlan, ue_state);

  // Validate GNB IP Address
  EXPECT_TRUE(convert_to_ipv4_host_format(request.enb_ip_address()) ==
              enb_ipv4_addr_str);

  // Validate UE IP Address
  EXPECT_TRUE(convert_to_ipv4_host_format(request.ue_ipv4_address()) ==
              ue_ipv4_addr_str);
  ...
  // UE Session State
  EXPECT_TRUE(request.ue_session_state().ue_config_state() ==
...
TEST(test_classifier_rpc, test_utility_proto_ip_flow_dl) {
  ...
  EXPECT_TRUE(convert_to_ipv4_host_format(req_flow_dl.dest_ip()) ==
              dst_ip_addr_str);

  // Validate flow_dl source IP Address
  EXPECT_TRUE(convert_to_ipv4_host_format(req_flow_dl.src_ip()) ==
              src_ip_addr_str);
...
TEST(test_classifier_add_rpc, test_tunnel_add_ipv4v6_flow_dl) {
  ...
  EXPECT_TRUE(convert_to_ipv6_host_format(request.ue_ipv6_address()) ==
              ue_ipv6_str);
...
TEST(test_classifier_rpc, test_tunnel_discard_data_ipv4_flow_dl) {
  ...
  EXPECT_TRUE(convert_to_ipv4_host_format(discard_request.ue_ipv4_address()) ==
              ue_v4);

  EXPECT_TRUE(in_teid == discard_request.in_teid());

  EXPECT_TRUE(discard_request.ue_session_state().ue_config_state() ==
...
TEST(test_classifier_rpc, test_tunnel_forward_data_ipv4v6_flow_dl) {
  ...
  EXPECT_TRUE(convert_to_ipv4_host_format(forward_request.ue_ipv4_address()) ==
              ue_v4);

  // Check the UE v6 Addresses
  EXPECT_TRUE(convert_to_ipv6_host_format(forward_request.ue_ipv6_address()) ==
              ue_ipv6);
  // Check the in-teid
  ...
  // Check the UE State
  EXPECT_TRUE(forward_request.ue_session_state().ue_config_state() ==
...
TEST(test_classifier_rpc, test_paging_uev4) {
  ...
  // Check the UE Addresses
  EXPECT_TRUE(convert_to_ipv4_host_format(paging_request.ue_ipv4_address()) ==
              ue_v4);
