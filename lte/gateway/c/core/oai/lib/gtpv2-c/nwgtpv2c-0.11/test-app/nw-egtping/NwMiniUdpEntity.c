...
static void NW_EVT_CALLBACK(nwUdpDataIndicationCallbackData) {
  ...
  if (bytesRead > 0) {
    ...
    NW_LOG(NW_LOG_LEVEL_DEBG,
           ...
           bytesRead, (peer.sin_addr.s_addr & 0x000000ff),
    ...
    switch (errno) {
      case ENETUNREACH:
        NW_LOG(NW_LOG_LEVEL_ERRO, "Network not reachable", errno,
               strerror(errno));
        break;
...
nw_rc_t nwGtpv2cUdpDataReq(nw_gtpv2c_UdpHandleT udpHandle, uint8_t* dataBuf,
  ...
  NW_LOG(NW_LOG_LEVEL_DEBG, "Sending %u bytes of data to %u.%u.%u.%u:%u",
         dataSize, (peerIp & 0xff000000) >> 24, (peerIp & 0x00ff0000) >> 16,
