...
static nw_rc_t nwGtpv2cCreateSessionRequestIeIndication(uint8_t ieType,
                                                        uint8_t ieLength,
                                                        uint8_t ieInstance,
                                                        uint8_t* ieValue,
                                                        void* arg) {
...
nw_rc_t nwGtpv2cUlpSenEchoRequestToPeer(NwGtpv2cNodeUlpT* thiz,
                                        uint32_t peerIp) {
  nw_rc_t rc;
  ...
  rc = nwGtpv2cMsgNew(thiz->hGtpv2cStack, NW_FALSE, NW_GTP_ECHO_REQ, 0, 0,
                      &(ulpReq.hMsg));
  NW_ASSERT(NW_OK == rc);
  rc = nwGtpv2cMsgAddIeTV1((ulpReq.hMsg), NW_GTPV2C_IE_RECOVERY, 0,
                           thiz->restartCounter);
  NW_ASSERT(NW_OK == rc);
...
nw_rc_t nwGtpv2cUlpCreateSessionRequestToPeer(NwGtpv2cNodeUlpT* thiz) {
  ...
  rc = nwGtpv2cMsgNew(thiz->hGtpv2cStack, NW_TRUE, NW_GTP_CREATE_SESSION_REQ, 0,
                      0, &(ulpReq.hMsg));
  NW_ASSERT(NW_OK == rc);
  rc = nwGtpv2cMsgAddIeTV1((ulpReq.hMsg), NW_GTPV2C_IE_RECOVERY, 0,
                           thiz->restartCounter);
...
nw_rc_t nwGtpv2cUlpProcessStackReqCallback(nw_gtpv2c_UlpHandleT hUlp,
  ...
  switch (pUlpApi->apiType) {
    case NW_GTPV2C_ULP_API_INITIAL_REQ_IND: {
      ...
      rc = nwGtpv2cMsgParserNew(thiz->hGtpv2cStack, NW_GTP_CREATE_SESSION_REQ,
      ...
      if (pUlpApi->apiInfo.initialReqIndInfo.msgType ==
        ...
        rc = nwGtpv2cMsgAddIe((ulpReq.hMsg), NW_GTPV2C_IE_CAUSE, 2, 0,
                              (uint8_t*)&cause);
        NW_ASSERT(NW_OK == rc);
        NW_LOG(NW_LOG_LEVEL_NOTI,
