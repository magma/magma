...
NwGtpv2cPeerT* nwGtpv2cUlpCreatePeerContext(NwGtpv2cNodeUlpT* thiz,
                                            uint32_t peerIp) {
  nw_rc_t rc;
...
nw_rc_t nwGtpv2cUlpSendEchoRequestToPeer(NwGtpv2cNodeUlpT* thiz,
                                         NwGtpv2cPeerT* pPeer) {
  nw_rc_t rc;
  struct timeval tv;
  nw_gtpv2c_ulp_api_t ulpReq;
...
nw_rc_t nwGtpv2cUlpPing(NwGtpv2cNodeUlpT* thiz, uint32_t peerIp,
                        uint32_t pingCount, uint32_t pingInterval,
                        uint32_t t3Time, uint32_t n3Count) {
  nw_rc_t rc;
  NwGtpv2cPeerT* pPeer;
  nw_gtpv2c_ulp_api_t ulpReq;
...
nw_rc_t nwGtpv2cUlpProcessStackReqCallback(nw_gtpv2c_UlpHandleT hUlp,
  ...
  switch (pUlpApi->apiType) {
    ...
    case NW_GTPV2C_ULP_API_RSP_FAILURE_IND: {
      ...
      NW_LOG(NW_LOG_LEVEL_DEBG, "No response from " NW_IPV4_ADDR " (2123)!",
