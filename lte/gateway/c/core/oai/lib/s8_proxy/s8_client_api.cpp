...
static void
convert_proto_msg_to_itti_csr(magma::feg::CreateSessionResponsePgw &response,
                              s8_create_session_response_t *s5_response,
                              bearer_qos_t dflt_bearer_qos);
...
static void
get_paa_from_proto_msg(const magma::feg::PDNType &proto_pdn_type,
...
static void recv_s8_create_session_response(
  ...
  OAILOG_DEBUG_UE(LOG_SGW_S8, imsi64,
                  "Sending create session response to sgw_s8 task");
  if ((send_msg_to_task(&grpc_service_task_zmq_ctx, TASK_SGW_S8, message_p)) !=
      RETURNok) {
    OAILOG_ERROR_UE(
        LOG_SGW_S8, imsi64,
...
static void
convert_paa_to_proto_msg(const itti_s11_create_session_request_t *msg,
...
static void
convert_bearer_context_to_proto(const bearer_context_to_be_created_t *msg_bc,
                                magma::feg::BearerContext *bc) {
  OAILOG_FUNC_IN(LOG_SGW_S8);
  bc->set_id(msg_bc->eps_bearer_id);
  char sgw_s8_up_ip[INET_ADDRSTRLEN];
...
static void
convert_pco_to_proto_msg(protocol_configuration_options_t pco,
...
static void
fill_s8_create_session_req(const itti_s11_create_session_request_t *msg,
                           magma::feg::CreateSessionRequestPgw *csr) {
  OAILOG_FUNC_IN(LOG_SGW_S8);
  csr->Clear();
