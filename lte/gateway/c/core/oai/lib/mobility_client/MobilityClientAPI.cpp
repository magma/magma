...
int pgw_handle_allocate_ipv4_address(const std::string subscriber_id,
  ...
  MobilityServiceClient::getInstance().AllocateIPv4AddressAsync(
      subscriber_id, apn_str,
      [subscriber_id, apn_str, pdn_type_str, context_teid, eps_bearer_id](
        ...
        handle_allocate_ipv4_address_status(
            status, addr, vlan, subscriber_id.c_str(), apn_str.c_str(),
...
static void handle_allocate_ipv4_address_status(
  ...
  if (!message_p) {
    OAILOG_ERROR(LOG_UTIL,
                 "Message IP Allocation Response allocation failed\n");
    return;
  }
  ...
  if (status.ok()) {
    increment_counter("ue_pdn_connection", 1, 2, "pdn_type", pdn_type, "result",
                      "success");
    ...
    OAILOG_DEBUG(LOG_UTIL,
                 "Allocated IPv4 address for imsi <%s>, apn <%s> vlan %d\n",
                 imsi, apn, vlan);
  } else {
    if (status.error_code() == RPC_STATUS_ALREADY_EXISTS) {
      ...
      increment_counter("ue_pdn_connection", 1, 2, "pdn_type", pdn_type,
                        "result", "failure");
      OAILOG_ERROR(LOG_UTIL,
      ...
      [subscriber_id, apn_str, pdn_type_str, context_teid, eps_bearer_id](
        ...
        if (ip_msg.ip_list_size() > 0) {
          ...
          OAILOG_ERROR(
              ...
              subscriber_id.c_str(), apn_str.c_str());
        }
        ...
        handle_allocate_ipv6_address_status(
            status, ip6_addr, vlan, subscriber_id.c_str(), apn_str.c_str(),
            pdn_type_str.c_str(), context_teid, eps_bearer_id);
      });
  return RETURNok;
}
...
static void handle_allocate_ipv6_address_status(
  ...
  if (status.ok()) {
    ...
    OAILOG_DEBUG(LOG_UTIL,
                 "Allocated IPv6 address for imsi <%s>, apn <%s> vlan %d\n",
                 imsi, apn, vlan);
  } else {
    if (status.error_code() == RPC_STATUS_ALREADY_EXISTS) {
      increment_counter("ue_pdn_connection", 1, 2, "pdn_type", pdn_type,
                        "result", "ip_address_already_allocated");
      ip_allocation_response_p->status = SGI_STATUS_ERROR_SYSTEM_FAILURE;
    } else {
      increment_counter("ue_pdn_connection", 1, 2, "pdn_type", pdn_type,
                        "result", "failure");
...
static void handle_allocate_ipv4v6_address_status(
  ...
  if (status.ok()) {
    ...
    OAILOG_DEBUG(LOG_UTIL,
                 "Allocated IPv4v6 address for imsi <%s>, apn <%s> vlan %d\n",
                 imsi, apn, vlan);
  } else {
    if (status.error_code() == RPC_STATUS_ALREADY_EXISTS) {
      increment_counter("ue_pdn_connection", 1, 2, "pdn_type", pdn_type,
