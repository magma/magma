...
int s6a_aia_cb(struct msg** msg, struct avp* paramavp, struct session* sess,
  ...
  OAILOG_DEBUG(LOG_S6A,
               "Received S6A Authentication Information Answer (AIA)\n");
  CHECK_FCT(fd_msg_search_avp(qry, s6a_fd_cnf.dataobj_s6a_user_name, &avp));

  if (avp) {
    CHECK_FCT(fd_msg_avp_hdr(avp, &hdr));
    snprintf(s6a_auth_info_ans_p->imsi, sizeof(s6a_auth_info_ans_p->imsi),
  ...
  if (avp) {
     ...
     */
    CHECK_FCT(fd_msg_search_avp(ans, s6a_fd_cnf.dataobj_s6a_experimental_result,
                                &avp));
  ...
  if (skip_auth_res == 0) {
    CHECK_FCT(fd_msg_search_avp(ans, s6a_fd_cnf.dataobj_s6a_authentication_info,
                                &avp));
...
int s6a_generate_authentication_info_req(s6a_auth_info_req_t* air_p) {
   ...
   */
  {
     ...
     */
    CHECK_FCT(fd_msg_avp_new(s6a_fd_cnf.dataobj_s6a_number_of_requested_vectors,
                             0, &child_avp));
     ...
     */
    CHECK_FCT(fd_msg_avp_new(s6a_fd_cnf.dataobj_s6a_immediate_response_pref, 0,
                             &child_avp));
    value.u32 = 0;
    CHECK_FCT(fd_msg_avp_setvalue(child_avp, &value));
    CHECK_FCT(fd_msg_avp_add(avp, MSG_BRW_LAST_CHILD, child_avp));
