...
int s6a_clr_cb(struct msg** msg_p, struct avp* paramavp_p,
   ...
   */
  CHECK_FCT(fd_msg_search_avp(qry_p, s6a_fd_cnf.dataobj_s6a_cancellation_type,
                              &avp_p));
  ...
  if (avp_p) {
    ...
    if (hdr_p->avp_value->u32 == SUBSCRIPTION_WITHDRAWL) {
      ...
      OAILOG_DEBUG(LOG_S6A,
                   "Sending S6A_CANCEL_LOCATION_REQ to task MME_APP\n");
      result_code = DIAMETER_SUCCESS;
    } else {
      OAILOG_ERROR(LOG_S6A,
                   "S6A_CANCEL_LOCATION cancellation type %d not supported \n",
                   hdr_p->avp_value->u32);
      result_code = ER_DIAMETER_INVALID_AVP_VALUE;
      goto out;
    }
...
int s6a_add_result_code(struct msg* ans, struct avp* failed_avp,
  ...
  if (DIAMETER_ERROR_IS_VENDOR(result_code) && experimental != 0) {
    ...
    CHECK_FCT(fd_msg_avp_new(s6a_fd_cnf.dataobj_s6a_experimental_result_code, 0,
                             &avp));
