...
void SpgwStateConverter::spgw_bearer_context_to_proto(
  ...
  BSTRING_TO_STRING(ip_addr_bstr,
                    sgw_eps_bearer_proto->mutable_mme_ip_address_s11());
  ...
  BSTRING_TO_STRING(ip_addr_bstr,
                    sgw_eps_bearer_proto->mutable_sgw_ip_address_s11_s4());
  bdestroy_wrapper(&ip_addr_bstr);
...
void SpgwStateConverter::proto_to_spgw_bearer_context(
  ...
  proto_to_ecgi(sgw_eps_bearer_context_proto.last_known_cell_id(),
  ...
  proto_to_sgw_pdn_connection(sgw_eps_bearer_context_proto.pdn_connection(),
...
void SpgwStateConverter::sgw_pdn_connection_to_proto(
  ...
  if (state_pdn->apn_in_use) {
    proto_pdn->set_apn_in_use(state_pdn->apn_in_use,
                              strlen(state_pdn->apn_in_use));
  }
  bstring ip_addr_bstr =
...
void SpgwStateConverter::proto_to_sgw_create_session_message(
  ...
  strncpy((char*)&session_request->imsi.digit, proto.imsi().c_str(),
          proto.imsi().length());
  session_request->imsi.length = proto.imsi().length();

  strncpy((char*)&session_request->msisdn.digit, proto.msisdn().c_str(),
          proto.msisdn().length());
  session_request->msisdn.length = proto.msisdn().length();

  if (MEI_IMEISV) {
    memcpy(&session_request->mei.choice.imeisv, &proto.mei(),
           proto.mei().length());
  } else if (MEI_IMEI) {
    memcpy(&session_request->mei.choice.imei, &proto.mei(),
           proto.mei().length());
  }
  ...
  memcpy(&session_request->charging_characteristics.value,
         proto.charging_characteristics().c_str(), length);
  session_request->charging_characteristics.value[length] = '\0';

  memcpy(&session_request->serving_network.mcc,
         proto.serving_network().mcc().c_str(), 3);
  memcpy(&session_request->serving_network.mnc,
  ...
  if (proto.sender_fteid_for_cp().ipv4_address()) {
    ...
    memcpy(&session_request->sender_fteid_for_cp.ipv6_address,
  ...
  for (int i = 0; i < proto.bearer_contexts_to_be_created_size(); i++) {
    ...
    proto_to_eps_bearer_qos(eps_bearer_proto.bearer_level_qos(),
                            &eps_bearer->bearer_level_qos);
  }
}
...
void SpgwStateConverter::sgw_eps_bearer_to_proto(
  ...
  BSTRING_TO_STRING(ip_addr_bstr,
                    eps_bearer_proto->mutable_pgw_address_in_use_up());
  bdestroy_wrapper(&ip_addr_bstr);
  ...
  BSTRING_TO_STRING(ip_addr_bstr,
                    eps_bearer_proto->mutable_sgw_ip_address_s5_s8_up());
  bdestroy_wrapper(&ip_addr_bstr);
  ...
  BSTRING_TO_STRING(ip_addr_bstr,
                    eps_bearer_proto->mutable_sgw_ip_address_s1u_s12_s4_up());
  bdestroy_wrapper(&ip_addr_bstr);
  ...
  BSTRING_TO_STRING(ip_addr_bstr,
                    eps_bearer_proto->mutable_enb_ip_address_s1u());
  bdestroy_wrapper(&ip_addr_bstr);
  ...
  eps_bearer_qos_to_proto(&eps_bearer->eps_bearer_qos,
                          eps_bearer_proto->mutable_eps_bearer_qos());
  traffic_flow_template_to_proto(&eps_bearer->tft,
                                 eps_bearer_proto->mutable_tft());
...
void SpgwStateConverter::proto_to_sgw_eps_bearer(
  ...
  proto_to_eps_bearer_qos(eps_bearer_proto.eps_bearer_qos(),
                          &eps_bearer->eps_bearer_qos);
  proto_to_traffic_flow_template(eps_bearer_proto.tft(), &eps_bearer->tft);
