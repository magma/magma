...
static void insert_sgw_c_teid_to_directoryd(sgw_state_t* state,
                                            imsi64_t imsi64);
...
sgw_create_bearer_context_information_in_collection(teid_t teid) {
  ...
  hash_table_ts_t* state_imsi_ht = get_sgw_ue_state();
  hashtable_ts_insert(state_imsi_ht, (const hash_key_t)teid,
...
void sgw_s8_handle_s11_create_session_request(
  ...
  if (sgw_update_teid_in_ue_context(sgw_state, imsi64,
                                    sgw_s11_tunnel.local_teid) == RETURNerror) {
    OAILOG_ERROR_UE(LOG_SGW_S8, imsi64,
  ...
  send_s8_create_session_request(sgw_s11_tunnel.local_teid, session_req_pP,
                                 imsi64);
...
static void insert_sgw_c_teid_to_directoryd(sgw_state_t* sgw_state,
                                            imsi64_t imsi64) {
  OAILOG_FUNC_IN(LOG_SGW_S8);
...
void sgw_s8_handle_modify_bearer_request(
  ...
  for (; idx <
    ...
    bearer_ctx_p = sgw_cm_get_eps_bearer_entry(&sgw_context_p->pdn_connection,
    ...
    if (!bearer_ctx_p) {
      ...
      if (does_bearer_context_hold_valid_enb_ip(
          ...
          is_enb_ip_address_same(&mbr_bearer_ctxt_p.s1_eNB_fteid,
      ...
      populate_sgi_end_point_update(sgi_rsp_idx, idx, modify_bearer_pP,
                                    bearer_ctx_p, &sgi_update_end_point_resp);
      sgi_rsp_idx++;
    }
  }  // for loop
...
static void sgw_send_modify_bearer_response(
  ...
  if (sgw_context_p) {
    ...
    sgw_s8_populate_mbr_bearer_contexts_modified(resp_pP, imsi64, sgw_context_p,
                                                 modify_response_p);
    sgw_populate_mbr_bearer_contexts_removed(resp_pP, imsi64, sgw_context_p,
                                             modify_response_p);
...
void sgw_s8_handle_release_access_bearers_request(
  ...
  if (sgw_context_p) {
    ...
    sgw_process_release_access_bearer_request(LOG_SGW_S8, imsi64,
                                              sgw_context_p);
  } else {
    sgw_send_release_access_bearer_response(LOG_SGW_S8, imsi64,
                                            CONTEXT_NOT_FOUND,
                                            release_access_bearers_req_pP, 0);
  }
  OAILOG_FUNC_OUT(LOG_SGW_S8);
}
...
imsi64_t sgw_s8_handle_create_bearer_request(
  ...
  sgw_eps_bearer_ctxt_t* bearer_ctxt_p = NULL;
  bearer_ctxt_p = sgw_cm_get_eps_bearer_entry(&sgw_context_p->pdn_connection,
                                              cb_req->linked_eps_bearer_id);
...
void sgw_s8_proc_s11_create_bearer_rsp(
  ...
  if (!pgw_ni_cbr_proc) {
    ...
    handle_failed_create_bearer_response(sgw_context_p,
                                         s11_actv_bearer_rsp->cause.cause_value,
                                         imsi64, bc_cbrsp, NULL, LOG_SGW_S8);
    OAILOG_FUNC_OUT(LOG_SGW_S8);
  }
