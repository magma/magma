...
static void *sgw_s8_thread(void *args) {
  ...
  AssertFatal(0, "Asserting as sgw_s8_thread should not be exiting on its own! "
...
static int handle_message(zloop_t *loop, zsock_t *reader, void *arg) {
  ...
  case S8_CREATE_SESSION_RSP: {
    sgw_s8_handle_create_session_response(
        sgw_state, &received_message_p->ittiMsg.s8_create_session_rsp, imsi64);
  } break;
  ...
  case S8_DELETE_SESSION_RSP: {
    sgw_s8_handle_delete_session_response(
        sgw_state, &received_message_p->ittiMsg.s8_delete_session_rsp, imsi64);
  } break;
