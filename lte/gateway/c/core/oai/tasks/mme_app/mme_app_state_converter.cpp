...
void MmeNasStateConverter::regional_subscription_to_proto(
  ...
  for (int itr = 0; itr < state_ue_context.num_reg_sub; itr++) {
    oai::Regional_subscription* reg_sub_proto =
      ...
      reg_sub_proto->set_zone_code((const char *)state_ue_context.reg_sub[itr].zone_code);
      OAILOG_DEBUG(
          LOG_MME_APP, "Writing regional_subscription at index %d with zonecode %s", itr,
...
void MmeNasStateConverter::proto_to_regional_subscription(
  ...
  for (int itr = 0; itr < ue_context_proto.num_reg_sub(); itr++) {
      memcpy(state_ue_context->reg_sub[itr].zone_code, ue_context_proto.reg_sub(itr).zone_code().c_str(), ue_context_proto.reg_sub(itr).zone_code().length());
      OAILOG_DEBUG(LOG_MME_APP, "Reading regional_subscription zone code %s at index %d", state_ue_context->reg_sub[itr].zone_code, itr);
  }
}
