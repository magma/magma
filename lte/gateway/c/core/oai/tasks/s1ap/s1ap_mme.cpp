...
static int handle_message(zloop_t *loop, zsock_t *reader, void *arg) {
  ...
  case SCTP_DATA_IND: {
    ...
    if (s1ap_mme_decode_pdu(&pdu, SCTP_DATA_IND(received_message_p).payload) <
      ...
      s1ap_mme_handle_message(state, SCTP_DATA_IND(received_message_p).assoc_id,
  ...
  case MME_APP_CONNECTION_ESTABLISHMENT_CNF: {
    is_task_state_same = false; // the following handler does not modify state
  ...
  case S1AP_PATH_SWITCH_REQUEST_FAILURE: {
    ...
    s1ap_handle_path_switch_req_failure(
        &received_message_p->ittiMsg.s1ap_path_switch_request_failure, imsi64);
  } break;
