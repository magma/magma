...
status_code_e s11_mme_create_session_request(
  ...
  rc = nwGtpv2cMsgNew(*stack_p, true, NW_GTP_CREATE_SESSION_REQ, req_p->teid, 0,
                      &(ulp_req.hMsg));
  ...
  if (num_dots) {
    ...
    gtpv2c_apn_plmn_ie_set(&(ulp_req.hMsg), req_p->apn,
                           &req_p->serving_network);
  }
  ...
  if (req_p->bearer_contexts_to_be_created.num_bearer_context > 1) {
    gtpv2c_ebi_ie_set(&(ulp_req.hMsg), (unsigned)req_p->default_ebi,
