...
int _nas5g_message_plain_encode(unsigned char* buffer,
  ...
  if (M5GS_MOBILITY_MANAGEMENT_MESSAGE ==
    ...
    OAILOG_DEBUG(LOG_AMF_APP, "[%s] Msg plain encode bytes[0-%d]\n%s",
                 get_message_type_str(msg->amf.header.message_type).c_str(),
...
static int _nas5g_message_protected_encode(
  ...
  if (plain_msg) {
     ...
     */
    int size = _nas5g_message_plain_encode(plain_msg, &msg->header, &msg->plain,
                                           length);
    if (size > 0) {
      /*
