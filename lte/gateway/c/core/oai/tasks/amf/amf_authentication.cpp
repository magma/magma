...
#if 0  // unused function, commented as part of warning cleanup
/****************************************************************************
 **                                                                        **
 ...
 **                    re-synchronization                                  **
 **      auts_pP : sent in case of re-synchronization                      **
 ** Outputs:                                                               **
 **     Return: None                                                       **
 **                                                                        **
 ***************************************************************************/
...
int amf_proc_authentication_complete(
  ...
  if (auth_proc) {
...
#if 0  // commented as part of warning cleanup, need to identify correct way to
       // print
    OAILOG_STREAM_HEX(
    ...
    OAILOG_STREAM_HEX(
        ...
        (const char*) (&(amf_ctx->_vector[auth_proc->ksi].xres_star[0])),
        AUTH_XRES_SIZE);
#endif
    rc = memcmp(
...
int amf_send_authentication_request(
  ...
  if (auth_proc) {
    ...
    if (rc != RETURNerror) {
      ...
      OAILOG_INFO(
          LOG_AMF_APP, "T3560: Authenthication timer id is %lu\n",
          auth_proc->T3560.id);
    }
    if (rc != RETURNerror) {
    }
