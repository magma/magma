...
//------------------------------------------------------------------------------
static int amf_cn_implicit_detach_ue(const amf_ue_ngap_id_t ue_id) {
  int rc                          = RETURNok;
  ...
  amf_proc_deregistration_request(ue_id, &params);
  OAILOG_FUNC_RETURN(LOG_NAS_AMF, rc);
}
...
int amf_cn_send(const amf_cn_t* msg) {
  ...
  switch (primitive) {
    ...
    case _AMFCN_IMPLICIT_DETACH_UE:
      rc = amf_cn_implicit_detach_ue(msg->u.amf_cn_implicit_detach.ue_id);
      break;
