...
int RequestType::DecodeRequestType(
  ...
  if (iei > 0) {
    ...
    MLOG(MDEBUG) << "In DecodeRequestType: iei" << hex << int(reqest_type->iei)
                 << endl;
    decoded++;
  }

  reqest_type->type_val = (*buffer & 0x07);
  MLOG(MDEBUG) << "DecodeRequestType: type_val = " << hex
               << int(reqest_type->type_val) << endl;
...
int RequestType::EncodeRequestType(
  ...
  if (iei > 0) {
    *buffer = (reqest_type->iei & 0x0f) << 4;
    ...
    MLOG(MDEBUG) << "In EncodeRequestType: iei" << hex << int(*buffer) << endl;
  }

  *buffer = 0x00 | (*buffer & 0xf0) | (reqest_type->type_val & 0x07);
  MLOG(MDEBUG) << "EncodeRequestType: type_val = " << hex << int(*(buffer))
               << endl;
  encoded++;
