...
//------------------------------------------------------------------------------
void emm_data_context_remove_mobile_ids(emm_data_t* emm_data,
...
//------------------------------------------------------------------------------
void emm_context_dump(const struct emm_context_s* const emm_context,
  ...
  for (k = 0; k < emm_context->_tai_list.numberoflists; k++) {
    switch (emm_context->_tai_list.partial_tai_list[k].typeoflist) {
      case TRACKING_AREA_IDENTITY_LIST_ONE_PLMN_NON_CONSECUTIVE_TACS: {
        tai_t tai = {0};
        COPY_PLMN(tai.plmn, emm_context->_tai_list.partial_tai_list[k]
  ...
  for (int vector_index = 0; vector_index < MAX_EPS_AUTH_VECTORS;
    ...
    for (k = 0; k < XRES_LENGTH_MAX; k++) {
      snprintf(&key_string[k * step], step, "%02x,",
  ...
  if (IS_EMM_CTXT_PRESENT_SECURITY(emm_context)) {
    ...
    if (SECURITY_CTX_TYPE_PARTIAL_NATIVE <= emm_context->_security.sc_type) {
      bformata(bstr_dump, "%*s         - dl_count.overflow: %05u",
               indent_spaces, " ", emm_context->_security.dl_count.overflow);
      bformata(bstr_dump, " dl_count.seq_num:  %03u\n",
               emm_context->_security.dl_count.seq_num);
      bformata(bstr_dump, "%*s         - ul_count.overflow: %05u",
               indent_spaces, " ", emm_context->_security.ul_count.overflow);
      ...
      if (true) {
        ...
        for (k = 0; k < AUTH_KNAS_ENC_SIZE; k++) {
          int ret = snprintf(&key_string[size], remaining_size, "%02x ",
  ...
  if (emm_context->esm_msg) {
    ...
    for (octet_index = 0; octet_index < blength(emm_context->esm_msg);
      ...
      if ((octet_index % 16) == 0) {
        ...
        bformata(bstr_dump, "%*s     |%04ld |", indent_spaces, " ",
                 octet_index);
      }
