...
static nas_emm_common_proc_t *
get_nas_common_procedure(const struct emm_context_s *const ctxt,
                         emm_common_proc_type_t proc_type);
static nas_cn_proc_t *
get_nas_cn_procedure(const struct emm_context_s *const ctxt,
...
static void
nas_delete_auth_info_procedure(struct emm_context_s *emm_context,
...
static nas_emm_proc_t *
nas_emm_find_procedure_by_puid(struct emm_context_s *const emm_context,
                               uint64_t puid);
...
inline bool is_nas_common_procedure_guti_realloc_running(
  ...
  if (get_nas_common_procedure_guti_realloc(ctxt))
    return true;
  return false;
}
...
bool is_nas_common_procedure_authentication_running(
    const struct emm_context_s *const ctxt) {
  if (get_nas_common_procedure_authentication(ctxt))
    return true;
  return false;
}
...
bool is_nas_common_procedure_smc_running(
    const struct emm_context_s *const ctxt) {
  if (get_nas_common_procedure_smc(ctxt))
    return true;
  return false;
}
...
bool is_nas_common_procedure_identification_running(
    const struct emm_context_s *const ctxt) {
  if (get_nas_common_procedure_identification(ctxt))
    return true;
  return false;
}
...
//------------------------------------------------------------------------------
nas_emm_guti_proc_t *
get_nas_common_procedure_guti_realloc(const struct emm_context_s *const ctxt) {
...
get_nas_cn_procedure_auth_info(const struct emm_context_s *const ctxt) {
  return (nas_auth_info_proc_t *)get_nas_cn_procedure(ctxt, CN_PROC_AUTH_INFO);
}

//------------------------------------------------------------------------------
nas_emm_smc_proc_t *
get_nas_common_procedure_smc(const struct emm_context_s *const ctxt) {
  return (nas_emm_smc_proc_t *)get_nas_common_procedure(ctxt,
                                                        EMM_COMM_PROC_SMC);
}
...
get_nas_specific_procedure_attach(const struct emm_context_s *const ctxt) {
  if ((ctxt) && (ctxt->emm_procedures) &&
      (ctxt->emm_procedures->emm_specific_proc) &&
      ((EMM_SPEC_PROC_TYPE_ATTACH ==
        ctxt->emm_procedures->emm_specific_proc->type)))
...
get_nas_specific_procedure_detach(const struct emm_context_s *const ctxt) {
  if ((ctxt) && (ctxt->emm_procedures) &&
      (ctxt->emm_procedures->emm_specific_proc) &&
      ((EMM_SPEC_PROC_TYPE_DETACH ==
        ctxt->emm_procedures->emm_specific_proc->type)))
...
//-----------------------------------------------------------------------------
nas_emm_tau_proc_t *
get_nas_specific_procedure_tau(const struct emm_context_s *const ctxt) {
  if ((ctxt) && (ctxt->emm_procedures) &&
      (ctxt->emm_procedures->emm_specific_proc) &&
      ((EMM_SPEC_PROC_TYPE_TAU ==
        ctxt->emm_procedures->emm_specific_proc->type)))
...
//-----------------------------------------------------------------------------
static emm_procedures_t *
nas_new_emm_procedures(struct emm_context_s *const emm_context) {
...
//-----------------------------------------------------------------------------
nas_emm_attach_proc_t *
nas_new_attach_procedure(struct emm_context_s *const emm_context) {
  if (!(emm_context->emm_procedures)) {
...
//-----------------------------------------------------------------------------
nas_emm_tau_proc_t *
nas_new_tau_procedure(struct emm_context_s *const emm_context) {
  if (!(emm_context->emm_procedures)) {
    emm_context->emm_procedures = nas_new_emm_procedures(emm_context);
  } else if (emm_context->emm_procedures->emm_specific_proc) {
    OAILOG_ERROR_UE(
  ...
  emm_context->emm_procedures->emm_specific_proc =
      reinterpret_cast<nas_emm_specific_proc_t *>(calloc(
          1,
          sizeof(
              nas_emm_tau_proc_t))); // NOLINT(clang-analyzer-unix.MallocSizeof)
  emm_context->emm_procedures->emm_specific_proc->emm_proc.base_proc.nas_puid =
      __sync_fetch_and_add(&nas_puid, 1);
  emm_context->emm_procedures->emm_specific_proc->emm_proc.base_proc.type =
      NAS_PROC_TYPE_EMM;
...
//-----------------------------------------------------------------------------
nas_sr_proc_t *
nas_new_service_request_procedure(struct emm_context_s *const emm_context) {
  if (!(emm_context->emm_procedures)) {
    emm_context->emm_procedures = nas_new_emm_procedures(emm_context);
  ...
  emm_context->emm_procedures->emm_con_mngt_proc =
      reinterpret_cast<nas_emm_con_mngt_proc_t *>(calloc(
          1,
          sizeof(nas_sr_proc_t))); // NOLINT(clang-analyzer-unix.MallocSizeof)
...
nas_new_identification_procedure(struct emm_context_s *const emm_context) {
  if (!(emm_context->emm_procedures)) {
    emm_context->emm_procedures = nas_new_emm_procedures(emm_context);
  }

  nas_emm_ident_proc_t *ident_proc = reinterpret_cast<nas_emm_ident_proc_t *>(
...
//-----------------------------------------------------------------------------
nas_emm_auth_proc_t *
nas_new_authentication_procedure(struct emm_context_s *const emm_context) {
  if (!(emm_context->emm_procedures)) {
    emm_context->emm_procedures = nas_new_emm_procedures(emm_context);
  }

  nas_emm_auth_proc_t *auth_proc = reinterpret_cast<nas_emm_auth_proc_t *>(
      calloc(1, sizeof(nas_emm_auth_proc_t)));
...
//-----------------------------------------------------------------------------
nas_emm_smc_proc_t *
nas_new_smc_procedure(struct emm_context_s *const emm_context) {
  if (!(emm_context->emm_procedures)) {
    emm_context->emm_procedures = nas_new_emm_procedures(emm_context);
  }
...
//-----------------------------------------------------------------------------
nas_auth_info_proc_t *
nas_new_cn_auth_info_procedure(struct emm_context_s *const emm_context) {
  if (!(emm_context->emm_procedures)) {
    emm_context->emm_procedures = nas_new_emm_procedures(emm_context);
  }
