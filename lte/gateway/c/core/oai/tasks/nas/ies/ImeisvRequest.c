...
#include "lte/gateway/c/core/oai/tasks/nas/ies/ImeisvRequest.h"
...
int encode_imeisv_request(ImeisvRequest* imeisvrequest, uint8_t iei,
   ...
   */
  CHECK_PDU_POINTER_AND_LENGTH_ENCODER(buffer, IMEISV_REQUEST_MINIMUM_LENGTH,
                                       len);
#if NAS_DEBUG
  dump_imeisv_request_xml(imeisvrequest, iei);
#endif
...
void dump_imeisv_request_xml(ImeisvRequest* imeisvrequest, uint8_t iei) {
  ...
  OAILOG_DEBUG(LOG_NAS, "    <IMEISV request value>%u</IMEISV request value>\n",
               *imeisvrequest);
  OAILOG_DEBUG(LOG_NAS, "</Imeisv Request>\n");
}
