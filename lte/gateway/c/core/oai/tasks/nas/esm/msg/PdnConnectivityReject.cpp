...
int decode_pdn_connectivity_reject(
   ...
   */
  if ((decoded_result = decode_esm_cause(&pdn_connectivity_reject->esmcause, 0,
    ...
    switch (ieiDecoded) {
      case PDN_CONNECTIVITY_REJECT_PROTOCOL_CONFIGURATION_OPTIONS_IEI:
        if ((decoded_result = decode_protocol_configuration_options_ie(
                 &pdn_connectivity_reject->protocolconfigurationoptions, true,
                 buffer + decoded, len - decoded)) <= 0)
          return decoded_result;
...
int encode_pdn_connectivity_reject(
  ...
  if ((encode_result = encode_esm_cause(
           &pdn_connectivity_reject->esmcause, 0, buffer + encoded,
           len - encoded)) < 0)  // Return in case of error
    return encode_result;
  else
    encoded += encode_result;

  if ((pdn_connectivity_reject->presencemask &
       PDN_CONNECTIVITY_REJECT_PROTOCOL_CONFIGURATION_OPTIONS_PRESENT) ==
      PDN_CONNECTIVITY_REJECT_PROTOCOL_CONFIGURATION_OPTIONS_PRESENT) {
