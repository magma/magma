...
int decode_bearer_resource_modification_request(
  ...
  CHECK_PDU_POINTER_AND_LENGTH_DECODER(
      buffer, BEARER_RESOURCE_MODIFICATION_REQUEST_MINIMUM_LENGTH, len);
  ...
  if ((decoded_result = decode_u8_linked_eps_bearer_identity(
           ...
           0, *(buffer + decoded) >> 4, len - decoded)) < 0)
    return decoded_result;

  decoded++;

  if ((decoded_result = decode_traffic_flow_template_ie(
           &bearer_resource_modification_request->trafficflowaggregate, 0,
...
int encode_bearer_resource_modification_request(
  ...
  *(buffer + encoded) = ((encode_u8_linked_eps_bearer_identity(
  ...
  if ((bearer_resource_modification_request->presencemask &
       BEARER_RESOURCE_MODIFICATION_REQUEST_REQUIRED_TRAFFIC_FLOW_QOS_PRESENT) ==
      BEARER_RESOURCE_MODIFICATION_REQUEST_REQUIRED_TRAFFIC_FLOW_QOS_PRESENT) {
