...
void StateConverter::guti_to_proto(const guti_t& state_guti,
                                   oai::Guti* guti_proto) {
  guti_proto->Clear();
...
void StateConverter::proto_to_guti(const oai::Guti& guti_proto,
                                   guti_t* state_guti) {
...
void StateConverter::ecgi_to_proto(const ecgi_t& state_ecgi,
                                   oai::Ecgi* ecgi_proto) {
  ecgi_proto->Clear();
...
void StateConverter::ambr_to_proto(const ambr_t& state_ambr,
                                   oai::Ambr* ambr_proto) {
...
void StateConverter::proto_to_ambr(const oai::Ambr& ambr_proto,
                                   ambr_t* state_ambr) {
...
void StateConverter::proto_to_apn_configuration(
  ...
  for (int i = 0; i < state_apn_configuration->nb_ip_address; i++) {
    ...
    bstring_to_ip_address(state_ip_addr_str,
                          &state_apn_configuration->ip_address[i]);
  }
  state_apn_configuration->pdn_type = apn_config_proto.pdn_type();
  strcpy(state_apn_configuration->service_selection,
         apn_config_proto.service_selection().c_str());
  state_apn_configuration->service_selection_length =
      apn_config_proto.service_selection().length();
  proto_to_eps_subscribed_qos_profile(apn_config_proto.subscribed_qos(),
                                      &state_apn_configuration->subscribed_qos);
...
void StateConverter::apn_config_profile_to_proto(
  ...
  for (int i = 0; i < state_apn_config_profile.nb_apns; i++) {
    apn_configuration_to_proto(state_apn_config_profile.apn_configuration[i],
...
void StateConverter::proto_to_apn_config_profile(
  ...
  for (int i = 0; i < state_apn_config_profile->nb_apns; i++) {
    proto_to_apn_configuration(apn_config_profile_proto.apn_configs(i),
...
void StateConverter::hashtable_uint64_ts_to_proto(
  ...
  for (auto i = 0; i < keys->num_keys; i++) {
    ...
    if (ht_rc == HASH_TABLE_OK) {
      ...
      OAILOG_ERROR(LOG_UTIL, "Key %lu not in %s", keys->keys[i],
                   htbl->name->data);
    }
  }
