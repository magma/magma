...
typedef struct s8_create_session_response_s {
  ...
  uint32_t temporary_session_id;  // temporary_session_id is created per pdn
                                  // session while sending create session
                                  // request and is temporarily used to fetch
                                  // session, once sgw_s8_teid is allocated by
                                  // orc8r, temporary session id is removed and
                                  // shall update imsi_ue_context_htbl based on
