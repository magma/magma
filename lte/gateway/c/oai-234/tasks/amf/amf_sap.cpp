...
int amf_sap_c::amf_sap_send(amf_sap_t* msg) {
  ...
  // else if ((primitive > (amf_primitive_t) AMFSMF_PRIMITIVE_MIN) && (primitive
  // < (amf_primitive_t) AMFSMF_PRIMITIVE_MAX))
  //{
  /*
   * Forward to the AMFESM-SAP
   */
  // msg->u.amf_smf.primitive = primitive;
  // rc = amf_smf_send_accept(&msg->u.amf_smf);
  //}
  else if (
...
void amf_as_data_t::amf_as_set_security_data(
  ...
  if (context && ((context->sc_type == SECURITY_CTX_TYPE_FULL_NATIVE) ||
    ...
    // OAILOG_WARNING(LOG_NAS_AMF, "NO Valid Security Context Available\n");
    /*
     * No valid 5GCN security context exists
     */
