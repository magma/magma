...
int PDUAddressMsg::EncodePDUAddressMsg(
  ...
  if (pdu_address->type_val == TYPE_VAL_IPV4) {
    ...
    *(buffer + encoded) = 0x00 | (pdu_address->type_val & 0x07);
    ...
    encoded++;
    memcpy(buffer + encoded, pdu_address->address_info, 4);
    MLOG(MDEBUG) << "NAS_TEST " << pdu_address->address_info << "encoded"
                 << encoded;
    encoded = encoded + 4;
    MLOG(MDEBUG) << "NAS_TEST "
                 << "encoded " << encoded;
  }
  return (encoded);
};
