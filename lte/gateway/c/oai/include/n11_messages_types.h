...
typedef struct itti_n11_pdu_session_modification_command_s {
  // common context
  char imsi[IMSI_BCD_DIGITS_MAX + 1];
  SMSessionFSMState_response sm_session_fsm_state;
  uint32_t sm_session_version;
  ...
  uint8_t procedure_trans_identity[2];
  // take data from TS 129 571
} itti_n11_pdu_session_modification_command_t;

typedef struct itti_n11_pdu_session_modification_reject_s {
  // common context
  char imsi[IMSI_BCD_DIGITS_MAX + 1];
  SMSessionFSMState_response sm_session_fsm_state;
  uint32_t sm_session_version;
  ...
  uint8_t procedure_trans_identity[2];
  // take data from TS 129 571
} itti_n11_pdu_session_modification_reject_t;

#define N11_PDU_SESSION_MODIFICATION_COMMAND(mSGpTR)     \
  (mSGpTR)->ittiMsg.n11_pdu_session_modification_command

#define N11_PDU_SESSION_MODIFICATION_REJECT(mSGpTR)      \
  (mSGpTR)->ittiMsg.n11_pdu_session_modification_reject
