...
int AmfMsg::M5gNasMessageDecodeMsg(AmfMsg* msg, uint8_t* buffer, uint32_t len) {
  ...
  MLOG(MDEBUG) << "   epd = 0x" << hex
               << int(msg->header.extendedprotocoldiscriminator) << "\n"
               << "   security hdr =  0x" << hex
               << int(msg->header.securityheadertype) << "\n"
...
int AmfMsg::AmfMsgDecodeHeaderMsg(
  ...
  if (len > 0 || buffer != NULL) {
    ...
    MLOG(MDEBUG) << "epd = 0x" << hex << int(hdr->extendedprotocoldiscriminator)
                 << "security hdr = 0x" << hex << int(hdr->securityheadertype)
                 << " hdr type = 0x" << hex << int(hdr->messagetype);
  } else {
    MLOG(MERROR) << "Error : Buffer is Empty" << endl;
    return (RETURN_ERROR);
  }
...
int AmfMsg::AmfMsgEncodeHeaderMsg(
  ...
  if ((unsigned char) hdr->extendedprotocoldiscriminator !=
...
int AmfMsg::AmfMsgDecodeMsg(AmfMsg* msg, uint8_t* buffer, uint32_t len) {
  ...
  switch ((unsigned char) msg->header.messagetype) {
    ...
    case REGISTRATION_REQUEST:
      ...
      decoderesult = msg->registrationrequestmsg.DecodeRegistrationRequestMsg(
          &msg->registrationrequestmsg, buffer, len);
      break;
    case REGISTRATION_ACCEPT:
      MLOG(MDEBUG) << "AmfMsgDecodeMsg: Registraion accept msg" << endl;
      decoderesult = msg->registrationacceptmsg.DecodeRegistrationAcceptMsg(
          &msg->registrationacceptmsg, buffer, len);
      break;
    case REGISTRATION_COMPLETE:
      decoderesult = msg->RegistrationCompleteMsg.DecodeRegistrationComplete(
          &msg->registrationcompletemsg, buffer, len);
      break;
    case REGISTRATION_REJECT:
      decoderesult = registration_complete_msg.decode_registration_reject(
          &msg->registrationrejectmsg, buffer, len);
      break;
    case IDENTITY_REQUEST:
      decoderesult = identity_request_msg.decode_identity_request(
          &msg->identityrequestmsg, buffer, len);
      break;
    case IDENTITY_RESPONSE:
      decoderesult = identity_response_msg.decode_identity_response(
          &msg->identityresponsemsg, buffer, len);
      break;
    case AUTHENTICATION_REQUEST:
      decoderesult = authentication_request_msg.decode_authentication_request(
          &msg->authenticationrequestmsg, buffer, len);
      break;
    case AUTHENTICATION_RESPONSE:
      decoderesult = authentication_response_msg.decode_authentication_response(
          &msg->authenticationresponsemsg, buffer, len);
      break;
    case AUTHENTICATION_REJECT:
      decoderesult = authentication_reject_msg.decode_authentication_reject(
          &msg->authenticationrejectmsg, buffer, len);
      break;
    case AUTHENTICATION_FAILURE:
      decoderesult = authentication_failure_msg.decode_authentication_failure(
          &msg->authenticationfailuremsg, buffer, len);
      break;
    case SECURITY_MODE_COMMAND:
      decoderesult = security_mode_command_msg.decode_security_mode_command(
          &msg->securitymodecommandmsg, buffer, len);
      break;
    case SECURITY_MODE_COMPLETE:
      decoderesult = security_mode_complete_msg.decode_security_mode_complete(
...
#ifdef HANDLE_POST_MVC
    case REGISTRATION_REQUEST:
      encoderesult = msg->registrationrequestmsg.EncodeRegistrationRequestMsg(
          &msg->registrationrequestmsg, buffer, len);
      break;
    case REGISTRATION_ACCEPT:
      MLOG(MDEBUG) << " registraion accept msg \n";
      encoderesult = msg->registrationacceptmsg.EncodeRegistrationAcceptMsg(
          &msg->registrationacceptmsg, buffer, len);
      break;
    case REGISTRATION_COMPLETE:
      encoderesult = msg->RegistrationCompleteMsg.EncodeRegistrationComplete(
          &msg->registrationcompletemsg, buffer, len);
      break;
    case REGISTRATION_REJECT:
      encoderesult = registration_complete_msg.encode_registration_reject(
          &msg->registrationrejectmsg, buffer, len);
      break;
    case IDENTITY_REQUEST:
      encoderesult = identity_request_msg.encode_identity_request(
          &msg->identityrequestmsg, buffer, len);
      break;
    case IDENTITY_RESPONSE:
      encoderesult = identity_response_msg.encode_identity_response(
          &msg->identityresponsemsg, buffer, len);
      break;
    case AUTHENTICATION_REQUEST:
      encoderesult = authentication_request_msg.encode_authentication_request(
          &msg->authenticationrequestmsg, buffer, len);
      break;
    case AUTHENTICATION_RESPONSE:
      encoderesult = authentication_response_msg.encode_authentication_response(
          &msg->authenticationresponsemsg, buffer, len);
      break;
    case AUTHENTICATION_REJECT:
      encoderesult = authentication_reject_msg.encode_authentication_reject(
          &msg->authenticationrejectmsg, buffer, len);
      break;
    case AUTHENTICATION_FAILURE:
      encoderesult = authentication_failure_msg.encode_authentication_failure(
          &msg->authenticationfailuremsg, buffer, len);
      break;
    case SECURITY_MODE_COMMAND:
      encoderesult = security_mode_command_msg.encode_security_mode_command(
          &msg->securitymodecommandmsg, buffer, len);
      break;
    case SECURITY_MODE_COMPLETE:
      encoderesult = security_mode_complete_msg.encode_security_mode_complete(
          &msg->securitymodecompletemsg, buffer, len);
      break;
    default:
