...
namespace magma5g
{
	...
	int ABBAMsg::DecodeABBAMsg(ABBAMsg *abba, uint8_t iei, uint8_t *buffer, uint32_t len) 
	{
		uint8_t decoded = 0;
	...
	int ABBAMsg::EncodeABBAMsg(ABBAMsg *abba, uint8_t iei, uint8_t * buffer, uint32_t len)
	{
		uint8_t *lenPtr;
		uint32_t encoded = 0;
