...
namespace magma5g
{
  ...
  int M5GSRegistrationResultMsg::DecodeM5GSRegistrationResultMsg(M5GSRegistrationResultMsg *m5gsregistrationresult, uint8_t iei, uint8_t *buffer, uint32_t len) 
  {
    return 0;
  };


  int M5GSRegistrationResultMsg::EncodeM5GSRegistrationResultMsg(M5GSRegistrationResultMsg *m5gsregistrationresult, uint8_t iei, uint8_t * buffer, uint32_t len)
  {
    uint8_t* lenPtr;
    uint32_t encoded = 0;
    ...
    if (iei > 0) {
      ...
      *buffer = iei;
      MLOG(MDEBUG)<< "In EncodeM5GSRegistrationResultMsg___: iei  = " << hex << int(*buffer) << endl;
      encoded++;
    }
    ...
    *(buffer + encoded) =
      ((m5gsregistrationresult->spare & 0xf) << 0x4) |  
