...
int SessionAMBRMsg::EncodeSessionAMBRMsg(
  ...
  *(buffer + encoded) = session_ambr->dl_unit;
  // MLOG(MDEBUG) << "buff 1 at " << buffer + encoded;
  // MLOG(MDEBUG) << "NAS_TEST session_ambr " << session_ambr->dl_unit;
  //  *(buffer + encoded) = 0x4;
  // uint32_t tmp = encoded;
  encoded++;
  ...
  *lenPtr = encoded - 1 - ((iei > 0) ? 1 : 0);
  //  MLOG(MDEBUG) << "lenptr 2 at " << lenPtr;
  //*(buffer + tmp) = session_ambr->dl_unit;
