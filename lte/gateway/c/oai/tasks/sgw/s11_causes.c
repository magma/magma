...
static const SGWCauseMapping_t causes[] = {
  ...
  {ERROR_IND_FROM_RNC_ENB_SGSN,
   "Error ind received from RNC/eNB/SGSN",
   0,
   0,
   0,
   0},
  ...
  {NEW_PDN_TYPE_SAB_ONLY,
   "New PDN type single address bearer only",
   1,
   0,
   0,
   0},
  ...
  {TEMP_REJECT_HO_IN_PROGRESS,
   "Temporarily rejected due to HO in progress",
   0,
   0,
   0,
   0},
  {M_PDN_APN_NOT_ALLOWED,
   "Multiple PDN for a given APN not allowed",
   1,
   0,
   0,
   0},
...
char *sgw_cause_2_string(uint8_t cause_value)
{
  ...
  res = bsearch(
    &key, causes, sizeof(causes), sizeof(SGWCauseMapping_t), compare_cause_id);
