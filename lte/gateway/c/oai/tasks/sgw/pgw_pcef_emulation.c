...
int pgw_pcef_emulation_init(const pgw_config_t *const pgw_config_p)
{
  ...
  //--------------------------
  pgw_app.deactivated_predefined_pcc_rules =
    hashtable_ts_create(32, NULL, free_pcc_rule, NULL);
  ...
  hrc = hashtable_ts_insert(
    pgw_app.deactivated_predefined_pcc_rules, pcc_rule->sdf_id, pcc_rule);
  ...
  pcc_rule->sdf_template.sdf_filter[0]
    .packetfiltercontents.ipv4remoteaddr[1]
    .addr = (uint8_t)((pgw_config_p->ue_pool_addr[0].s_addr >> 8) & 0x000000FF);
  pcc_rule->sdf_template.sdf_filter[0]
    .packetfiltercontents.ipv4remoteaddr[2]
    .addr =
  ...
  for (int i = 0; i < (SDF_ID_MAX - 1); i++) {
    if (pgw_config_p->pcef.preload_static_sdf_identifiers[i]) {
      pgw_pcef_emulation_apply_rule(
        pgw_config_p->pcef.preload_static_sdf_identifiers[i], pgw_config_p);
    } else
      break;
  }
  ...
  const pgw_config_t *const pgw_config_p)
{
  ...
  hashtable_rc_t hrc = hashtable_ts_get(
    pgw_app.deactivated_predefined_pcc_rules, sdf_id, (void **) &pcc_rule);
  ...
  if (HASH_TABLE_OK == hrc) {
    if (!pcc_rule->is_activated) {
      ...
      for (int sdff_i = 0;
           sdff_i < pcc_rule->sdf_template.number_of_packet_filters;
           sdff_i++) {
        pgw_pcef_emulation_apply_sdf_filter(
          &pcc_rule->sdf_template.sdf_filter[sdff_i],
          pcc_rule->sdf_id,
          pgw_config_p);
      }
    }
...
void pgw_pcef_emulation_apply_sdf_filter(
  sdf_filter_t *const sdf_f,
  const sdf_id_t sdf_id,
  const pgw_config_t *const pgw_config_p)
{
  if (
    (TRAFFIC_FLOW_TEMPLATE_BIDIRECTIONAL == sdf_f->direction) ||
  ...
  uint8_t direction)
{
  ...
  if (
    (TRAFFIC_FLOW_TEMPLATE_DOWNLINK_ONLY == direction) ||
  ...
  if (
    TRAFFIC_FLOW_TEMPLATE_IPV6_REMOTE_ADDR_FLAG & packetfiltercontents->flags) {
  ...
  if (
    TRAFFIC_FLOW_TEMPLATE_LOCAL_PORT_RANGE_FLAG & packetfiltercontents->flags) {
  ...
  uint8_t *const num_pf)
{
  ...
  if (HASH_TABLE_OK == hrc) {
    if (pcc_rule->is_activated) {
      ...
      memcpy(
        packet_filter,
        &pcc_rule->sdf_template.sdf_filter,
