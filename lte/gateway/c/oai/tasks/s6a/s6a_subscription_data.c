...
{
  DevCheck(
    hdr_sub_status->avp_value->u32 < SS_MAX,
    hdr_sub_status->avp_value->u32,
    SS_MAX,
    0);
...
static inline int
s6a_parse_msisdn(struct avp_hdr *hdr_msisdn, char *msisdn, uint8_t *length)
{
  int i;

  DevCheck(
    hdr_msisdn->avp_value->os.len <= MSISDN_LENGTH,
    hdr_msisdn->avp_value->os.len,
    MSISDN_LENGTH,
    0);
  ...
  access_restriction_t *access_restriction)
{
  DevCheck(
    ...
    hdr_access_restriction->avp_value->u32,
    ARD_MAX,
    0);
...
static inline int s6a_parse_pdn_type(struct avp_hdr *hdr, pdn_type_t *pdn_type)
{
  ...
  int *length)
{
  DevCheck(
    ...
    hdr_service_selection->avp_value->os.len,
...
{
  DevCheck(
    hdr->avp_value->u32 < PRE_EMPTION_CAPABILITY_MAX,
    hdr->avp_value->u32,
...
{
  DevCheck(
    hdr->avp_value->u32 < PRE_EMPTION_VULNERABILITY_MAX,
    hdr->avp_value->u32,
  ...
  eps_subscribed_qos_profile_t *ptr)
{
  ...
  while (avp) {
    ...
    switch (hdr->avp_code) {
      case AVP_CODE_QCI: CHECK_FCT(s6a_parse_qci(hdr, &ptr->qci)); break;
    ...
    switch (hdr->avp_code) {
      ...
      case AVP_CODE_APN_CONFIGURATION: {
        ...
        CHECK_FCT(s6a_parse_apn_configuration(
          avp,
          &apn_config_profile->apn_configuration[apn_config_profile->nb_apns]));
        apn_config_profile->nb_apns++;
      } break;
    }
