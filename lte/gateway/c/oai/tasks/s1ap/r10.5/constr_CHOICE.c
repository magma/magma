      ...
      do {
        ...
        switch (rval.code) {
          ...
          case RC_WMORE: /* More data expected */
            ...
            RETURN(RC_FAIL);
          case RC_FAIL: /* Fatal error */ RETURN(rval.code);
        } /* switch(rval) */
  ...
  void *app_key)
{
  ...
  if (present > 0 && present <= td->elements_count) {
    ...
    if (elm->memb_constraints) {
      return elm->memb_constraints(elm->type, memb_ptr, ctfailcb, app_key);
    } else {
      int ret =
  ...
  size_t size)
{
  ...
  for (edx = ctx->step; ctx->phase <= 4;) {
    ...
    if (ctx->phase == 2) {
      ...
      assert(_fetch_present_idx(st, specs->pres_offset, specs->pres_size) == 0);
    ...
    if (ctx->phase == 4) {
      ...
      switch (xer_skip_unknown(tcv, &ctx->left)) {
        ...
        case 2: ctx->phase = 3; break;
      }
    }

    switch (tcv) {
      case XCT_BOTH: break; /* No CHOICE? */
      case XCT_CLOSING:
      ...
      case XCT_UNKNOWN_BO:
        ...
        for (edx = 0; edx < td->elements_count; edx++) {
          ...
          switch (tcv) {
            ...
            default: edx = td->elements_count; break; /* Phase out */
          }
          break;
        }
  ...
  asn_per_outp_t *po)
{
  ...
  if (ct && ct->range_bits >= 0) {
    if (per_put_few_bits(po, present_enc, ct->range_bits)) _ASN_ENCODE_FAILED;

    return elm->type->uper_encoder(
      elm->type, elm->per_constraints, memb_ptr, po);
  } else {
    asn_enc_rval_t rval;
    if (specs->ext_start == -1) _ASN_ENCODE_FAILED;
    if (uper_put_nsnnwn(po, present_enc - specs->ext_start)) _ASN_ENCODE_FAILED;
    if (uper_open_type_put(elm->type, elm->per_constraints, memb_ptr, po))
      _ASN_ENCODE_FAILED;
    rval.encoded = 0;
  ...
  asn_per_outp_t *po)
{
  ...
  if (ct && ct->range_bits >= 0) {
    if (per_put_few_bits(po, present, ct->range_bits)) _ASN_ENCODE_FAILED;

    return elm->type->aper_encoder(
      elm->type, elm->per_constraints, memb_ptr, po);
  } else {
    asn_enc_rval_t rval;
    if (specs->ext_start == -1) _ASN_ENCODE_FAILED;
  ...
  void *app_key)
{
  ...
  if (present > 0 && present <= td->elements_count) {
    ...
    if (0) {
      if (
        cb(elm->name, strlen(elm->name), app_key) < 0 ||
        cb(": ", 2, app_key) < 0)
        return -1;
    }

    return elm->type->print_struct(elm->type, memb_ptr, ilevel, cb, app_key);
  } else {
...
asn_comp_rval_t *CHOICE_compare(
