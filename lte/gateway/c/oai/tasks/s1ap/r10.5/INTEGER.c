 ...
 */
asn_enc_rval_t INTEGER_encode_der(
  ...
  int plainOrXER)
{
  ...
  if (ret == 0) {
    ...
    if (el) {
      ...
      if (plainOrXER == 0)
        ret = snprintf(scr, scrsize, "%ld (%s)", value, el->enum_name);
      else
      ...
      scr = scratch;
      ret = snprintf(
        scr, scrsize, (specs && specs->field_unsigned) ? "%lu" : "%ld", value);
    }
  ...
  const char *lstop)
{
  ...
  el_found = (asn_INTEGER_enum_map_t *) bsearch(
    &key,
    specs->value2enum,
    count,
 ...
 */
static enum xer_pbd_rval INTEGER__xer_body_decode(
  asn_TYPE_descriptor_t *td,
  void *sptr,
  ...
  size_t chunk_size)
{
  ...
  if (chunk_size)
    ASN_DEBUG(
      "INTEGER body %ld 0x%2x..0x%2x", (long) chunk_size, *lstart, lstop[-1]);

  if (INTEGER_st_prealloc(st, (chunk_size / 3) + 1)) return XPBD_SYSTEM_FAILURE;
  ...
  for (lp = lstart; lp < lstop; lp++) {
    ...
    switch (lv) {
      ...
      case 0x20:
        switch (state) {
          ...
          case ST_SKIPSPHEX: continue;
          case ST_DIGITS:
            dec_value_end = lp;
            state = ST_DIGITS_TRAILSPACE;
            continue;
          case ST_HEXCOLON: state = ST_HEXDIGITS_TRAILSPACE; continue;
          default: break;
        }
        break;
      case 0x2d: /* '-' */
      ...
      case 0x39:
        switch (state) {
          ...
          case ST_HEXDIGIT2:
            ...
            continue;
          case ST_HEXCOLON: return XPBD_BROKEN_ENCODING;
          case ST_LEADSPACE:
            dec_value = 0;
            dec_value_start = lp;
            /* FALL THROUGH */
          case ST_WAITDIGITS: state = ST_DIGITS; continue;
          default: break;
        }
        break;
      case 0x3c: /* '<', start of XML encoded enumeration */
        if (state == ST_LEADSPACE) {
          ...
          el = INTEGER_map_enum2value(
            (asn_INTEGER_specifics_t *) td->specifics, lstart, lstop);
          if (el) {
    ...
    ASN_DEBUG(
      "INTEGER :: Found non-numeric 0x%2x at %ld", lv, (long) (lp - lstart));
    state = ST_UNEXPECTED;
    break;
  }
  ...
  switch (state) {
    ...
    case ST_DIGITS_TRAILSPACE:
      ...
      switch (asn_strtol_lim(dec_value_start, &dec_value_end, &dec_value)) {
        case ASN_STRTOL_OK: break;
        case ASN_STRTOL_ERROR_RANGE: return XPBD_DECODER_LIMIT;
        case ASN_STRTOL_ERROR_INVAL:
        case ASN_STRTOL_EXPECT_MORE:
        case ASN_STRTOL_EXTRA_DATA: return XPBD_BROKEN_ENCODING;
      }
      break;
    case ST_HEXCOLON:
    case ST_HEXDIGITS_TRAILSPACE:
    ...
    case ST_HEXDIGIT1:
    case ST_HEXDIGIT2:
    case ST_SKIPSPHEX: return XPBD_BROKEN_ENCODING;
    case ST_LEADSPACE:
      /* Content not found */
  ...
  /*
	 ...
	 */
  if (asn_long2INTEGER(st, dec_value)) return XPBD_SYSTEM_FAILURE;

  return XPBD_BODY_CONSUMED;
}

asn_dec_rval_t INTEGER_decode_xer(
...
asn_enc_rval_t INTEGER_encode_xer(
  asn_TYPE_descriptor_t *td,
  void *sptr,
  int ilevel,
  ...
  asn_per_data_t *pd)
{
  ...
  if (ct && ct->flags != APC_UNCONSTRAINED) {
    ...
    if (ct->range_bits >= 0) {
      ...
      ASN_DEBUG("Got value %ld + low %" PRIdMAX, value, ct->lower_bound);
      value += ct->lower_bound;
      if (
        (specs && specs->field_unsigned) ? asn_ulong2INTEGER(st, value) :
  ...
  if (ct && ct->lower_bound) {
    ...
    if (asn_long2INTEGER(st, value + ct->lower_bound)) _ASN_DECODE_FAILED;
  }
  ...
  if (ct && ct->flags != APC_UNCONSTRAINED) {
    ...
    if (ct->range_bits >= 0) {
      if (ct->range_bits > 16) {
        ...
        for (i = 1;; i++) {
          int upper = 1 << i;
          if (upper >= max_range_bytes) break;
        }
        ASN_DEBUG(
          "Can encode %d (%d bytes) in %d bits",
          ct->range_bits,
          max_range_bytes,
          i);

        if ((length = per_get_few_bits(pd, i)) < 0) _ASN_DECODE_FAILED;

        /* X.691 #12.2.6 length determinant + lb (1) */
        length += 1;
        ASN_DEBUG("Got length %d", length);

        if (aper_get_align(pd) != 0) _ASN_DECODE_STARVED;

        while (length--) {
          int buf = per_get_few_bits(pd, 8);
          if (buf < 0) _ASN_DECODE_STARVED;
          value += (((int64_t) buf) << (8 * length));
        }

        value += ct->lower_bound;
        if (
          (specs && specs->field_unsigned) ? asn_uint642INTEGER(st, value) :
                                             asn_int642INTEGER(st, value))
          _ASN_DECODE_FAILED;
        ASN_DEBUG("Got value %lld + low %lld", value, ct->lower_bound);
      } else {
        long value = 0;
        if (ct->range_bits < 8) {
          value = per_get_few_bits(pd, ct->range_bits);
          ...
          if (aper_get_align(pd) < 0) _ASN_DECODE_FAILED;
          value = per_get_few_bits(pd, ct->range_bits);
          if (value < 0) _ASN_DECODE_STARVED;
        } else {
          /* Align */
        ...
        if (
          (specs && specs->field_unsigned) ? asn_ulong2INTEGER(st, value) :
                                             asn_long2INTEGER(st, value))
          _ASN_DECODE_FAILED;
        ASN_DEBUG("Got value %ld + low %lld", value, ct->lower_bound);
      }
      return rval;
    } else {
      _ASN_DECODE_FAILED;
  ...
  asn_per_outp_t *po)
{
  ...
  if (ct) {
    ...
    if (specs && specs->field_unsigned) {
      ...
      if (ct->flags & APC_SEMI_CONSTRAINED) {
        ...
        if (
          uval < (unsigned long) ct->lower_bound ||
          uval > (unsigned long) ct->upper_bound)
          inext = 1;
      }
      ASN_DEBUG(
        "Value %lu (%02x/%d) lb %" PRIuMAX " ub %" PRIuMAX " %s",
        uval,
        st->buf[0],
        st->size,
        ct->lower_bound,
      ...
      ASN_DEBUG(
        "Value %ld (%02x/%d) lb %" PRIdMAX " ub %" PRIdMAX " %s",
        value,
        st->buf[0],
        st->size,
        ct->lower_bound,
  ...
  if (ct && ct->range_bits >= 0) {
    ...
    if (ct->range_bits == 32) {
      ...
      if (per_put_few_bits(po, v >> 1, 31) || per_put_few_bits(po, v, 1))
        _ASN_ENCODE_FAILED;
    } else {
      if (per_put_few_bits(po, value - ct->lower_bound, ct->range_bits))
        _ASN_ENCODE_FAILED;
    }
    _ASN_ENCODED_OK(er);
  }
  ...
  asn_per_outp_t *po)
{
  ...
  if (ct) {
    ...
    if (specs && specs->field_unsigned) {
      uint64_t uval;
      if (asn_INTEGER2uint64(st, &uval)) _ASN_ENCODE_FAILED;
      /* Check proper range */
      if (ct->flags & APC_SEMI_CONSTRAINED) {
        if (uval < ct->lower_bound) inext = 1;
      } else if (ct->range_bits >= 0) {
        if (uval < ct->lower_bound || uval > ct->upper_bound) inext = 1;
      }
      ASN_DEBUG(
        "Value %llu (%02x/%d) lb %llu ub %llu %s",
        uval,
        st->buf[0],
        st->size,
        ct->lower_bound,
      ...
      ASN_DEBUG(
        "Value %lld (%02x/%d) lb %lld ub %lld %s",
        value,
        st->buf[0],
        st->size,
        ct->lower_bound,
  ...
  if (ct && ct->range_bits >= 0) {
    ...
    if (ct->range_bits < 8) {
      ...
      if (aper_put_align(po) < 0) _ASN_ENCODE_FAILED;
      if (per_put_few_bits(po, 0x00 | value, ct->range_bits))
        _ASN_ENCODE_FAILED;
    } else if (ct->range_bits <= 16) {
      // Consume the bytes to align on octet
      if (aper_put_align(po) < 0) _ASN_ENCODE_FAILED;
      if (per_put_few_bits(po, 0x0000 | value, 16)) _ASN_ENCODE_FAILED;
    } else {
      ...
      ASN_DEBUG(
        "Putting n - lb (%" PRIdMAX
        ") with range %d bytes (real size - lb (1)) %d in %d bits",
        v,
        max_range_bytes,
        j,
        i);
      ...
      if (per_put_few_bits(po, j, i)) _ASN_ENCODE_FAILED;
...
int asn_ulong2INTEGER(INTEGER_t *st, unsigned long value)
{
  ...
  buf[0] = 0;
  for (b = buf + 1, shr = (sizeof(unsigned long) - 1) * 8; b < end;
       shr -= 8, b++)
    *b = (uint8_t)(value >> shr);
...
enum asn_strtol_result_e asn_strtol(const char *str, const char *end, long *lp)
{
  ...
  switch (asn_strtol_lim(str, &endp, lp)) {
    case ASN_STRTOL_ERROR_RANGE: return ASN_STRTOL_ERROR_RANGE;
    case ASN_STRTOL_ERROR_INVAL: return ASN_STRTOL_ERROR_INVAL;
    case ASN_STRTOL_EXPECT_MORE:
      return ASN_STRTOL_ERROR_INVAL; /* Retain old behavior */
    case ASN_STRTOL_OK: return ASN_STRTOL_OK;
...
asn_comp_rval_t *INTEGER_compare(
  asn_TYPE_descriptor_t *td1,
  void *sptr1,
