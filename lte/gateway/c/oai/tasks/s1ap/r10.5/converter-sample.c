...
#define _ASX(x) _AXS(x)
        ...
        if (pduType->uper_decoder)
          fprintf(
            stderr,
            "  -iper        Input is in Unaligned PER (Packed Encoding Rules) "
            "(DEFAULT)\n");
        fprintf(
          stderr,
        ...
        fprintf(
          stderr, "  -ixer        Input is in XER (XML Encoding Rules)\n");
        ...
        if (pduType->uper_decoder)
          fprintf(
            stderr, "  -per-nopad   Assume PER PDUs are not padded (-iper)\n");
#ifdef ASN_PDU_COLLECTION
        fprintf(
          stderr,
...
static void buffer_shift_left(size_t offset, int bits)
{
  ...
  DEBUG(
    "Shifting left %d bits off %ld (o=%ld, u=%ld, l=%ld)",
    bits,
    (long) offset,
    (long) DynamicBuffer.offset,
    (long) DynamicBuffer.unbits,
...
static void add_bytes_to_buffer(const void *data2add, size_t bytes)
{
  ...
  if (
    ...
    DEBUG(
      "\tBuffer reallocated to %ld (%d time)",
      newsize,
      DynamicBuffer.nreallocs);
  }
  ...
  int on_first_pdu)
{
  ...
  for (tolerate_eof = 1; /* Allow EOF first time buffer is non-empty */
    ...
    switch (iform) {
      case INP_BER:
        rval = ber_decode(
          opt_codec_ctx, pduType, (void **) &structure, i_bptr, i_size);
        break;
      case INP_XER:
        rval = xer_decode(
          opt_codec_ctx, pduType, (void **) &structure, i_bptr, i_size);
  ...
  /*
	 ...
	 */
  if (
    on_first_pdu || DynamicBuffer.length ||
