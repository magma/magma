 ...
 */
asn_dec_rval_t OCTET_STRING_decode_ber(
  ...
  switch (ctx->phase) {
    ...
    phase1:
      ...
      do {
        ...
        ssize_t Left =
          ((!sel || (size_t) sel->left >= size) ? (ssize_t) size : sel->left);
        ...
        if (sel && sel->left <= 0 && sel->want_nulls == 0) {
          if (sel->prev) {
            ...
            if (prev->left != -1) {
              if (prev->left < sel->got) RETURN(RC_FAIL);
              prev->left -= sel->got;
            }
            prev->got += sel->got;
        ...
        ASN_DEBUG(
          "fetch tag(size=%ld,L=%ld), %sstack, left=%ld, wn=%ld, tl=%ld",
          (long) size,
          (long) Left,
          sel ? "" : "!",
          (long) (sel ? sel->left : 0),
        ...
        ll = ber_fetch_length(
          tlv_constr, (const char *) buf_ptr + tl, Left - tl, &tlv_len);
        ASN_DEBUG(
          "Got tag=%s, tc=%d, left=%ld, tl=%ld, len=%ld, ll=%ld",
        ...
        if (
          sel && sel->want_nulls && ((const uint8_t *) buf_ptr)[0] == 0 &&
          ((const uint8_t *) buf_ptr)[1] == 0) {
          ASN_DEBUG("Eat EOC; wn=%d--", sel->want_nulls);

          if (
            type_variant == ASN_OSUBV_ANY && (tag_mode != 1 || sel->cont_level))
            APPEND("\0\0", 2);
        ...
        switch (type_variant) {
          ...
          default:
            ...
            /* Fall through */
          case ASN_OSUBV_ANY: expected_tag = tlv_tag; break;
        }
        ...
        if (tlv_tag != expected_tag) {
          ...
          ber_tlv_tag_snprint(
            td->tags[td->tags_count - 1], buf[1], sizeof(buf[1]));
        ...
        ASN_DEBUG(
          "+EXPECT2 got=%ld left=%ld, wn=%d, clvl=%d",
          (long) sel->got,
          (long) sel->left,
          sel->want_nulls,
          sel->cont_level);
 ...
 */
asn_enc_rval_t OCTET_STRING_encode_der(
  asn_TYPE_descriptor_t *td,
  void *sptr,
  int tag_mode,
  ber_tlv_tag_t tag,
  ...
  int have_more)
{
  ...
  for (; p < pend; p++) {
    ...
    switch (ch) {
      ...
      case 0x37:
      case 0x38:
      case 0x39: /*56789*/ clv = (clv << 4) + (ch - 0x30); break;
      case 0x41:
      case 0x42:
      case 0x43: /* ABC */
      case 0x44:
      case 0x45:
      case 0x46: /* DEF */ clv = (clv << 4) + (ch - 0x41 + 10); break;
      case 0x61:
      case 0x62:
      case 0x63: /* abc */
      case 0x64:
      case 0x65:
      case 0x66: /* def */ clv = (clv << 4) + (ch - 0x61 + 10); break;
      default:
        *buf = 0; /* JIC */
        return -1;
    }
    ...
    switch (ch) {
      ...
      case 0x31:
        ...
        *buf |= (ch & 1) << bits_unused;
        break;
      default: st->bits_unused = bits_unused; return -1;
    }
  }
...
OS__strtoent(int base, const char *buf, const char *end, int32_t *ret_value)
{
  ...
  for (p = buf; p < end; p++) {
    ...
    switch (ch) {
      ...
      case 0x37:
      case 0x38:
      case 0x39: /*56789*/ val = val * base + (ch - 0x30); break;
      case 0x41:
      case 0x42:
      case 0x43: /* ABC */
      case 0x44:
      case 0x45:
      case 0x46: /* DEF */ val = val * base + (ch - 0x41 + 10); break;
      case 0x61:
      case 0x62:
      case 0x63: /* abc */
      case 0x64:
      case 0x65:
      case 0x66: /* def */ val = val * base + (ch - 0x61 + 10); break;
      case 0x3b: /* ';' */ *ret_value = val; return (p - buf) + 1;
      default: return -1; /* Character set error */
    }
  }
...
static int OCTET_STRING_per_get_characters(
  asn_per_data_t *po,
  uint8_t *buf,
  size_t units,
  unsigned int bpc,
...
static int OCTET_STRING_per_put_characters(
  asn_per_outp_t *po,
  const uint8_t *buf,
  size_t units,
  unsigned int bpc,
  ...
  asn_per_data_t *pd)
{
  ...
  ASN_DEBUG(
    "PER Decoding %s size %" PRIdMAX " .. %" PRIdMAX " bits %d",
  ...
  if (csiz->effective_bits == 0) {
    int ret;
    if (bpc) {
      ASN_DEBUG("Encoding OCTET STRING size %" PRIdMAX, csiz->upper_bound);
      ...
      ASN_DEBUG("Encoding BIT STRING size %" PRIdMAX, csiz->upper_bound);
  ...
  asn_per_data_t *pd)
{
  ...
  if (csiz->effective_bits == 0) {
    ...
    if (bpc) {
      ASN_DEBUG("Decoding OCTET STRING size %" PRIdMAX, csiz->upper_bound);
      ret = OCTET_STRING_per_get_characters(
        pd,
        st->buf,
        csiz->upper_bound,
      ...
      ASN_DEBUG("Decoding BIT STRING size %" PRIdMAX, csiz->upper_bound);
      ret = per_get_many_bits(pd, st->buf, 0, unit_bits * csiz->upper_bound);
    }
    if (ret < 0) RETURN(RC_WMORE);
    consumed_myself += unit_bits * csiz->upper_bound;
  ...
  asn_per_outp_t *po)
{
  ...
  switch (specs->subvariant) {
    default:
    case ASN_OSUBV_ANY: _ASN_ENCODE_FAILED;
    case ASN_OSUBV_BIT:
      canonical_unit_bits = unit_bits = 1;
      bpc = OS__BPC_BIT;
      sizeinunits = st->size * 8 - (st->bits_unused & 0x07);
      ASN_DEBUG(
        "BIT STRING of %d bytes, %d bits unused", sizeinunits, st->bits_unused);
      break;
    case ASN_OSUBV_STR:
      canonical_unit_bits = unit_bits = 8;
      if (cval->flags & APC_CONSTRAINED) unit_bits = cval->range_bits;
  ...
  if (csiz->effective_bits >= 0) {
    ASN_DEBUG(
      "Encoding %d bytes (%" PRIdMAX "), length in %d bits",
      st->size,
      sizeinunits - csiz->lower_bound,
      csiz->effective_bits);
    ret = per_put_few_bits(
      po, sizeinunits - csiz->lower_bound, csiz->effective_bits);
  ...
  asn_per_outp_t *po)
{
  ...
  switch (specs->subvariant) {
    ...
    case ASN_OSUBV_BIT:
      ...
      bpc = OS__BPC_BIT;
      sizeinunits = st->size * 8 - (st->bits_unused & 0x07);
      ASN_DEBUG("BIT STRING of %d bits", sizeinunits);
      break;
    case ASN_OSUBV_ANY:
    case ASN_OSUBV_STR:
      canonical_unit_bits = unit_bits = 8;
  ...
  if (sizeinunits == 0) {
    if (aper_put_length(po, -1, 0)) _ASN_ENCODE_FAILED;
    _ASN_ENCODED_OK(er);
  }
...
asn_comp_rval_t *OCTET_STRING_compare(
