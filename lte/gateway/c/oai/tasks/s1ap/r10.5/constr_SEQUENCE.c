 ...
 */
asn_dec_rval_t SEQUENCE_decode_ber(
  ...
  switch (ctx->phase) {
    ...
    case 1:
      ...
      for (edx = (ctx->step >> 1); edx < td->elements_count;
        ...
        if (
          ctx->left == 0 /* No more stuff is expected */
          && (
        ...
        if (n == opt_edx_end) {
          ...
          if (!IN_EXTENSION_GROUP(specs, edx + elements[edx].optional)) {
            ASN_DEBUG(
              "Unexpected tag %s (at %d)", ber_tlv_tag_string(tlv_tag), edx);
            ASN_DEBUG(
              "Expected tag %s (%s)%s",
        ...
        switch (rval.code) {
          ...
          case RC_WMORE: /* More data expected */
            ...
            ASN_DEBUG(
              ...
              (long) size);
            /* Fall through */
          case RC_FAIL: /* Fatal error */ RETURN(RC_FAIL);
        } /* switch(rval) */
 ...
 */
asn_enc_rval_t SEQUENCE_encode_der(
  asn_TYPE_descriptor_t *td,
  void *sptr,
  int tag_mode,
  ber_tlv_tag_t tag,
  ...
  void *app_key)
{
  ...
  for (edx = 0; edx < td->elements_count; edx++) {
    ...
    erval = elm->type->der_encoder(
      elm->type, memb_ptr, elm->tag_mode, elm->tag, 0, 0);
    if (erval.encoded == -1) return erval;
    computed_size += erval.encoded;
    ASN_DEBUG(
      "Member %d %s estimated %ld bytes", edx, elm->name, (long) erval.encoded);
  }
  ...
  for (edx = ctx->step; ctx->phase <= 3;) {
      ...
      default:
        switch (ch_type) {
          ...
          case PXER_TEXT:         /* Ignore free-standing text */
            ...
            continue;
          case PXER_TAG: break; /* Check the rest down there */
        }
    }
    ...
    if (ctx->phase == 3) {
      switch (xer_skip_unknown(tcv, &ctx->left)) {
        case -1: ctx->phase = 4; RETURN(RC_FAIL);
        case 0: XER_ADVANCE(ch_size); continue;
        case 1:
          XER_ADVANCE(ch_size);
          ctx->phase = 1;
          continue;
        case 2: ctx->phase = 1; break;
      }
    }
          ...
          for (n = edx; n < edx_end; n++) {
            ...
            switch (tcv) {
              ...
              case XCT_OPENING:
                ...
                break;
              case XCT_UNKNOWN_OP:
              case XCT_UNKNOWN_BO: continue;
              default: n = edx_end; break; /* Phase out */
            }
            break;
          }
  ...
  void *app_key)
{
  ...
  /* Dump preamble */
  if (
    cb(td->name, strlen(td->name), app_key) < 0 || cb(" ::= {", 6, app_key) < 0)
    return -1;
  ...
  for (edx = 0; edx < td->elements_count; edx++) {
    ...
    if (
      cb(elm->name, strlen(elm->name), app_key) < 0 || cb(": ", 2, app_key) < 0)
      return -1;
  ...
  asn_per_data_t *pd)
{
  ...
  for (edx = 0; edx < td->elements_count; edx++) {
    ...
    rv = elm->type->uper_decoder(
      opt_codec_ctx, elm->type, elm->per_constraints, memb_ptr2, pd);
    if (rv.code != RC_OK) {
  ...
  if (extpresent) {
    ...
    epmd.nbits = bmlength;
    ASN_DEBUG(
      "Read in extensions bitmap for %s of %d bits (%x..)",
      td->name,
      bmlength,
      *epres);
...
static int SEQUENCE_handle_extensions(
  asn_TYPE_descriptor_t *td,
  void *sptr,
  asn_per_outp_t *po1,
  asn_per_outp_t *po2)
  ...
  asn_per_outp_t *po)
{
  ...
  for (edx = 0; edx < ((specs->ext_after < 0) ? td->elements_count :
    ...
    er =
      elm->type->uper_encoder(elm->type, elm->per_constraints, *memb_ptr2, po);
  ...
  asn_per_outp_t *po)
{
  ...
  for (edx = 0; edx < ((specs->ext_after < 0) ? td->elements_count :
    ...
    ASN_DEBUG("Encoding %s->%s", td->name, elm->name);
    er =
      elm->type->aper_encoder(elm->type, elm->per_constraints, *memb_ptr2, po);
    if (er.encoded == -1) return er;
  }
...
asn_comp_rval_t *SEQUENCE_compare(
