...
static asn_dec_rval_t uper_sot_suck(
  asn_codec_ctx_t *,
...
#define UPDRESTOREPD                                                           \
  ...
  if (padding) {
    ...
    switch (pvalue) {
      ...
      default:
        ASN_DEBUG(
          "Non-blank padding (%d bits 0x%02x)", (int) padding, (int) pvalue);
        UPDRESTOREPD;
        _ASN_DECODE_FAILED;
    }
  }
  ...
  if (arg.unclaimed) {
    ...
    switch (per_skip_bits(pd, arg.unclaimed)) {
      case -1:
        ...
        _ASN_DECODE_STARVED;
      case 0: ASN_DEBUG("Got claim of %d", (int) arg.unclaimed); break;
      default:
...
asn_dec_rval_t uper_open_type_get(
  asn_codec_ctx_t *ctx,
  asn_TYPE_descriptor_t *td,
  asn_per_constraints_t *constraints,
  void **sptr,
...
static int uper_ugot_refill(asn_per_data_t *pd)
{
  ...
  ASN_DEBUG("Refilled pd%s old%s", per_data_string(pd), per_data_string(oldpd));
  return 0;
}
