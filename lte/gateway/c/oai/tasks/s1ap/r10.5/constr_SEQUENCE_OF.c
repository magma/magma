  ...
  void *app_key)
{
  ...
  for (edx = 0; edx < list->count; edx++) {
    ...
    if (!memb_ptr) continue;
    erval =
      elm->type->der_encoder(elm->type, memb_ptr, 0, elm->tag, cb, app_key);
    if (erval.encoded == -1) return erval;
  ...
  asn_per_outp_t *po)
{
  ...
  if (ct) {
    ...
    ASN_DEBUG(
      "lb %" PRIdMAX " ub %" PRIdMAX " %s",
      ct->lower_bound,
      ct->upper_bound,
      ct->flags & APC_EXTENSIBLE ? "ext" : "fix");
    if (ct->flags & APC_EXTENSIBLE) {
      /* Declare whether size is in extension root */
      if (per_put_few_bits(po, not_in_root, 1)) _ASN_ENCODE_FAILED;
      if (not_in_root) ct = 0;
  ...
  for (seq = -1; seq < list->count;) {
    ...
    while (mayEncode--) {
      ...
      er =
        elm->type->uper_encoder(elm->type, elm->per_constraints, memb_ptr, po);
  ...
  asn_per_outp_t *po)
{
  ...
  ASN_DEBUG(
    "Encoding %s as SEQUENCE OF size (%d) using ALIGNED PER",
    td->name,
    list->count);
  ...
  for (seq = -1; seq < list->count;) {
    ...
    while (mayEncode--) {
      ...
      if (!memb_ptr) _ASN_ENCODE_FAILED;
      er =
        elm->type->aper_encoder(elm->type, elm->per_constraints, memb_ptr, po);
      if (er.encoded == -1) _ASN_ENCODE_FAILED;
    }
  }
