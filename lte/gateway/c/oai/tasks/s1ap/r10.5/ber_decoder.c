  ...
  int *opt_tlv_form)
{
  ...
  if (tag_mode == 0 && tagno == td->tags_count) {
    ...
    len_len = ber_fetch_length(
      tlv_constr, (const char *) ptr + tag_len, size - tag_len, &tlv_len);
    switch (len_len) {
      case -1: RETURN(RC_FAIL);
      case 0: RETURN(RC_WMORE);
    }
  ...
  for ((void) tagno; tagno < td->tags_count; tagno++, step++) {
    ...
    if (limit_len == -1) {
      ...
      /*
			 ...
			 */
      ASN_DEBUG(
        "Outer TLV is %ld and inner is %ld", (long) limit_len, (long) tlv_len);
      RETURN(RC_FAIL);
    }
