...
static asn_enc_rval_t aper_encode_internal(
  asn_TYPE_descriptor_t *td,
  asn_per_constraints_t *,
  void *sptr,
  asn_app_consume_bytes_f *cb,
  ...
  void **buffer_r)
{
  ...
  switch (er.encoded) {
    case -1: FREEMEM(key.buffer); return -1;
    case 0:
      FREEMEM(key.buffer);
      key.buffer = MALLOC(1);
      if (key.buffer) {
  ...
  void *app_key)
{
  ...
  /*
	 ...
	 */
  if (!td || !td->uper_encoder) _ASN_ENCODE_FAILED; /* PER is not compiled in */
  ...
  void *app_key)
{
  ...
  /*
	 * Invoke type-specific encoder.
	 */
  if (!td || !td->aper_encoder) _ASN_ENCODE_FAILED; /* PER is not compiled in */
  ...
  if (er.encoded != -1) {
    ...
    if (_aper_encode_flush_outp(&po)) _ASN_ENCODE_FAILED;
  }
