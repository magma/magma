  ...
  asn_per_data_t *pd)
{
  ...
  if (ct && ct->effective_bits >= 0) {
    ...
    ASN_DEBUG(
      "Preparing to fetch %ld+%" PRIdMAX " elements from %s",
      (long) nelems,
      ct->lower_bound,
      td->name);
  ...
  do {
    ...
    for (i = 0; i < nelems; i++) {
      ...
      rv = elm->type->uper_decoder(
        opt_codec_ctx, elm->type, elm->per_constraints, &ptr, pd);
      ASN_DEBUG(
        "%s SET OF %s decoded %d, %p", td->name, elm->type->name, rv.code, ptr);
      if (rv.code == RC_OK) {
  ...
  do {
    ...
    if (nelems < 0) {
      nelems = aper_get_length(
        pd,
        ct ? ct->upper_bound - ct->lower_bound + 1 : -1,
...
asn_comp_rval_t *SET_OF_compare(
