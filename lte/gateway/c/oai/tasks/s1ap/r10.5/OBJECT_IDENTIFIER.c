...
int OBJECT_IDENTIFIER_get_single_arc(
  uint8_t *arcbuf,
  unsigned int arclen,
  signed int add,
  void *rvbufp,
  unsigned int rvsize)
{
...

  {
    ...
    for (bits = rvsize - arclen; bits > CHAR_BIT;
  ...
  void *app_key)
{
  ...
  if (OBJECT_IDENTIFIER_get_single_arc(
        arcbuf, arclen, add, &accum, sizeof(accum)))
    return -1;
  ...
  if (accum) {
    ...
    if (cb(p, len, app_key) < 0) return -1;
    return len;
  } else {
    *scratch = 0x30;
    if (cb(scratch, 1, app_key) < 0) return -1;
    return 1;
  }
}

int OBJECT_IDENTIFIER_print_arc(
  uint8_t *arcbuf,
  int arclen,
  int add,
  asn_app_consume_bytes_f *cb,
  ...
  void *app_key)
{
  ...
  for (i = 0, startn = 0; i < st->size; i++) {
    ...
    add = OBJECT_IDENTIFIER__dump_arc(
      &st->buf[startn], i - startn + 1, add, cb, app_key);
    if (add < 0) return -1;
    wrote_len += 1 + add;
    startn = i + 1;
    add = 0;
  ...
  size_t chunk_size)
{
  ...
  if ((size_t) arcs_count > sizeof(s_arcs) / sizeof(s_arcs[0])) {
    ...
    ret = OBJECT_IDENTIFIER_parse_arcs(
      (const char *) chunk_buf, chunk_size, arcs, arcs_count, &endptr);
  ...
  void *app_key)
{
  ...
  if (OBJECT_IDENTIFIER__dump_body(st, cb, app_key) < 0) return -1;

  return (cb(" }", 2, app_key) < 0) ? -1 : 0;
}

int OBJECT_IDENTIFIER_get_arcs(
  ...
  unsigned int arc_slots)
{
  ...
  for (i = 0; i < oid->size; i++) {
    ...
    if (num_arcs == 0) {
      ...
      *(unsigned char
          *) ((char *) arcs + ((*(char *) &LE) ? 0 : (arc_type_size - 1))) =
        first_arc;
    ...
    if (arcs < arcs_end) {
      if (OBJECT_IDENTIFIER_get_single_arc(
            &oid->buf[startn], i - startn + 1, add, arcs, arc_type_size))
        return -1;
      startn = i + 1;
      arcs = ((char *) arcs) + arc_type_size;
      add = 0;
 ...
 */
int OBJECT_IDENTIFIER_set_single_arc(
  uint8_t *arcbuf,
  const void *arcval,
  ...
  unsigned int arc_slots)
{
  ...
  if (
    !oid || !arcs || arc_type_size < 1 || arc_type_size > 16 || arc_slots < 2) {
    errno = EINVAL;
    return -1;
  }
  ...
  switch (arc_type_size) {
    ...
    default:
      ...
      if (isLittleEndian) { /* Little endian (x86) */
        ...
        /* If more significant bytes are present,
			 * make them > 255 quick */
        for (ps = (const unsigned char *) arcs, pe = ps + arc_type_size - 1;
             ps < pe;
             ps++)
          arc0 |= *ps, arc1 |= *(ps + arc_type_size);
	 ...
	 */
  {
    ...
    *fv++ = 0;
    ...
    bp +=
      OBJECT_IDENTIFIER_set_single_arc(bp, first_value, fv - first_value, 1);
  }

  /*
	 * Save the rest of arcs.
	 */
  for (arcs = ((const char *) arcs) + arc_type_size, i = 2; i < arc_slots;
       i++, arcs = ((const char *) arcs) + arc_type_size) {
    bp += OBJECT_IDENTIFIER_set_single_arc(bp, arcs, arc_type_size, 0);
  }
...
#define _OID_CAPTURE_ARC(oid_text, oid_end)                                    \
  do {                                                                         \
    ...
    switch (asn_strtol_lim(oid_text, &endp, &value)) {                         \
      ...
      case ASN_STRTOL_OK:                                                      \
        if (arcs_count < arcs_slots) arcs[arcs_count] = value;                 \
        arcs_count++;                                                          \
        oid_text = endp - 1;                                                   \
        break;                                                                 \
  ...
  for (oid_end = oid_text + oid_txt_length; oid_text < oid_end; oid_text++) {
    switch (*oid_text) {
      ...
      case 0x20: /* whitespace */
        switch (state) {
          case ST_LEADSPACE:
          case ST_TAILSPACE: continue;
          case ST_AFTERVALUE: state = ST_TAILSPACE; continue;
          case ST_WAITDIGITS:
      ...
      case 0x2e: /* '.' */
        switch (state) {
          ...
          case ST_WAITDIGITS:
            ...
            return -1;
            break;
          case ST_AFTERVALUE: state = ST_WAITDIGITS; continue;
        }
        break;
      case 0x30:
      case 0x31:
  ...
  /* Finalize last arc */
  switch (state) {
    case ST_LEADSPACE: return 0; /* No OID found in input data */
    case ST_WAITDIGITS:
      errno = EINVAL; /* Broken OID */
      return -1;
    case ST_AFTERVALUE:
    case ST_TAILSPACE: return arcs_count;
  }
