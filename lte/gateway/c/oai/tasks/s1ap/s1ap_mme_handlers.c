...
int s1ap_mme_handle_initial_context_setup_response(
  ...
  if (ie) {
    ...
    if ((ue_ref_p = s1ap_state_get_ue_mmeid(
             state, (uint32_t) mme_ue_s1ap_id)) == NULL) {
      OAILOG_DEBUG(
          LOG_S1AP,
          "No UE is attached to this mme UE s1ap id: " MME_UE_S1AP_ID_FMT
          " %u(10)\n",
...
int s1ap_mme_handle_path_switch_request(
  ...
  s1ap_mme_itti_s1ap_path_switch_request(
      assoc_id, new_ue_ref_p->enb->enb_id, new_ue_ref_p->enb_ue_s1ap_id,
...
static bool s1ap_send_enb_deregistered_ind(
    __attribute__((unused)) const hash_key_t keyP, void* const dataP,
...
int s1ap_handle_sctp_disconnection(
  ...
  if (!enb_association->nb_ue_associated) {
    if (reset) {
      ...
      OAILOG_INFO(
          ...
          assoc_id, reset);
      OAILOG_INFO(
          LOG_S1AP, "Moving eNB with association id %u to INIT state\n",
          assoc_id);
  ...
  hashtable_ts_apply_callback_on_elements(
      &enb_association->ue_coll, s1ap_send_enb_deregistered_ind, (void*) &arg,
      (void**) &message_p);
  ...
  // Mark the eNB's s1 state as appopriate, the eNB will be deleted or moved to
  // init state when the last UE's s1 state is cleaned up or clean-up timer
  // expires
  ...
  if (enb_association->s1_state == S1AP_SHUTDOWN) {
    ...
    if (timer_setup(
      ...
      OAILOG_ERROR(
          LOG_S1AP,
          "Failed to start wait_for_ue_cleanup timer for eNB association id  "
          "%u "
          "\n",
          assoc_id);
...
int s1ap_mme_handle_enb_reset(
  ...
  switch (s1ap_reset_type) {
    case RESET_ALL:
      ...
      hashtable_ts_apply_callback_on_elements(
          &enb_association->ue_coll, construct_s1ap_mme_full_reset_req, &arg,
          NULL);
      ...
      for (i = 0; i < resetType->choice.partOfS1_Interface.list.count; i++) {
        ...
        if (s1_sig_conn_id_p->mME_UE_S1AP_ID != NULL) {
          ...
          hashtable_uint64_ts_get(
              imsi_map->mme_ue_id_imsi_htbl, (const hash_key_t) mme_ue_s1ap_id,
              &imsi64);
          if ((ue_ref_p = s1ap_state_get_ue_mmeid(state, mme_ue_s1ap_id)) !=
              NULL) {
...
void s1ap_enb_assoc_clean_up_timer_expiry(
    s1ap_state_t* state, enb_description_t* enb_ref_p) {
  OAILOG_FUNC_IN(LOG_S1AP);
...
int s1ap_mme_handle_enb_configuration_transfer(
  ...
  OAILOG_DEBUG(
      LOG_S1AP,
      "Recieved eNB Confiuration Request from assoc_id "
      "%u\n",
      assoc_id);
  enb_association = s1ap_state_get_enb(state, assoc_id);
  if (enb_association == NULL) {
    OAILOG_ERROR(
        LOG_S1AP,
        "Ignoring eNB Confiuration Request from unknown "
        "assoc %u\n",
        assoc_id);
    OAILOG_FUNC_RETURN(LOG_S1AP, RETURNerror);
  }
...
int s1ap_handle_path_switch_req_ack(
  ...
  s1ap_PathSwitchRequestAcknowledgeIEs_p->value.choice.MME_UE_S1AP_ID =
      path_switch_req_ack_p->mme_ue_s1ap_id;
  s1ap_PathSwitchRequestAcknowledgeIEs_p->value.choice.ENB_UE_S1AP_ID =
      path_switch_req_ack_p->enb_ue_s1ap_id;
  s1ap_PathSwitchRequestAcknowledgeIEs_p->value.choice.SecurityContext.nextHopChainingCount =
      path_switch_req_ack_p->NCC;
  s1ap_PathSwitchRequestAcknowledgeIEs_p->value.choice.SecurityContext.nextHopParameter.buf =
      calloc(AUTH_NEXT_HOP_SIZE, sizeof(uint8_t));
  memcpy(
      s1ap_PathSwitchRequestAcknowledgeIEs_p->value.choice.SecurityContext.nextHopParameter.buf,
      path_switch_req_ack_p->NH, AUTH_NEXT_HOP_SIZE);
  s1ap_PathSwitchRequestAcknowledgeIEs_p->value.choice.SecurityContext.nextHopParameter
