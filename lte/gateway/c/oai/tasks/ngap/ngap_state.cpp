...
gnb_description_t* ngap_state_get_gnb( ngap_state_t* state, sctp_assoc_id_t assoc_id)
{
  ...
  void** resultP)
{
  ...
  if (*amf_ue_ngap_id_p == ue_ref->amf_ue_ngap_id) {
    *resultP = elementP;
    OAILOG_TRACE( LOG_NGAP, "Found ue_ref %p amf_ue_ngap_id " MME_UE_NGAP_ID_FMT "\n",
      ue_ref,
