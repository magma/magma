...
//ue_description_t* ngap_state_get_ue_enbid(
  ...
  gnb_ue_ngap_id_t gnb_ue_ngap_id)
{
  ...
  hashtable_ts_get(
    state_ue_ht, (const hash_key_t) comp_ngap_id, (void**) &ue);

  return ue;
}

//ue_description_t* ngap_state_get_ue_amfid(amf_ue_ngap_id_t amf_ue_ngap_id)
ue_description_t* ngap_state_get_ue_amfid(amf_ue_ngap_id_t amf_ue_ngap_id)
{
  ue_description_t* ue = nullptr;
...
void put_ngap_ue_state(imsi64_t imsi64) {
  if(NgapStateManager::getInstance().is_persist_state_enabled()) {
    ue_description_t* ue_ctxt = ngap_state_get_ue_imsi(imsi64);
    if (ue_ctxt) {
