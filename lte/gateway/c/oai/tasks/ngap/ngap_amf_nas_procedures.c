...
int ngap_amf_handle_initial_ue_message(
  ...
  if (ue_ref == NULL) {
    OAILOG_ERROR(LOG_NGAP, "#####Initial UE message ue_ref:%p ", ue_ref);
    tai_t tai       = {0};
    guamfi_t guamfi = {
    ...
    OAILOG_ERROR(LOG_NGAP, "#####Initial UE message:%p ", ue_ref);
...
int ngap_generate_downlink_nas_transport(
  ...
  if (HASH_TABLE_OK ==
    ...
    OAILOG_ERROR(
        LOG_NGAP, "##### SCTP association id %d i, ue_ref:%p\n", sctp_assoc_id,
        ue_ref);
    if (gnb_ref) {
      ue_ref = ngap_state_get_ue_gnbid(gnb_ref->sctp_assoc_id, gnb_ue_ngap_id);
    } else {
  ...
  OAILOG_ERROR(LOG_NGAP, "##### SCTP  ue_ref:%p\n", ue_ref);
  // TODO remove soon:
  if (!ue_ref) {
    ue_ref = ngap_state_get_ue_amfid(ue_id);
  }
  OAILOG_ERROR(LOG_NGAP, "##### SCTP  ue_ref:%p, ue_id:%d\n", ue_ref, ue_id);
  // finally!
  //  if (!ue_ref) {
  if (ue_ref) {  // ACL_TAG tmp for test
    ...
    OCTET_STRING_fromBuf(
        &ie->value.choice.NAS_PDU, (char*) bdata(*payload),
        (blength(*payload) - 3));
...
void ngap_handle_amf_ue_id_notification(
  ...
  // gnb_ue_ngap_id_t gnb_ue_ngap_id =
  // notification_p->gnb_ue_ngap_id;//cobraranu hashtable need to revert
  sctp_assoc_id_t sctp_assoc_id   = 0;
  gnb_ue_ngap_id_t gnb_ue_ngap_id = 0;
  amf_ue_ngap_id_t amf_ue_ngap_id = notification_p->amf_ue_ngap_id;

  OAILOG_ERROR(LOG_NGAP, " ##### sctp_assoc_id:%d\n", sctp_assoc_id);
  OAILOG_ERROR(LOG_NGAP, " ##### gnb_ue_ngap_id:%d\n", gnb_ue_ngap_id);
  OAILOG_ERROR(LOG_NGAP, " ##### amf_ue_ngap_id:%d\n", amf_ue_ngap_id);

  gnb_description_t* gnb_ref = ngap_state_get_gnb(state, sctp_assoc_id);
  OAILOG_ERROR(LOG_NGAP, " ##### gnb_ref:%p\n", gnb_ref);
  if (gnb_ref) {
  ...
  if (ue_ref) {  // tmp for testing
    ...
    for (int i = 0; i < pdusession_resource_setup_req
      ...
      tx_ie->value.choice.UPTransportLayerInformation.choice.gTPTunnel
          .transportLayerAddress.buf = /*BIT_STRING_t*/
          calloc(
              blength(amf_pdu_ses_setup_transfer_req.up_transport_layer_info
...
/*******************************/
      ...
      // Ngap_QosFlowSetupRequestList_t * Qos_list =
      // &tx_ie->value.choice.QosFlowSetupRequestList; //here add sequence of
      // items
      ...
      for (int i = 0; i < /*no_of_qos_items*/ 1; i++) {
        ...
        /* Ngap_QosCharacteristics */
        {
          ...
          qos_item->qosFlowLevelQosParameters.qosCharacteristics.choice
              .nonDynamic5QI.fiveQI = /*typedef long*/
              amf_pdu_ses_setup_transfer_req.qos_flow_setup_request_list
        ...
        qos_item->qosFlowLevelQosParameters.allocationAndRetentionPriority
            .priorityLevelARP = /*typedef long*/
            amf_pdu_ses_setup_transfer_req.qos_flow_setup_request_list
                .qos_flow_req_item.qos_flow_level_qos_param.alloc_reten_priority
                .priority_level;

        qos_item->qosFlowLevelQosParameters.allocationAndRetentionPriority
            .pre_emptionCapability = /*typedef long*/
            amf_pdu_ses_setup_transfer_req.qos_flow_setup_request_list
                .qos_flow_req_item.qos_flow_level_qos_param.alloc_reten_priority
                .pre_emption_cap;

        qos_item->qosFlowLevelQosParameters.allocationAndRetentionPriority
            .pre_emptionVulnerability = /*typedef long*/
            amf_pdu_ses_setup_transfer_req.qos_flow_setup_request_list
                .qos_flow_req_item.qos_flow_level_qos_param.alloc_reten_priority
                .pre_emption_vul;
      ...
      OAILOG_ERROR(
          LOG_NGAP, " ##### NGAP_PDUSESSION_RESOURCE_SETUP_REQ :%ld\n",
          er.encoded);
      ...
      // (void *)bdata(transfer), er.encoded);

      OAILOG_ERROR(
          LOG_NGAP,
          " ##### NGAP_PDUSESSION_RESOURCE_SETUP_REQ transfer->data:%p\n",
          transfer->data);
      OAILOG_ERROR(
          LOG_NGAP,
          " ##### NGAP_PDUSESSION_RESOURCE_SETUP_REQ transfer->data:%p\n",
      ...
      OAILOG_ERROR(
          LOG_NGAP,
          " ##### NGAP_PDUSESSION_RESOURCE_SETUP_REQ  blength(transfer):%d\n",
          blength(transfer));
...
int ngap_generate_ngap_pdusession_resource_rel_cmd(
  ...
  uint32_t length              = 0;
  void* id                     = NULL;
  const gnb_ue_ngap_id_t gnb_ue_ngap_id = pdusessionresource_rel_cmd->gnb_ue_ngap_id;
  const amf_ue_ngap_id_t amf_ue_ngap_id = pdusessionresource_rel_cmd->amf_ue_ngap_id;

  hashtable_ts_get( &state->amfid2associd, (const hash_key_t) amf_ue_ngap_id, (void**) &id);
  if (id) {
    sctp_assoc_id_t sctp_assoc_id = (sctp_assoc_id_t)(uintptr_t) id;
    gnb_description_t* gnb_ref    = ngap_state_get_gnb(state, sctp_assoc_id);
    if (gnb_ref) {
  ...
  if (ue_ref) {  // temp for testing
    ...
    out = &pdu.choice.initiatingMessage.value.choice.PDUSessionResourceReleaseCommand;
     ...
     */
    /* mandatory */
    ie = (Ngap_PDUSessionResourceReleaseCommandIEs_t*) calloc( 1, sizeof(Ngap_PDUSessionResourceReleaseCommandIEs_t));
    ie->id          = Ngap_ProtocolIE_ID_id_AMF_UE_NGAP_ID;
    ie->criticality = Ngap_Criticality_reject;
     ...
     * Fill in the NAS pdu
     */
//    NGAP_FIND_PROTOCOLIE_BY_ID( Ngap_PDUSessionResourceReleaseCommandIEs_t, ie, out, Ngap_ProtocolIE_ID_id_PDUSessionResourceToReleaseListRelCmd, true);
#if 0
    if(pdusessionresource_rel_cmd->nas_pdu) //optional
{
    ...
    OAILOG_ERROR( LOG_NGAP, " ##### items:%d\n", pdusessionresource_rel_cmd->pduSessionResourceToRelReqList.no_of_items);

 //   Ngap_PDUSessionResourceToReleaseItemRelCmd_t* ngap_pdusessionItemIEs[pdusessionresource_rel_cmd->pduSessionResourceToRelReqList .no_of_items];

    for (int i = 0; i < pdusessionresource_rel_cmd->pduSessionResourceToRelReqList.no_of_items; i++) {

//      ngap_pdusessionItemIEs[i] = calloc(1, sizeof *ngap_pdusessionItemIEs[i]);
//      ngap_pdusessionItemIEs[i]->pDUSessionID = pdusessionresource_rel_cmd->pduSessionResourceToRelReqList.item[i]
//              .Pdu_Session_ID;

    Ngap_PDUSessionResourceToReleaseItemRelCmd_t* RelItem = calloc( 1, sizeof(Ngap_PDUSessionResourceToReleaseItemRelCmd_t)); 

    RelItem->pDUSessionID = pdusessionresource_rel_cmd->pduSessionResourceToRelReqList.item[i].Pdu_Session_ID;
	

      Ngap_Cause_t cause_p = {0};
      //ngap_amf_set_cause( &cause_p, Ngap_Cause_PR_radioNetwork, Ngap_CauseRadioNetwork_unspecified);

      cause_p.choice.nas = Ngap_CauseNas_normal_release;
      cause_p.present = Ngap_Cause_PR_nas;

      OCTET_STRING_fromBuf( &RelItem->pDUSessionResourceReleaseCommandTransfer, (char*) &cause_p, sizeof(cause_p));

      for (int j = 0; j < RelItem->pDUSessionResourceReleaseCommandTransfer.size; j++)
        OAILOG_ERROR( LOG_NGAP, " ##### ies: :%02x\n", ((char*)RelItem->pDUSessionResourceReleaseCommandTransfer.buf)[j]);

    //ASN_SEQUENCE_ADD(&out->protocolIEs.list, ie);
    ASN_SEQUENCE_ADD(&ie->value.choice.PDUSessionResourceToReleaseListRelCmd.list, RelItem);
    }
    OAILOG_ERROR(LOG_NGAP, " ##### NGAP_PDUSESSIONRESOURCE_REL_REQ\n");
...
int ngap_generate_ngap_pdusession_resource_rel_cmd(
  ...
  // pdu_session_resource_release_command_transfer Rel_Cause = {0};
...
//    ie, out, Ngap_ProtocolIE_ID_id_PDUSessionResourceToReleaseListRelCmd,
//    true);
#if 0
    if(pdusessionresource_rel_cmd->nas_pdu) //optional
{
    ...
    for (int i = 0;
...
#if 1
      // Rel_Cause =
      // pdusessionresource_rel_cmd->pduSessionResourceToRelReqList.item[i].PDU_Session_Resource_TO_Release_Command_Transfer;
      ...
      PDUSessionResourceReleaseCommandTransferIEs->cause.present    = 4;
      PDUSessionResourceReleaseCommandTransferIEs->cause.choice.nas = 4;
      //     Ngap_Cause_t cause_p = {0};
      //    cause_p.present = Ngap_Cause_PR_nas;
      //   cause_p.choice.nas = Ngap_CauseNas_normal_release;
      // ngap_amf_set_cause( &cause_p, Ngap_Cause_PR_radioNetwork,
      // Ngap_CauseRadioNetwork_unspecified); ngap_amf_set_Rel_cause( &cause_p,
      // &Rel_Cause);
      ...
      asn_enc_rval_t er = aper_encode_to_buffer(
          &asn_DEF_Ngap_PDUSessionResourceToReleaseItemRelCmd, NULL,
          PDUSessionResourceReleaseCommandTransferIEs, buffer, buffer_size);

      bstring transfer = blk2bstr(buffer, er.encoded);

      OAILOG_ERROR(
          LOG_NGAP, " ##### NGAP_PDUSESSION_RESOURCE_REL_REQ :%ld\n",
          er.encoded);
      OAILOG_ERROR(
          LOG_NGAP,
          " ##### NGAP_PDUSESSION_RESOURCE_REL_REQ transfer->data:%p\n",
          transfer->data);
      OAILOG_ERROR(
          LOG_NGAP,
          " ##### NGAP_PDUSESSION_RESOURCE_REL_REQ transfer->data:%p\n",
      ...
      OAILOG_ERROR(
          LOG_NGAP,
          " ##### NGAP_PDUSESSION_RESOURCE_REL_REQ  blength(transfer):%d\n",
          blength(transfer));
      ...
      RelItem->pDUSessionResourceReleaseCommandTransfer.size =
          blength(transfer);

      //      OCTET_STRING_fromBuf(
      //      &RelItem->pDUSessionResourceReleaseCommandTransfer, (char*)
      //      &cause_p, sizeof(cause_p));

      // RelItem->pDUSessionResourceReleaseCommandTransfer.buf = (uint8_t*)
      // calloc(1, sizeof(uint8_t));
      // RelItem->pDUSessionResourceReleaseCommandTransfer.buf[0] = 0x10;
      // RelItem->pDUSessionResourceReleaseCommandTransfer.size = 1;

#endif

      for (int j = 0;
           j < RelItem->pDUSessionResourceReleaseCommandTransfer.size; j++)
        OAILOG_ERROR(
            LOG_NGAP, " ##### ies: :%02x\n",
            ((char*) RelItem->pDUSessionResourceReleaseCommandTransfer.buf)[j]);
    ...
    // ue_ref->sctp_stream_send, ue_ref->amf_ue_ngap_id);
    ngap_amf_itti_send_sctp_request(&b, ASSOC_ID, 0, 1);
    // ngap_amf_itti_send_sctp_request(&b, 65, 0, 1);
    OAILOG_ERROR(LOG_NGAP, " ##### NGAP_PDUSESSIONRESOURCE_REL_REQ\n");
  }
