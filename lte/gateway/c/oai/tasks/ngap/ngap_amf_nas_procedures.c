...
//#include "NGap-AllocationAndRetentionPriority.h"
//#include "NGap-E-RABLevelQoSParameters.h"
//#include "NGap-E-RABToBeSetupItemBearerSUReq.h"
//#include "NGap-E-RABToBeSetupItemCtxtSUReq.h"
#include "Ngap_EUTRA-CGI.h"
//#include "NGap-EncryptionAlgorithms.h"
#include "Ngap_GBR-QosInformation.h"
#include "Ngap_GUAMI.h"
//#include "NGap-IntegrityProtectionAlgorithms.h"
#include "Ngap_NAS-PDU.h"
#include "Ngap_PLMNIdentity.h"
#include "Ngap_ProcedureCode.h"
#include "Ngap_ProtocolIE-Field.h"
//#include "NGap-S-TMSI.h"
    ...
    const sctp_stream_id_t stream, Ngap_NGAP_PDU_t* pdu) 
{
  ...
  Ngap_InitialUEMessage_IEs_t *ie = NULL, *ie_e_tmsi, *ie_csg_id = NULL, *ie_guamfi, *ie_cause;
  ...
  gnb_ue_ngap_id = (gnb_ue_ngap_id_t)(ie->value.choice.RAN_UE_NGAP_ID & GNB_UE_NGAP_ID_MASK);
  OAILOG_INFO(
      LOG_NGAP,
  ...
  if (ue_ref == NULL) {
    ...
    // TAI mandatory IE
    //NGAP_FIND_PROTOCOLIE_BY_ID( Ngap_InitialUEMessage_IEs_t, ie, container, Ngap_ProtocolIE_ID_id_SupportedTAList, true);
    NGAP_FIND_PROTOCOLIE_BY_ID( Ngap_InitialUEMessage_IEs_t, ie, container, Ngap_ProtocolIE_ID_id_UserLocationInformation, true);
#if 1
ie->value.choice.UserLocationInformation.choice.userLocationInformationNR.tAI.tAC.size=2; // temp hadrcode to test
#endif
    ...
    // CGI mandatory IE
    //NGAP_FIND_PROTOCOLIE_BY_ID( Ngap_InitialUEMessage_IEs_t, ie, container, Ngap_ProtocolIE_ID_id_EUTRA_CGI, true);
    NGAP_FIND_PROTOCOLIE_BY_ID( Ngap_InitialUEMessage_IEs_t, ie, container, Ngap_ProtocolIE_ID_id_UserLocationInformation, true);
    DevAssert(ie->value.choice.UserLocationInformation.choice.userLocationInformationEUTRA.eUTRA_CGI.pLMNIdentity.size == 3);
    TBCD_TO_PLMN_T(&ie->value.choice.UserLocationInformation.choice.userLocationInformationEUTRA.eUTRA_CGI.pLMNIdentity, &ecgi.plmn);
    BIT_STRING_TO_CELL_IDENTITY(
        &ie->value.choice.UserLocationInformation.choice.userLocationInformationEUTRA.eUTRA_CGI.eUTRACellIdentity, ecgi.cell_identity);
...
OAILOG_DEBUG( LOG_NGAP, "cobaranu %s", __func__);
  ...
  if (INVALID_AMF_UE_NGAP_ID == ie->value.choice.AMF_UE_NGAP_ID) {
    ...
    if (!(ue_ref = ngap_state_get_ue_gnbid( gnb_ref->sctp_assoc_id, (gnb_ue_ngap_id_t) gnb_ue_ngap_id))) {
      OAILOG_WARNING(
          LOG_NGAP,
...
NGAP_FIND_PROTOCOLIE_BY_ID(Ngap_UplinkNASTransport_IEs_t, ie_nas_pdu,
  ...
  // TAI mandatory IE
  NGAP_FIND_PROTOCOLIE_BY_ID(
      Ngap_UplinkNASTransport_IEs_t, ie, container, Ngap_ProtocolIE_ID_id_SupportedTAList,
      true);
  OCTET_STRING_TO_TAC(&ie->value.choice.UserLocationInformation.choice.userLocationInformationNR.tAI.tAC, tai.tac);
  DevAssert(ie->value.choice.UserLocationInformation.choice.userLocationInformationNR.tAI.pLMNIdentity.size == 3);
