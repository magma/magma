...
int ngap_generate_ngap_pdusession_resource_setup_req(
  ...
  if (ue_ref) {  // tmp for testing
    ...
    for (int i = 0; i < pdusession_resource_setup_req
      ...
      memcpy( tx_ie->value.choice.UPTransportLayerInformation.choice.gTPTunnel .transportLayerAddress.buf, amf_pdu_ses_setup_transfer_req.up_transport_layer_info.gtp_tnl .endpoint_ip_address->data, blength(amf_pdu_ses_setup_transfer_req.up_transport_layer_info.gtp_tnl .endpoint_ip_address));
...
// OCTET_STRING_fromBuf(
// &ngap_pdusession_setup_item_ies->pDUSessionResourceSetupRequestTransfer, (char
// *)pduSessionResourceSetupRequestTransferIEs,
// sizeof(pduSessionResourceSetupRequestTransferIEs));
...
/*******************************/
      ...
      if (ret != 0) OAILOG_ERROR(LOG_NGAP, " ##### encode error \n");
        ...
        // (char*)
        // bdata(pdusession_resource_setup_req->pduSessionResource_setup_list.item[i].PDU_Session_Resource_Setup_Request_Transfer),blength(pdusession_resource_setup_req->pduSessionResource_setup_list.item[i].PDU_Session_Resource_Setup_Request_Transfer));
