...
void NgapStateManager::create_state()
{
  ...
  //hashtable_ts_init( &state_cache_p->gnbs, max_gnbs_, nullptr, free_wrapper, ht_name); 
  hashtable_ts_init( &state_cache_p->gnbs, 2, nullptr, free_wrapper, ht_name); 
  //state_ue_ht = hashtable_ts_create(max_ues_, nullptr, free_wrapper, ht_name);
  state_ue_ht = hashtable_ts_create(2 , nullptr, free_wrapper, ht_name);
  bdestroy(ht_name);
	

  ht_name = bfromcstr(NGAP_AMF_ID2ASSOC_ID_COLL);
  //hashtable_ts_init( &state_cache_p->amfid2associd, max_ues_, nullptr, hash_free_int_func, ht_name);
  hashtable_ts_init( &state_cache_p->amfid2associd,2, nullptr, hash_free_int_func, ht_name);
  bdestroy(ht_name);
