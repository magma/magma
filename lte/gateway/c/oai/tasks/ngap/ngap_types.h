...
enum amf_ng_gnb_state_s {
  NGAP_INIT, /// The sctp association has been established but n1 hasn't been setup.
  NGAP_RESETING, /// The n1state is resetting due to an SCTP reset on the bound association.
...
enum ng_ue_state_s {
  ...
  NGAP_UE_WAITING_CRR, /// UE Context release Procedure initiated , waiting for UE context Release Complete
};
...
typedef struct m5g_supported_ta_list_s {
  ...
  m5g_supported_tai_items_t supported_tai_items[NGAP_MAX_TAI_ITEMS];  ///< List of TAIs
...
typedef struct m5g_ue_description_s {
  ...
  uint64_t comp_ngap_id; ///< Unique composite UE id (sctp_assoc_id & gnb_ue_ngap_id)
