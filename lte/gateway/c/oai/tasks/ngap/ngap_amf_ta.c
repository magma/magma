...
OAILOG_ERROR( LOG_NGAP, "#####cobraranu :%s, %d", plmn->buf, (int)plmn->size);
OAILOG_ERROR( LOG_NGAP, "#####cobraranu :%02x %02x %02x ", plmn->buf[0], plmn->buf[1], plmn->buf[2]);
...
OAILOG_ERROR( LOG_NGAP, "#####cobraranu :%02x %02x %02x ", buf_plmn[0], buf_plmn[1], buf_plmn[2]);
  ...
  for (i = 0; i < amf_config.served_tai.nb_tai; i++) {
    //OAILOG_TRACE(
    OAILOG_ERROR( LOG_NGAP, "Comparing plmn_mcc %d/%d, plmn_mnc %d/%d plmn_mnc_len %d/%d\n",
...
static int ngap_amf_compare_plmns(Ngap_BroadcastPLMNList_t* b_plmns) {
  ...
  for (i = 0; i < b_plmns->list.count; i++) {
	if (ngap_amf_compare_plmn(&b_plmns->list.array[i]->pLMNIdentity) == TA_LIST_AT_LEAST_ONE_MATCH)
		 matching_occurence++;
...
/* @brief compare a TAC
 */
static int ngap_amf_compare_tac(/*const*/ Ngap_TAC_t* /*const*/ tac) {
  int i              = 0;
  uint16_t tac_value = 0;
  tac->size          = 2;  // ACL_TAG temp to test remove later
