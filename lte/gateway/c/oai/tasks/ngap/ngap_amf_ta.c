...
//#include
//"/home/vagrant/magma/lte/gateway/c/oai/tasks/ngap/messages/asn1/r15.8/src/Ngap_PLMNIdentity.h"
...
static int ngap_amf_compare_plmns(Ngap_BroadcastPLMNList_t* b_plmns) {
  ...
  for (i = 0; i < b_plmns->list.count; i++) {
    ...
    //  TA_LIST_AT_LEAST_ONE_MATCH) matching_occurence++;
...
static int ngap_paging_compare_plmns(
  ...
  for (plmn_idx = 0; plmn_idx < gnb_plmn_count; plmn_idx++) {
    ...
    for (p_plmn_idx = 0; p_plmn_idx < (p_tai_list->numoftac + 1);
      ...
      if ((gnb_plmn->mcc_digit1 == p_plmn.mcc_digit1) &&
          (gnb_plmn->mcc_digit2 == p_plmn.mcc_digit2) &&
          (gnb_plmn->mcc_digit3 == p_plmn.mcc_digit3) &&
          (gnb_plmn->mnc_digit1 == p_plmn.mnc_digit1) &&
          (gnb_plmn->mnc_digit2 == p_plmn.mnc_digit2) &&
          (gnb_plmn->mnc_digit3 == p_plmn.mnc_digit3)) {
        return true;
      }
    }
  }
...
int ngap_paging_compare_ta_lists(
    supported_ta_list_t* gnb_ta_list, const paging_tai_list_t* p_tai_list,
  ...
  for (gnb_tai_count = 0; gnb_tai_count < gnb_ta_list->list_count;
       gnb_tai_count++) {
    supported_tai_items_t* gnb_tai_item = NULL;
