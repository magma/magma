...
static int ngap_send_init_sctp(void)
{
  ...
  //  amf_config.ip.ng_amf_v4.s_addr;      // Need change in future
  ...
  switch (ITTI_MSG_ID(received_message_p)) {
   ...
   case ACTIVATE_MESSAGE: {
    ...
    case SCTP_DATA_IND: {
	  ...
	  // initialUEMessage_IEs = ngap_decode_initialuemessage_ies(SCTP_DATA_IND(received_message_p).payload, &message_id);
	  // ngap_amf_itti_ngap_initial_ue_message(assoc_id, initialUEMessage_IEs);
    } break;
...
int ngap_amf_init(const amf_config_t* amf_config_p)
{
  ...
  if (
    ngap_state_init(      // Need change  ##############################
      amf_config_p->max_ues,
      amf_config_p->max_gnbs,
