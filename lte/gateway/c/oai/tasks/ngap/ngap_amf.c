...
//extern int ngap_amf_decode_pdu(ngap_message* message,  const bstring const raw,  MessagesIds* messages_id) __attribute__((warn_unus    ed_result));
...
static int ngap_send_init_sctp(void)
{
  ...
  message_p->ittiMsg.sctpInit.ipv4_address[0].s_addr = mme_config.ip.ng_amf_v4.s_addr;      // Need change in future
...
static int handle_message (zloop_t* loop, zsock_t* reader, void* arg)
{
  ...
  imsi64_t imsi64 = itti_get_associated_imsi(received_message_p);
   ...
   AssertFatal(state != NULL, "failed to retrieve ngap state (was null)"); // Need change
  ...
  switch (ITTI_MSG_ID(received_message_p)) {
   ...
   case ACTIVATE_MESSAGE: {
    ...
    case SCTP_DATA_IND: {
	  ...
	  if (ngap_amf_decode_pdu( &pdu, SCTP_DATA_IND(received_message_p).payload)) {
...
amf_config_t* config = amf_config_p;
  ...
  if ( ngap_state_init( amf_config_p->max_ues, amf_config_p->max_gnbs, amf_config_p->use_stateless) < 0) {
  ...
  OAILOG_ERROR(LOG_NGAP, "Initializing NGAP interface: DONE\n");
  return RETURNok;
}
...
gnb_description_t* ngap_new_gnb(ngap_state_t* state) {
  ...
  //hashtable_uint64_ts_init(&gnb_ref->ue_id_coll, amf_config.max_ues, NULL, bs); // Need change
  hashtable_uint64_ts_init(&gnb_ref->ue_id_coll, 2, NULL, bs); // Need change  
  bdestroy_wrapper(&bs);
