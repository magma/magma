...
int ngap_amf_set_Rel_cause(
  ...
  Ngap_Cause_PR cause_type =
      (Ngap_Cause_PR) Rel_Cause->cause.cause_group.cause_group_type;
  // cause_group_t cause_present
  DevAssert(cause_p != NULL);
  // cause_p->present = cause_type;
  ...
  switch (cause_type) {
    case Ngap_Cause_PR_radioNetwork:
      ...
      cause_p->choice.radioNetwork =
          Rel_Cause->cause.cause_group.u_group.network_layer.nw_layer_cause;
      break;

    case Ngap_Cause_PR_transport:
      cause_p->choice.transport =
          Rel_Cause->cause.cause_group.u_group.trasport_layer.cause;
      break;

    case Ngap_Cause_PR_nas:
      cause_p->choice.nas = Rel_Cause->cause.cause_group.u_group.nas.cause;
      break;

    case Ngap_Cause_PR_protocol:
      cause_p->choice.protocol =
          Rel_Cause->cause.cause_group.u_group.protocal.cause;
      break;

    case Ngap_Cause_PR_misc:
      cause_p->choice.misc =
          Rel_Cause->cause.cause_group.u_group.miscellaneous.cause;
      break;
...
amf_config_read_lock(&amf_config);
  ...
  OAILOG_ERROR(
      LOG_NGAP, "#####cobraranu :%02x %02x %02x ", buf_plmn[0], buf_plmn[1],
      buf_plmn[2]);
  ...
  OAILOG_ERROR(
      LOG_NGAP, "#####cobraranu :%02x %02x %02x ",
      plmnItem->pLMNIdentity.buf[0], plmnItem->pLMNIdentity.buf[1],
      plmnItem->pLMNIdentity.buf[2]);
...
int ngap_amf_handle_ue_context_release_request(
  ...
  if (ie) {
    ...
    for (int i = 0;
      ...
      OAILOG_ERROR(LOG_NGAP, "PDUID[%d]:%ld ", i, RelItem->pDUSessionID);
...
int ngap_handle_new_association(
  ...
  OAILOG_DEBUG(
      LOG_NGAP, "#####SCTP_NEW_ASSOCIATION: gnb_association:%p\n",
      gnb_association);
  if (gnb_association == NULL) {
    OAILOG_DEBUG(
    ...
    OAILOG_DEBUG(
        LOG_NGAP, "#####SCTP_NEW_ASSOCIATION, associd:%d\n",
        sctp_new_peer_p->assoc_id);
...
int ngap_amf_handle_pduSession_release_response(
  ...
  //  ue_ref_p->amf_ue_ngap_id;
  NGAP_PDUSESSIONRESOURCE_REL_RSP(message_p).amf_ue_ngap_id = amf_ue_ngap_id =
      1;
  // NGAP_PDUSESSIONRESOURCE_REL_RSP(message_p).gnb_ue_ngap_id =
  // ue_ref_p->gnb_ue_ngap_id;
  NGAP_PDUSESSIONRESOURCE_REL_RSP(message_p).gnb_ue_ngap_id = gnb_ue_ngap_id =
      1;
  ...
  message_p->ittiMsgHeader.imsi = imsi64;
  rc = send_msg_to_task(&ngap_task_zmq_ctx, TASK_AMF_APP, message_p);
  OAILOG_ERROR(LOG_NGAP, "REL msg sent to amf\n");
  OAILOG_FUNC_RETURN(LOG_NGAP, rc);
}
#endif
