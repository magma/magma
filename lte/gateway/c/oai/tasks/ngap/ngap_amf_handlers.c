...
//  {ngap_amf_handle_path_switch_request, 0, 0}, 	/* TODO PathSwitchRequest */
    {0, 0, 0},                                   	/* HandoverCancel */

// {0,0, ngap_amf_handle_pduSession_setup_failure},      /* TODO PduSession*/
    {0, 0, 0},                                          /* PduSessionModify */
 // {0, ngap_amf_handle_pduSession_release_response, 0}, /*TODO  PduSessionRelease */
  ...
  //  {ngap_amf_handle_gnb_reset, 0, 0},               /* TODO Reset */
    ...
    {0, ngap_amf_handle_ue_context_modification_response,
     ngap_amf_handle_ue_context_modification_failure}, /* UEContextModification
                                                        */
  //  {ngap_amf_handle_ue_cap_indication, 0, 0}, /* TODO UECapabilityInfoIndication */
...
OAILOG_DEBUG( LOG_NGAP, "cobaranu %s, procedureCode: %ld, pdu->present-1:%d\n", 
		__func__, pdu->choice.initiatingMessage.procedureCode, (pdu->present - 1));

  ngap_message_handler_t handler = ngap_message_handlers[pdu->choice.initiatingMessage.procedureCode] [pdu->present - 1];
  ...
  //return ngap_amf_handle_uplink_nas_transport(state, assoc_id, stream, pdu);
}
...
int ngap_amf_handle_ng_setup_request(
  ...
  OAILOG_MESSAGE_START_SYNC( OAILOG_LEVEL_DEBUG, LOG_NGAP, (&context), "New ng setup request incoming from ");
  ...
  if (ie->value.choice.GlobalRANNodeID.choice.globalGNB_ID.gNB_ID.present ==
    ...
    // Home gNB ID = 28 bits
    uint8_t* gnb_id_buf = ie->value.choice.GlobalRANNodeID.choice.globalGNB_ID.gNB_ID.choice.gNB_ID.buf;

    if (ie->value.choice.GlobalRANNodeID.choice.globalGNB_ID.gNB_ID.choice.gNB_ID.size != 28) {
  ...
  ta_ret = ngap_amf_compare_ta_lists(&ie_supported_tas->value.choice.SupportedTAList);
  ...
  Ngap_SupportedTAList_t* ta_list = &ie_supported_tas->value.choice.SupportedTAList;
  ...
  for (int tai_idx = 0; tai_idx < supp_ta_list->list_count; tai_idx++) {
    ...
    tai->tAC.size=2;//// temp to test remove later
    OCTET_STRING_TO_TAC(
  ...
  if (gnb_name != NULL) {
    memcpy(
        gnb_association->gnb_name, ie_gnb_name->value.choice.RANNodeName.buf, ie_gnb_name->value.choice.RANNodeName.size);
    gnb_association->gnb_name[ie_gnb_name->value.choice.RANNodeName.size] = '\0';
  }
...
int ngap_generate_ng_setup_response(
  ...
  for (i = 0; i < amf_config.served_tai.nb_tai; i++) {
    ...
    if (false == plmn_added) {
      ...
      MCC_MNC_TO_PLMNID( amf_config.served_tai.plmn_mcc[i], amf_config.served_tai.plmn_mnc[i],
          amf_config.served_tai.plmn_mnc_len[i], plmn);
      //ASN_SEQUENCE_ADD(&servedGUAMFI->servedPLMNs.list, plmn);
...
int ngap_amf_handle_initial_context_setup_response(
  ...
  AMF_APP_INITIAL_CONTEXT_SETUP_RSP(message_p).pdusesssion_setup_list.no_of_items =
      ie->value.choice.PDUSessionResourceSetupListCxtRes.list.count;

  for (int item = 0; item < ie->value.choice.PDUSessionResourceSetupListCxtRes.list.count;
       item++) {
    /*
     * Bad, very bad cast...
     */
//Info: need to update 38413 for this
    ...
    AMF_APP_INITIAL_CONTEXT_SETUP_RSP(message_p)
        ...
        .Pdu_Session_ID =
        pduSessionSetupListCtxRes->pDUSessionID;
   // AMF_APP_INITIAL_CONTEXT_SETUP_RSP(message_p)
  //      .pdusesssion_setup_list.item[item]
    //    .gtp_teid = htonl(*((uint32_t*) pduSessionSetupListCtxRes->value.choice
