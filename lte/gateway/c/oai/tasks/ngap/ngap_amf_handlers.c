...
//#include "Ngap_LAI.h"
//#include "Ngap_AMF-Code.h"
//#include "Ngap_AMF-Group-ID.h"
...
//#include "Ngap_ServedGroupIDs.h"
//#include "Ngap_ServedAMFCs.h"
//#include "Ngap_ServedPLMNs.h"
#include "Ngap_TAI.h"
//#include "Ngap_TAIItem.h"
...
ngap_message_handler_t ngap_message_handlers[][3] = {
    ...
    {ngap_amf_handle_initial_ue_message, 0, 0},      /* initialUEMessage 15*/
    ...
    {0, ngap_amf_handle_ue_context_modification_response,
     ngap_amf_handle_ue_context_modification_failure}, /* UEContextModification
                                                        */
    {ngap_amf_handle_ng_setup_request, 0, 0},          /* NGSetup 21*/
    ...
    {0, ngap_amf_handle_pduSession_setup_response,
     ngap_amf_handle_pduSession_setup_failure}, /* TODO PduSession 29*/
    ...
    {ngap_amf_handle_ue_context_release_request,
     ngap_amf_handle_ue_context_release_complete, 0}, /* UEContextRelease 41 */
...
int ngap_amf_handle_message(
  ...
  if (pdu->choice.initiatingMessage.procedureCode >=
    ...
    OAILOG_ERROR(
        LOG_NGAP,
        "NGAP [SCTP %d] Either procedureCode %d or direction %d exceed "
        "expected\n",
  ...
  if (handler == NULL) {
    ...
    OAILOG_ERROR(
        LOG_NGAP, "NGAP: [SCTP %d] No handler for procedureCode %d in %s\n",
        assoc_id, (int) pdu->choice.initiatingMessage.procedureCode,
        ngap_direction2str(pdu->present));
    return -2;
  }
  OAILOG_ERROR(
      LOG_NGAP, "NGAP: [SCTP %d]  handler for procedureCode %d in %s\n",
      assoc_id, (int) pdu->choice.initiatingMessage.procedureCode,
      ngap_direction2str(pdu->present));
  ASSOC_ID = assoc_id;
  ...
  // return ngap_amf_handle_ng_setup_request(state, assoc_id, stream, pdu);
  // return ngap_amf_handle_initial_ue_message(state, assoc_id, stream, pdu);
}
...
int ngap_amf_handle_ng_setup_request(
  ...
  OAILOG_MESSAGE_FINISH((void*) context);  // ACL_TAG its failing here
  OAILOG_ERROR(LOG_NGAP, "######ACL_TAG: %s, %d  ", __func__, __LINE__);
...
OAILOG_ERROR( LOG_NGAP, "#####cobraranu :%02x %02x %02x ", buf_plmn[0], buf_plmn[1], buf_plmn[2]);

  OCTET_STRING_fromBuf(&plmnItem->pLMNIdentity, buf_plmn, sizeof(buf_plmn)/*3bytes*/);

OAILOG_ERROR( LOG_NGAP, "#####cobraranu :%02x %02x %02x ", plmnItem->pLMNIdentity.buf[0], plmnItem->pLMNIdentity.buf[1], plmnItem->pLMNIdentity.buf[2]);

  //memncpy(&plmnItem->pLMNIdentity, buf_plmn, sizeof(buf_plmn)/*3bytes*/);

/* Ngap_SliceSupportList */

  Ngap_SliceSupportItem_t *SliceItem = (Ngap_SliceSupportItem_t*) calloc(1, sizeof(Ngap_SliceSupportItem_t));

 char *from_buf = "0x11";

 OCTET_STRING_fromBuf(&SliceItem->s_NSSAI.sST, from_buf, 1);

  ASN_SEQUENCE_ADD(&plmnItem->sliceSupportList.list, SliceItem); //adding slice item to slice list

  ASN_SEQUENCE_ADD(&ie->value.choice.PLMNSupportList.list, plmnItem); //adding plmn item to plmn list
  ...
  OAILOG_ERROR(
      LOG_NGAP, "######ACL_TAG: %s, %d encode done ", __func__, __LINE__);
  ...
  if (enc_rval < 0) {
    OAILOG_ERROR(
        LOG_NGAP, "######ACL_TAG: %s, %d encode not done ", __func__, __LINE__);
   ...
   */
  //  OAILOG_ERROR(LOG_NGAP, "buffer:%s", buffer);
  //  OAILOG_ERROR(LOG_NGAP, "buffer:%p", buffer);
  // OAILOG_ERROR(LOG_NGAP, "len:%d", length);
  ...
  //  OAILOG_ERROR(LOG_NGAP, "buffer:%p", buffer);
  //  OAILOG_ERROR(LOG_NGAP, "len:%d", length);
  //  OAILOG_ERROR(LOG_NGAP, "b:%p", b);
  // OAILOG_ERROR(LOG_NGAP, "slen:%d", b->slen);
  // OAILOG_ERROR(LOG_NGAP, "mlen:%d", b->mlen);

  // ASSOC_ID = gnb_association->sctp_assoc_id; //ACL_TAG global assocID
  OAILOG_ERROR(
...
int ngap_amf_handle_ue_context_modification_response(
  ...
  Ngap_UEContextModificationResponse_t* container = NULL;
  m5g_ue_description_t* ue_ref_p                  = NULL;
  MessageDef* message_p                           = NULL;
  int rc                                          = RETURNok;
  imsi64_t imsi64                                 = INVALID_IMSI64;
  ...
  NGAP_FIND_PROTOCOLIE_BY_ID(
      Ngap_UEContextModificationResponseIEs_t, ie, container,
      Ngap_ProtocolIE_ID_id_AMF_UE_NGAP_ID, true);
  NGAP_FIND_PROTOCOLIE_BY_ID(
      Ngap_UEContextModificationResponseIEs_t, ie, container,
      Ngap_ProtocolIE_ID_id_RAN_UE_NGAP_ID, true);

  if ((ie) && (ue_ref_p = ngap_state_get_ue_amfid(
                   ie->value.choice.AMF_UE_NGAP_ID)) == NULL) {
    /*
     * AMF doesn't know the AMF UE NGAP ID provided.
     * No need to do anything. Ignore the message
     */
    OAILOG_DEBUG(
        ...
        " gnb_ue_ngap_id " GNB_UE_NGAP_ID_FMT " ",
        (uint32_t) ie->value.choice.AMF_UE_NGAP_ID,
        (uint32_t) ie->value.choice.RAN_UE_NGAP_ID);
    OAILOG_FUNC_RETURN(LOG_NGAP, RETURNerror);
  } else {
    if ((ie) && (ue_ref_p->gnb_ue_ngap_id ==
                 (ie->value.choice.RAN_UE_NGAP_ID & GNB_UE_NGAP_ID_MASK))) {
      /*
       * Both gNB UE NGAP ID and AMF UE NGAP ID match.
       * Send a UE context Release Command to gNB after releasing Ng-U bearer
      ...
      message_p = itti_alloc_new_message(
          TASK_NGAP, NGAP_UE_CONTEXT_MODIFICATION_RESPONSE);
      AssertFatal(message_p != NULL, "itti_alloc_new_message Failed");
      memset(
          (void*) &message_p->ittiMsg.ngap_ue_context_mod_response, 0,
          sizeof(itti_ngap_ue_context_mod_resp_t));
      NGAP_UE_CONTEXT_MODIFICATION_RESPONSE(message_p).amf_ue_ngap_id =
          ue_ref_p->amf_ue_ngap_id;
      NGAP_UE_CONTEXT_MODIFICATION_RESPONSE(message_p).gnb_ue_ngap_id =
          ue_ref_p->gnb_ue_ngap_id;
      ...
      OAILOG_DEBUG_UE(
          LOG_NGAP, imsi64,
          "NGAP_UE_CONTEXT_MODIFICATION_RESPONSE ignored, cause mismatch "
          "gnb_ue_ngap_id: ctxt" GNB_UE_NGAP_ID_FMT
          " != request " GNB_UE_NGAP_ID_FMT " ",
          (uint32_t) ue_ref_p->gnb_ue_ngap_id,
          (uint32_t) ie->value.choice.RAN_UE_NGAP_ID);
      OAILOG_FUNC_RETURN(LOG_NGAP, RETURNerror);
    }
  }
...
int ngap_amf_handle_ue_context_modification_failure(
  ...
  Ngap_UEContextModificationFailure_t* container = NULL;
  m5g_ue_description_t* ue_ref_p                 = NULL;
  MessageDef* message_p                          = NULL;
  int rc                                         = RETURNok;
  Ngap_Cause_PR cause_type                       = {0};
  int64_t cause_value;
  imsi64_t imsi64 = INVALID_IMSI64;
  ...
  NGAP_FIND_PROTOCOLIE_BY_ID(
      Ngap_UEContextModificationFailureIEs_t, ie, container,
      Ngap_ProtocolIE_ID_id_AMF_UE_NGAP_ID, true);
  NGAP_FIND_PROTOCOLIE_BY_ID(
      Ngap_UEContextModificationFailureIEs_t, ie, container,
      Ngap_ProtocolIE_ID_id_RAN_UE_NGAP_ID, true);
  ...
  if ((ie) && (ue_ref_p = ngap_state_get_ue_amfid(
    ...
    if ((ie) && (ue_ref_p->gnb_ue_ngap_id ==
      ...
      message_p = itti_alloc_new_message(
          TASK_NGAP, NGAP_UE_CONTEXT_MODIFICATION_FAILURE);
      AssertFatal(message_p != NULL, "itti_alloc_new_message Failed");
      memset(
          (void*) &message_p->ittiMsg.ngap_ue_context_mod_response, 0,
          sizeof(itti_ngap_ue_context_mod_resp_fail_t));
      NGAP_UE_CONTEXT_MODIFICATION_FAILURE(message_p).amf_ue_ngap_id =
          ue_ref_p->amf_ue_ngap_id;
      NGAP_UE_CONTEXT_MODIFICATION_FAILURE(message_p).gnb_ue_ngap_id =
          ue_ref_p->gnb_ue_ngap_id;
      NGAP_UE_CONTEXT_MODIFICATION_FAILURE(message_p).cause = cause_value;
      ...
      OAILOG_DEBUG_UE(
          LOG_NGAP, imsi64,
          "NGAP_UE_CONTEXT_MODIFICATION_FAILURE ignored, cause mismatch "
          "gnb_ue_ngap_id: ctxt " GNB_UE_NGAP_ID_FMT
          " != request " GNB_UE_NGAP_ID_FMT " ",
          (uint32_t) ue_ref_p->gnb_ue_ngap_id,
          (uint32_t) ie->value.choice.RAN_UE_NGAP_ID);
...
OAILOG_ERROR( LOG_NGAP, "####ACL_TAG ");
  ...
  hashtable_uint64_ts_get(
      imsi_map->amf_ue_id_imsi_htbl, (const hash_key_t) 1, &imsi64);
  ...
  NGAP_PDUSESSIONRESOURCE_SETUP_RSP(message_p).amf_ue_ngap_id = gnb_ue_ngap_id =
      1;
  NGAP_PDUSESSIONRESOURCE_SETUP_RSP(message_p).gnb_ue_ngap_id = amf_ue_ngap_id =
      1;

  NGAP_PDUSESSIONRESOURCE_SETUP_RSP(message_p)
      .pduSessionResource_setup_list.no_of_items = 0;
  NGAP_PDUSESSIONRESOURCE_SETUP_RSP(message_p)
      .pduSessionResourceFailedToSetupList.no_of_items = 0;  // ACL_TAG
  ...
  if (ie) {
    ...
    for (int index = 0; index < pduSessionResource; index++) {
      ...
      NGAP_PDUSESSIONRESOURCE_SETUP_RSP(message_p)
          .pduSessionResource_setup_list.item[index]
          .PDU_Session_Resource_Setup_Response_Transfer = blk2bstr(
...
int ngap_amf_handle_pduSession_release_response(
  ...
  NGAP_PDUSESSIONRESOURCE_REL_RSP(message_p)
      .pduSessionResourceReleasedRspList.no_of_items = 1;
  // NGAP_PDUSESSIONRESOURCE_REL_RSP(message_p).pduSessionResourceFailedToRelRspList.no_of_items
  // = 0;
  ...
  if (ie) {
    for (int index = 0; index < num_pduSessionResource; index++) {
      ...
      //      const Ngap_E_RABItem_t* const erab_item = (Ngap_E_RABItem_t*)
      //      &erab_item_ies->value.choice.E_RABItem;
      NGAP_PDUSESSIONRESOURCE_REL_RSP(message_p)
          .pduSessionResourceReleasedRspList.item[index]
          .Pdu_Session_ID = pduSessionResource_item->pDUSessionID;
      //      NGAP_E_RAB_REL_RSP(message_p).e_rab_rel_list.item[index].cause =
      //      erab_item->cause;
      NGAP_PDUSESSIONRESOURCE_REL_RSP(message_p)
          .pduSessionResourceReleasedRspList.no_of_items++;

      //      NGAP_PDUSESSIONRESOURCE_REL_RSP(message_p).pduSessionResourceReleasedRspList.item[index].pduSessionResourceRelRspList
      //      =
      //      blk2bstr(pduSessionResource_item->pDUSessionResourceReleaseResponseTransfer.buf,
      //      pduSessionResource_item->pDUSessionResourceReleaseResponseTransfer.size);//ACL_TAG
      //      need to fix
    }
  }
  message_p->ittiMsgHeader.imsi = imsi64;
