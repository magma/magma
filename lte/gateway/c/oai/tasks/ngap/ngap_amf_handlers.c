...
int ngap_amf_generate_ue_context_release_command(
    ngap_state_t* state, ue_description_t* ue_ref_p, enum Ngcause,
    imsi64_t imsi64);


/* Handlers matrix. Only amf related procedures present here.
 */
ngap_message_handler_t message_handlers[][3] = {
...
int ngap_amf_handle_ng_setup_request(
  ...
  supported_ta_list_t* supp_ta_list = &gnb_association->supported_ta_list;
...
//------------------------------------------------------------------------------
int ngap_amf_generate_ue_context_release_command(
    ngap_state_t* state, ue_description_t* ue_ref_p, enum Ngcause cause,
    imsi64_t imsi64) {
  uint8_t* buffer = NULL;
  uint32_t length = 0;
  Ngap_NGAP_PDU_t pdu;
...
void ngap_amf_handle_ue_context_rel_comp_timer_expiry(
    ngap_state_t* state, ue_description_t* ue_ref_p) {
  MessageDef* message_p = NULL;
  OAILOG_FUNC_IN(LOG_NGAP);
...
void ngap_amf_release_ue_context(
    ngap_state_t* state, ue_description_t* ue_ref_p, imsi64_t imsi64) {
  MessageDef* message_p = NULL;
  OAILOG_FUNC_IN(LOG_NGAP);
  DevAssert(ue_ref_p != NULL);
  OAILOG_DEBUG_UE(
