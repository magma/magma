  ...
  uint32_t len)
{
   ...
   */
  if (
    (decoded_result = decode_u8_detach_type(
       &detach_request->detachtype,
       0,
       *(buffer + decoded) & 0x0f,
       len - decoded)) < 0)
  ...
  if (
    (decoded_result = decode_u8_nas_key_set_identifier(
       &detach_request->naskeysetidentifier,
       0,
       *(buffer + decoded) >> 4,
       len - decoded)) < 0)
    return decoded_result;
  ...
  if (
    (decoded_result = decode_eps_mobile_identity(
       &detach_request->gutiorimsi, 0, buffer + decoded, len - decoded)) < 0)
    return decoded_result;
  else
  ...
  uint32_t len)
{
  ...
  *(buffer + encoded) = nw_detach_request->nw_detachtype;
  encoded++;
  if (
    (nw_detach_request->presenceMask & NW_DETACH_REQ_EMM_CAUSE_PRESENCE) ==
    NW_DETACH_REQ_EMM_CAUSE_PRESENCE) {
