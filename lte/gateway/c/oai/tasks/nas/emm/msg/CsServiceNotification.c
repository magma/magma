  ...
  uint32_t len)
{
   ...
   */
  if (
    (decoded_result = decode_paging_identity(
       &cs_service_notification->pagingidentity,
       0,
       buffer + decoded,
       len - decoded)) < 0)
  ...
  while (len - decoded > 0) {
    ...
    switch (ieiDecoded) {
      case CS_SERVICE_NOTIFICATION_CLI_IEI:
        if (
          (decoded_result = decode_cli(
             &cs_service_notification->cli,
      ...
      case CS_SERVICE_NOTIFICATION_LCS_INDICATOR_IEI:
        if (
          (decoded_result = decode_lcs_indicator(
      ...
      case CS_SERVICE_NOTIFICATION_LCS_CLIENT_IDENTITY_IEI:
        if (
          (decoded_result = decode_lcs_client_identity(
  ...
  uint32_t len)
{
  ...
  if (
    (encode_result = encode_paging_identity(
       &cs_service_notification->pagingidentity,
       0,
       buffer + encoded,
  ...
  if (
    ...
    CS_SERVICE_NOTIFICATION_LCS_INDICATOR_PRESENT) {
    if (
      (encode_result = encode_lcs_indicator(
         &cs_service_notification->lcsindicator,
         CS_SERVICE_NOTIFICATION_LCS_INDICATOR_IEI,
         buffer + encoded,
         len - encoded)) < 0)
  ...
  if (
    ...
    CS_SERVICE_NOTIFICATION_LCS_CLIENT_IDENTITY_PRESENT) {
    if (
      (encode_result = encode_lcs_client_identity(
         cs_service_notification->lcsclientidentity,
         CS_SERVICE_NOTIFICATION_LCS_CLIENT_IDENTITY_IEI,
         buffer + encoded,
         len - encoded)) < 0)
