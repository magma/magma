...
#define TIMER_S6A_AUTH_INFO_RSP_DEFAULT_VALUE                                  \
  3 // three second timeout value to wait for auth_info_rsp message from HSS
...
#define EMM_AUTHENTICATION_SYNC_FAILURE_MAX 2
  int
    auth_sync_fail_count; /* counter of successive AUTHENTICATION FAILURE messages from the UE with EMM cause #21 "synch failure" */
  ...
  nas_timer_t T3450; /* EMM message retransmission timer */
  nas_timer_t T3460; /* Authentication timer         */
  nas_timer_t T3470; /* Identification timer         */
  nas_timer_t T3422; /* Detach timer         */
  void *t3422_arg;
  nas_timer_t
    timer_s6a_auth_info_rsp; // MME<->HSS Authentication Information Rsp Timer over S6a interface
  void *timer_s6a_auth_info_rsp_arg;

  esm_data_context_t esm_data_ctx;

  UeNetworkCapability
    _ue_network_capability_ie; /* stored TAU Request IE Requirement MME24.301R10_5.5.3.2.4_2*/
  MsNetworkCapability
    _ms_network_capability_ie; /* stored TAU Request IE Requirement MME24.301R10_5.5.3.2.4_2*/
  DrxParameter
    _current_drx_parameter; /* stored TAU Request IE Requirement MME24.301R10_5.5.3.2.4_4*/
  DrxParameter
    _pending_drx_parameter; /* stored TAU Request IE Requirement MME24.301R10_5.5.3.2.4_4*/
...
typedef struct emm_data_s {
   ...
   */
  hash_table_ts_t
    *ctx_coll_ue_id; // key is emm ue id, data is struct emm_context_s
  hash_table_ts_t
...
typedef struct s6a_auth_info_rsp_timer_arg_s {
  mme_ue_s1ap_id_t ue_id; // UE identifier
  bool
    resync; // Indicates whether the authentication information is requested due to sync failure
} s6a_auth_info_rsp_timer_arg_t;

mme_ue_s1ap_id_t emm_ctx_get_new_ue_id(emm_context_t *ctxt)
  __attribute__((nonnull));

void emm_ctx_mark_common_procedure_running(
...
void emm_ctx_unmark_common_procedure_running(
  ...
  const int attribute_bit_pos) __attribute__((nonnull))
__attribute__((flatten));
bool emm_ctx_is_common_procedure_running(
  emm_context_t *const ctxt,
  const int proc_id) __attribute__((nonnull)) __attribute__((flatten));

void emm_ctx_mark_specific_procedure(
  emm_context_t *const ctxt,
  const int attribute_bit_pos) __attribute__((nonnull))
__attribute__((flatten));
void emm_ctx_unmark_specific_procedure(
  emm_context_t *const ctxt,
  const int attribute_bit_pos) __attribute__((nonnull))
__attribute__((flatten));
bool emm_ctx_is_specific_procedure(emm_context_t *const ctxt, const int proc_id)
...
void emm_ctx_clear_ue_nw_cap_ie(emm_context_t *const ctxt)
  __attribute__((nonnull));
void emm_ctx_set_ue_nw_cap_ie(
  emm_context_t *const ctxt,
  UeNetworkCapability *ue_nw_cap_ie) __attribute__((nonnull));
void emm_ctx_set_valid_ue_nw_cap_ie(
  emm_context_t *const ctxt,
  UeNetworkCapability *ue_nw_cap_ie) __attribute__((nonnull));
...
void emm_ctx_clear_current_drx_parameter(emm_context_t *const ctxt)
  __attribute__((nonnull));
void emm_ctx_set_current_drx_parameter(
  emm_context_t *const ctxt,
  DrxParameter *drx) __attribute__((nonnull));
void emm_ctx_set_valid_current_drx_parameter(
  emm_context_t *const ctxt,
  DrxParameter *drx) __attribute__((nonnull));
...
void emm_ctx_clear_pending_current_drx_parameter(emm_context_t *const ctxt)
  __attribute__((nonnull));
void emm_ctx_set_pending_current_drx_parameter(
  emm_context_t *const ctxt,
  DrxParameter *drx) __attribute__((nonnull));
void emm_ctx_set_valid_pending_current_drx_parameter(
  emm_context_t *const ctxt,
  DrxParameter *drx) __attribute__((nonnull));

void emm_ctx_clear_eps_bearer_context_status(emm_context_t *const ctxt)
  __attribute__((nonnull));
void emm_ctx_set_eps_bearer_context_status(
  emm_context_t *const ctxt,
  EpsBearerContextStatus *status) __attribute__((nonnull));
void emm_ctx_set_valid_eps_bearer_context_status(
  emm_context_t *const ctxt,
  EpsBearerContextStatus *status) __attribute__((nonnull));

struct emm_context_s *emm_data_context_get(
  emm_data_t *emm_data,
  const mme_ue_s1ap_id_t ueid) __attribute__((nonnull));
...
struct emm_context_s *emm_data_context_get_by_guti(
  emm_data_t *emm_data,
  guti_t *guti) __attribute__((nonnull));

struct emm_context_s *emm_data_context_remove(
  emm_data_t *_emm_data,
  struct emm_context_s *elm) __attribute__((nonnull));

void emm_data_context_remove_mobile_ids(
  emm_data_t *_emm_data,
  struct emm_context_s *elm) __attribute__((nonnull));

int emm_data_context_add(emm_data_t *emm_data, struct emm_context_s *elm)
  __attribute__((nonnull));
int emm_data_context_add_guti(emm_data_t *emm_data, struct emm_context_s *elm)
  __attribute__((nonnull));
int emm_data_context_add_old_guti(
  emm_data_t *emm_data,
  struct emm_context_s *elm) __attribute__((nonnull));
int emm_data_context_add_imsi(emm_data_t *emm_data, struct emm_context_s *elm)
  __attribute__((nonnull));
int emm_data_context_upsert_imsi(
  emm_data_t *emm_data,
  struct emm_context_s *elm) __attribute__((nonnull));

void emm_context_silently_reset_procedures(struct emm_context_s *emm_ctx)
  __attribute__((nonnull));
void emm_context_stop_all_timers(struct emm_context_s *emm_ctx)
  __attribute__((nonnull));
void free_emm_data_context(struct emm_context_s *const emm_ctx)
  __attribute__((nonnull));
void emm_data_context_dump(const struct emm_context_s *const elm_pP)
  __attribute__((nonnull));
