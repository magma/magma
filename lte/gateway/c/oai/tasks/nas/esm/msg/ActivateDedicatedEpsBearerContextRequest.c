  ...
  uint32_t len)
{
  ...
  CHECK_PDU_POINTER_AND_LENGTH_DECODER(
    buffer, ACTIVATE_DEDICATED_EPS_BEARER_CONTEXT_REQUEST_MINIMUM_LENGTH, len);
  ...
  if (
    (decoded_result = decode_u8_linked_eps_bearer_identity(
       &activate_dedicated_eps_bearer_context_request->linkedepsbeareridentity,
       0,
       *(buffer + decoded),
       len - decoded)) < 0)
  ...
  if (
    (decoded_result = decode_eps_quality_of_service(
       &activate_dedicated_eps_bearer_context_request->epsqos,
       0,
       buffer + decoded,
       len - decoded)) < 0)
    return decoded_result;
  ...
  if (
    ...
    if (
      (encode_result = encode_linked_ti_ie(
         &activate_dedicated_eps_bearer_context_request->transactionidentifier,
  ...
  if (
    ...
    if (
      (encode_result = encode_packet_flow_identifier_ie(
         &activate_dedicated_eps_bearer_context_request->packetflowidentifier,
         true,
         buffer + encoded,
         len - encoded)) < 0)
      // Return in case of error
