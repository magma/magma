  ...
  esm_proc_pdn_request_t request_typeP)
{
  ...
  OAILOG_INFO(
    LOG_NAS_EMM,
    "Sending PDN CONFIG REQ to MME_APP , (ue_idP = %u) \n",
    ue_idP);
  ...
  const_bstring const auts_pP)
{
  ...
  itti_send_msg_to_task(TASK_S6A, message_p);
  ...
  const uint8_t service_type)
{
  ...
  if (ue_mm_context) {
    ...
    OAILOG_INFO(
      LOG_NAS_EMM,
      "Sending NAS Connection Establishment confirm for "
...
void nas_itti_sgs_detach_req(const uint32_t ue_idP, const uint8_t detach_type)
{
  ...
  OAILOG_INFO(
    LOG_MME_APP, "Send SGS Detach Request to MME for ue_id = %u\n", ue_idP);
