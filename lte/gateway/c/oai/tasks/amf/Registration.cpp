...
int amf_registration_procedure::amf_proc_registration_request(
  ...
  if (ies->imsi) {
    ...
    OAILOG_INFO(LOG_AMF_APP,"During initial registration request "
		    "SUPI as IMSI converted to imsi64 " IMSI_64_FMT " = ", imsi64);
  } else if (ies->guti) {
...
int amf_registration_procedure::amf_send_registration_accept(
  ...
  if (amf_context) {
    ...
    if (registration_proc) {
      ...
      OAILOG_INFO(LOG_NAS_AMF," AMF_TEST in %s assigned GUTI to amf_sap from amf_context\n", __FUNCTION__);
      OAILOG_INFO(LOG_NAS_AMF," AMF_TEST Value of TMSI of GUTI %08" PRIx32 "\n",
