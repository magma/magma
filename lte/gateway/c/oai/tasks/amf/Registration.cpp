...
namespace magma5g
{
     ...
     static int amf_registration_failure_identification_cb(amf_context_t *amf_context);
     static int amf_start_registration_proc_security(amf_context_t *amf_context, 
    ...
    static int amf_start_registration_proc_authentication(amf_context_t *amf_context, nas_amf_registration_proc_t *registration_proc)
    {
        ...
        {
           //rc = m5g_auth.amf_proc_authentication(amf_context,&registration_proc->amf_spec_proc,
           //     amf_registration_success_authentication_cb, amf_registration_failure_authentication_cb);
        }
        //amf_registration_success_authentication_cb(amf_context);
        OAILOG_FUNC_RETURN(LOG_NAS_AMF, rc);
    }

        int amf_registration_procedure::amf_proc_registration_request(amf_ue_ngap_id_t ue_id, const bool is_mm_ctx_new,
        ...
        */
        ...
        rc = amf_registration_procedure::amf_registration_run_procedure(&ue_ctx.amf_context);
        OAILOG_FUNC_RETURN(LOG_NAS_AMF, rc);
...
//------------------------------------------------------------------------------
int amf_registration_procedure::amf_proc_registration_reject(amf_ue_ngap_id_t ue_id, amf_cause_t amf_cause) 
{

  OAILOG_FUNC_IN(LOG_NAS_AMF);
  int rc = RETURNerror;
  amf_context_t* amf_ctx ;//= amf_context_get(&amf_data, ue_id);//TODO -  NEED-RECHECK need to declare the function
      nas_amf_registration_proc_t* registration_proc =
          (nas_amf_registration_proc_t*) (amf_ctx->amf_procedures->amf_specific_proc);
      registration_proc->amf_cause = amf_cause;
  if (amf_ctx) {
    if ( nas_procedure_reg.is_nas_specific_procedure_registration_running(amf_ctx)) {

      // TODO could be in callback of attach procedure triggered by
      // AMF REG__REJ
      rc = amf_registration_procedure::amf_registration_reject(amf_ctx, registration_proc);
      amf_sap_t amf_sap;//               = {0};
...
int amf_registration_procedure::amf_registration_reject(amf_context_t* amf_context, nas_amf_registration_proc_t* nas_base_proc) 
{
  OAILOG_FUNC_IN(LOG_NAS_AMF);
  int rc = RETURNerror;
  ...
  OAILOG_WARNING(LOG_AMF_APP, "AMF-PROC  - AMF Registration procedure not accepted ");
  ...
  if (amf_context) {//TODO -  NEED-RECHECK
    amf_data_sec.amf_as_set_security_data( &amf_sap.u.amf_as.u.establish.sctx, 
                &amf_context->_security, false,  false);
  } else {
    amf_data_sec.amf_as_set_security_data(&amf_sap.u.amf_as.u.establish.sctx,
                NULL, false, false);
  }
  ...
  OAILOG_FUNC_RETURN(LOG_NAS_AMF, rc);
    ...
    */
    ...
    //------------------------------------------------------------------------------
    int amf_registration_procedure::amf_registration_run_procedure(amf_context_t *amf_context)
    {
        OAILOG_FUNC_IN(LOG_NAS_AMF);
        int rc = RETURNerror;
        nas_amf_registration_proc_t *registration_proc = nas_procedure_reg.get_nas_specific_procedure_registration(amf_context);
           ...
           // }
            ...
            if (
               ...
               //TODO amf_context_upsert_imsi(&_amf_data, amf_context);
                ...
                rc = amf_proc.amf_proc_identification(amf_context,(nas_amf_proc_t *) registration_proc, 
                     IDENTITY_TYPE_2_IMSI,amf_registration_procedure::amf_registration_success_identification_cb,
                     amf_registration_failure_identification_cb);
              }
            } 
            else if (registration_proc->ies->guti) 
            {
            rc = amf_proc.amf_proc_identification(amf_context,
                     (nas_amf_proc_t *) registration_proc, IDENTITY_TYPE_2_IMSI,
    ...
    static int amf_start_registration_proc_security(amf_context_t *amf_context, nas_amf_registration_proc_t *registration_proc)
    {
        ...
        if ((amf_context) && (registration_proc)) 
        {
            ...
            */
            //amf_ctx_clear_security(amf_context);
            //rc = amf_proc_security_mode_control(amf_context, 
            //     &registration_proc->amf_spec_proc,registration_proc->ksi,
            //     _amf_registration_success_security_cb,_amf_registration_failure_security_cb);
 ...
 */
            ...
            */
            amf_sap.u.amf_as.u.establish.nas_msg = registration_proc->amf_msg_out;
            OAILOG_TRACE(LOG_NAS_AMF,"ue_id=" AMF_UE_NGAP_ID_FMT 
    ...
    **                                                                        **
    ***************************************************************************/
    int amf_registration_procedure:: amf_send_registration_accept_dl_nas(const amf_as_data_t* msg, 
                                            RegistrationAcceptMsg *amf_msg) {
    OAILOG_FUNC_IN(LOG_NAS_AMF);
    int size = AMF_HEADER_MAXIMUM_LENGTH;

    // Get the UE context
    amf_context_t* amf_ctx;// = amf_context_get(&_amf_data, msg->ue_id);//TODO -  NEED-RECHECK
    //DevAssert(amf_ctx);
...
int amf_registration_procedure::amf_proc_registration_complete(
  ...
  int rc                             = RETURNerror;
  amf_sap_t amf_sap; 
  //smf_sap_t smf_sap ; //TODO -  NEED-RECHECK as PDU ses req comes in different mesg 
  ...
  if (ue_amf_context) {
    if (nas_procedure_reg.is_nas_specific_procedure_registration_running(&ue_amf_context->amf_context)) 
     {
      registration_proc =
      ...
      /*
       * Upon receiving an REGISTRATION COMPLETE message, the AMF shall enter state
       * AMF-REGISTERED and consider the GUTI sent in the REGISTRATION ACCEPT message
       * as valid.
       */
      //amf_ctx_set_attribute_valid(amf_ctx, AMF_CTXT_MEMBER_GUTI);//TODO-RECHECK
  ...
  if ((rc != RETURNerror))
  {
    ...
    if (rc == RETURNok) {
      ...
      //amf_proc_amf_informtion(ue_amf_context); //TODO-RECHECK to be implement
      increment_counter("ue_registration", 1, 1, "result", "registration_proc_successful");
    ...
    //OAILOG_WARNING(
    //    LOG_NAS_AMF,
    //    "Ignore SMF procedure failure/received message has been discarded for "
    //    "ue_id = (%u)\n",
    //    ue_id);
    rc = RETURNok;
  }
