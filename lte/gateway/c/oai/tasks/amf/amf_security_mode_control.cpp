  ...
  for (preference_index = 0; preference_index < 8; preference_index++) {
    if (ue_eiaP &
      ...
      OAILOG_DEBUG(
          ...
          amf_data.prefered_integrity_algorithm[preference_index], preference_index);
  ...
  for (preference_index = 0; preference_index < 8; preference_index++) {
    if (ue_eeaP &
      ...
      OAILOG_DEBUG(
          ...
          amf_data.prefered_ciphering_algorithm[preference_index], preference_index);
...
static int amf_security_ll_failure(
  ...
  int rc = RETURNerror;
  if (nas_amf_proc) {
    //nas_amf_smc_proc_t* smc_proc = (nas_amf_smc_proc_t*) nas_amf_proc;//TODO-RECHECK
    
    amf_sap_t amf_sap_seq ;
    amf_ue_ngap_id_t ue_id =
        PARENT_STRUCT(amf_context, ue_m5gmm_context_s, amf_context)->amf_ue_ngap_id;
...
static int amf_security_request(nas_amf_smc_proc_t* const smc_proc) {
  ...
  if (smc_proc) {
    ...
    if (ue_mm_context) {
      ...
      OAILOG_ERROR( LOG_NAS_AMF, "UE 5G-MM Context NULL! for ue_id = (%u)\n", smc_proc->ue_id);
  ...
  OAILOG_FUNC_RETURN(LOG_NAS_AMF, rc);
    ...
    ***************************************************************************/
    int amf_proc_security_mode_control( amf_context_t* amf_ctx,
        nas_amf_specific_proc_t* amf_specific_proc, ksi_t ksi,
    ...
    */
    ...
    OAILOG_INFO(LOG_NAS_AMF,"AMF-PROC  - Initiate security mode control procedure, " "KSI = %d\n", ksi);
    ...
    amf_ue_ngap_id_t ue_id = PARENT_STRUCT(amf_ctx, ue_m5gmm_context_s, amf_context)->amf_ue_ngap_id;
    nas_amf_smc_proc_t* smc_proc;// = get_nas5g_common_procedure_smc(amf_ctx);//sandeep
    if (!smc_proc) {
        //smc_proc = nas5g_new_smc_procedure(amf_ctx);//sandeep
        if (smc_proc) {
            smc_proc->saved_selected_eea = amf_ctx->_security.selected_algorithms.encryption;
            smc_proc->saved_selected_eia = amf_ctx->_security.selected_algorithms.integrity;
...
// need to wright m5g_ue_network_capability class in 24.501 Sandeep
 ...
 // need to wright m5g_ue_network_capability class in 24.501 Sandeep
        ...
        if (rc == RETURNerror) {
            //OAILOG_WARNING(
            //    LOG_NAS_AMF, "AMF-PROC  - Failed to select security algorithms\n");
            OAILOG_FUNC_RETURN(LOG_NAS_AMF, RETURNerror);
        }

        smc_ctrl.amf_ctx_set_security_type(amf_ctx, SECURITY_CTX_TYPE_FULL_NATIVE);
        // TODO AssertFatal(KSI_NO_KEY_AVAILABLE > amf_ctx->_security.eksi, "eksi not valid");
        derive_key_nas(
    ...
    if (smc_proc) {
        ...
        */
        //((nas5g_base_proc_t*) smc_proc)->parent =    (nas_base_proc_t*) amf_specific_proc;
        ...
        //smc_proc->amf_com_proc.amf_proc.not_delivered_ho = _security_non_delivered_ho;
        ...
        //smc_proc->amf_com_proc.amf_proc.base_proc.abort         = _security_abort;
        ...
        //smc_proc->amf_com_proc.amf_proc.base_proc.time_out =   _security_t3560_handler;
        ...
        */
        ...
        // smc_proc->imeisv_request = (IS_EMM_CTXT_PRESENT_IMEISV(amf_ctx)) ?
        // false:true;

        /*
        * Send security mode command message to the UE
        */
        //rc = amf_security_request(smc_proc); //TODO-RECHECK
