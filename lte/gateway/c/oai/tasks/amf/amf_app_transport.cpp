...
int amf_app_defs::amf_app_handle_nas_dl_req(
  ...
  // NGAP_NAS_DL_DATA_REQ(message_p).nas_msg->slen += 3;//TODO fix length in all
  // other DownlinkNASTransport

  OAILOG_ERROR(
      LOG_AMF_APP, "##########cobraranu nas to send DL:slen:%d \n",
      NGAP_NAS_DL_DATA_REQ(message_p).nas_msg->slen);

  int i;
  for (i = 0; i < NGAP_NAS_DL_DATA_REQ(message_p).nas_msg->slen; i++) {
    OAILOG_ERROR(
        LOG_NGAP, "%02x ",
        ((unsigned char*) NGAP_NAS_DL_DATA_REQ(message_p).nas_msg->data)[i]);
  }
