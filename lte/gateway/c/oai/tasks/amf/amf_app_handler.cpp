    ...
    const guti_m5_t* const guti_p)  //  never NULL, if none put &ue_context_p->guti
{
  ...
  //hash_table_ts_t* amf_state_ue_id_ht = get_amf_ue_state();//TODO -  NEED-RECHECK
  hash_table_ts_t* amf_state_ue_id_ht; //= get_amf_ue_state();//TODO -  NEED-RECHECK as it is used in function
  ...
  //    ue_context_p, gnb_ngap_id_key, amf_ue_ngap_id, imsi, GUTI_ARG_M5G(guti_p));
  ...
  if ((INVALID_GNB_UE_NGAP_ID_KEY != gnb_ngap_id_key) &&
    ...
    //    "gnb_ue_ngap_ue_id amf_ue_ngap_id " GENB_UE_NGAP_ID_FMT, AMF_UE_NGAP_ID_FMT "\n",
  ...
  if (guti_p) {
    if ((guti_p->guamfi.amf_code !=
      ...
      if (HASH_TABLE_OK != h_rc) {
        ...
        //    ue_context_p, ue_context_p->gnb_ue_ngap_id, ue_context_p->amf_ue_ngap_id, GUTI_ARG_M5G(guti_p),
        //    hashtable_rc_code2string(h_rc));
      }
...
//----------------------------------------------------------------------------------------------
    ...
    ue_m5gmm_context_s* amf_ue_context_exists_guti(amf_ue_context_t* const amf_ue_context_p, const guti_m5_t* const guti_p) 
    {
        ...
        if (HASH_TABLE_OK == h_rc) {
            ...
            //    (amf_ue_ngap_id_t) amf_ue_ngap_id64);     // not finding in include dir but in old_include
        } else {
...
//-----------------------------------------------------------------------------------------
        ...
        if (is_guti_valid) 
        {
            ...
            if (ue_context_p) {
                ...
                */
                ...
                //ue_context_p->ue_context_rel_cause = NGAP_INVALID_GNB_ID; //TODO -  NEED-RECHECK
               // OAILOG_ERROR(LOG_AMF_APP," Sending UE Context Release to NGAP for ue_id =(%u)\n",
               //     ue_context_p->amf_ue_ngap_id);
                amf_app_message.amf_app_ue_context_release(ue_context_p, ue_context_p->ue_context_rel_cause);
                ...
                //ue_context_p->ue_context_rel_cause = NGAP_INVALID_CAUSE;//TODO -  NEED-RECHECK
                }
  ...
  if (!(ue_context_p)) {
    ...
    if (!(ue_context_p=amf_create_new_ue_context()));
        ...
        amf_app_ue_context::notify_ngap_new_ue_amf_ngap_id_association(ue_context_p);
        ...
        nas_procedure.nas_proc_establish_ind(ue_context_p->amf_ue_ngap_id, is_mm_ctx_new, 
  ...
  int amf_app_defs::amf_app_handle_uplink_nas_message(amf_app_desc_t* amf_app_desc_p, std::string* msg)
  {
    OAILOG_FUNC_IN(LOG_NAS_AMF);
    int rc = RETURNerror;
    //OAILOG_INFO(LOG_NAS, "Received NAS UPLINK DATA IND from NGAP for ue_id = (%u)\n", 
    ...
    if (msg) {
      ...
      */
      ...
      amf_sap.u.amf_as.u.data.ue_id     = amf_app_desc_p->amf_app_ue_ngap_id_generator;
      amf_sap.u.amf_as.u.data.delivered = true;
      amf_sap.u.amf_as.u.data.nas_msg   = *msg;
      msg                              = NULL;
      
      rc   = amf_sap_handler.amf_sap_send(&amf_sap);
    } else {
      OAILOG_WARNING( LOG_NAS, "Received NAS message in uplink is NULL for ue_id = (%u)\n",
                   amf_app_desc_p->amf_app_ue_ngap_id_generator);
    }
