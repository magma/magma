...
static uint32_t _nas5g_message_get_mac(
    const unsigned char* const buffer, uint32_t const length, int const direction,
    amf_security_context_t* const amf_security_context);
...
int nas5g_message_encrypt(
    ...
    const amf_msg_header* header, uint32_t length,
    void* security) {
   ...
   * Encode the header
   */
  int size = _nas5g_message_header_encode(outbuf, header, length);
  ...
  if (size < 0) {
    ...
    bytes = _nas5g_message_encrypt(
        ...
        amf_security_context->direction_encode, length - size,
  ...
  if (SECU_DIRECTION_DOWNLINK == amf_security_context->direction_encode) {
    ...
    OAILOG_DEBUG(
        LOG_AMF_APP, "Incremented amf_security_context.dl_count.seq_num -> %u\n",
    ...
    OAILOG_DEBUG(
        LOG_AMF_APP, "Incremented amf_security_context.ul_count.seq_num -> %u\n",
...
int nas5g_message_decrypt(
   ...
   * Decode the header
   */
  int size = nas5g_message_header_decode(inbuf, header, length, status, &is_sr);

  if (size < 0) {
    OAILOG_DEBUG(LOG_AMF_APP, "MESSAGE TOO SHORT\n");
    OAILOG_FUNC_RETURN(LOG_AMF_APP, TLV_BUFFER_TOO_SHORT);
  } else if (size > 1) {
    if (amf_security_context) {
      status->security_context_available = 1;

      if (amf_security_context->ul_count.seq_num > header->sequence_number) {
        amf_security_context->ul_count.overflow += 1;
      }

      amf_security_context->ul_count.seq_num = header->sequence_number;
    }
    ...
    if (mac == header->message_authentication_code) {
      ...
      OAILOG_DEBUG(LOG_AMF_APP, "Integrity: MAC Success\n");
    } else {
      OAILOG_CRITICAL(
          LOG_AMF_APP,
          ...
          (amf_security_context) ? amf_security_context->sc_type : 0);
      // LG: Do not return now (out of spec but we need that with only one MME)
      // LOG_FUNC_RETURN (LOG_AMF_APP, TLV_MAC_MISMATCH);
    }
    ...
    header->extended_protocol_discriminator = _nas5g_message_decrypt(
        ...
        header->sequence_number, length - size, amf_security_context, status);
    // OAI_GCC_DIAG_ON(discarded-qualifiers);

    bytes = length - size;
  } else {
    OAILOG_DEBUG(LOG_AMF_APP, "Plain NAS message found\n");
    /*
...
int nas5g_message_decode(
   ...
   */
  //OAILOG_STREAM_HEX(
  //    OAILOG_LEVEL_DEBUG, LOG_AMF_APP, "Incoming NAS message: ", buffer, length);
  if (amf_security_context) {
    status->security_context_available = 1;
  }
  size =
  ...
  OAILOG_DEBUG(LOG_AMF_APP, "nas5g_message_header_decode returned size %d\n", size);
  ...
  if i(is_sr) {
    if (length < NAS_MESSAGE_SERVICE_REQUEST_SECURITY_HEADER_SIZE) {
       ...
       */
      OAILOG_WARNING(
          LOG_AMF_APP, "Message header %lu bytes is too short %u bytes\n", length,
          NAS5G_MESSAGE_SECURITY_HEADER_SIZE);
    ...
    // shortcut
    msg->plain.amf.header.message_type = SERVICE_REQUEST;
    msg->plain.amf.service_request.ksiandsequencenumber.ksi =
        sequence_number >> 5;
    msg->plain.amf.service_request.ksiandsequencenumber.sequencenumber =
        sequence_number & 0x1F;
    msg->plain.amf.service_request.messageauthenticationcode = short_mac;
    msg->plain.amf.service_request.protocoldiscriminator =
        M5GS_MOBILITY_MANAGEMENT_MESSAGE;
    msg->plain.amf.service_request.securityheadertype =
        SECURITY_HEADER_TYPE_SERVICE_REQUEST;
    msg->plain.amf.service_request.messagetype = SERVICE_REQUEST;

    if (amf_security_context == NULL) {
      /*
    ...
    temp_sequence_number = (amf_security_context->ul_count.seq_num & 0xE0) >> 5;
    if ((amf_security_context->ul_count.seq_num & 0x1F) > sequence_number) {
      temp_sequence_number += 1;
    }
    sequence_number =
        ((temp_sequence_number & 0x07) << 5) | (sequence_number & 0x1F);

    if (amf_security_context->ul_count.seq_num > sequence_number) {
      amf_security_context->ul_count.overflow += 1;
    }
    amf_security_context->ul_count.seq_num = sequence_number;
    ...
    mac = _nas5g_message_get_mac(
        buffer, SR_MAC_SIZE_BYTES, SECU_DIRECTION_UPLINK, amf_security_context);
    ...
    if (short_mac == msg->plain.amf.service_request.messageauthenticationcode) {
      status->mac_matched = 1;
      OAILOG_DEBUG(
          LOG_AMF_APP, "Service Request: message MAC = %04X == computed = %04X\n",
          msg->plain.amf.service_request.messageauthenticationcode, short_mac);
    } else {
      OAILOG_DEBUG(
          LOG_AMF_APP, "Service Request: message MAC = %04X != computed = %04X\n",
...
int nas5g_message_header_decode(
    ...
    amf_msg_header* const header, const uint32_t length,
    amf_nas_message_decode_status_t* const status, bool* const is_sr) {
  OAILOG_FUNC_IN(LOG_AMF_APP);
  int size = 0;
  ...
  *is_sr = false;
  if (header->extended_protocol_discriminator == M5GS_MOBILITY_MANAGEMENT_MESSAGE) {
...
static int _nas5g_message_plain_decode(
  ...
  if (header->extended_protocol_discriminator == M5GS_MOBILITY_MANAGEMENT_MESSAGE) {
     ...
     */
    bytes = amf_msg_test.M5gNasMessageDecodeMsg((AmfMsg*) &msg->amf, (uint8_t*) buffer, length);
  } else {
    /*
...
int _nas5g_message_plain_encode(
  ...
  if (M5GS_MOBILITY_MANAGEMENT_MESSAGE ==
     ...
     */
    bytes = amf_msg_test.M5gNasMessageEncodeMsg((AmfMsg*) &msg->amf, (uint8_t*) buffer, length);
  } else {
    /*
     * Discard L3 messages with not supported protocol discriminator
     */
...
static int _nas5g_message_protected_encode(
  ...
  if (plain_msg) {
     ...
     */
    int size =
        _nas5g_message_plain_encode(plain_msg, &msg->header, &msg->plain, length);
...
static int _nas5g_message_decrypt(
  ...
  switch (security_header_type) {
    ...
    case SECURITY_HEADER_TYPE_INTEGRITY_PROTECTED_NEW:
      ...
      // LOG_FUNC_RETURN (LOG_AMF_APP, length);
      break;
    ...
    case SECURITY_HEADER_TYPE_INTEGRITY_PROTECTED_CYPHERED_NEW:
      if (amf_security_context) {
        ...
        switch (amf_security_context->selected_algorithms.encryption) {
#if 0
          case M5G_NAS_SECURITY_ALGORITHMS_5G_EA1: {
            if (0 == status->mac_matched) {
              OAILOG_ERROR(LOG_AMF_APP, "MAC integrity failed\n");
              OAILOG_FUNC_RETURN(LOG_AMF_APP, 0);
            }
            if (direction == SECU_DIRECTION_UPLINK) {
              count = 0x00000000 |
                      ((amf_security_context->ul_count.overflow & 0x0000FFFF)
                       << 8) |
                      (amf_security_context->ul_count.seq_num & 0x000000FF);
            } else {
              count = 0x00000000 |
                      ((amf_security_context->dl_count.overflow & 0x0000FFFF)
                       << 8) |
            ...
            OAILOG_DEBUG(
                LOG_AMF_APP,
                "M5G_NAS_SECURITY_ALGORITHMS_5G_EA1 dir %s count.seq_num %u count "
                "%u\n",
                ...
                (direction == SECU_DIRECTION_UPLINK) ?
                    ...
                    amf_security_context->dl_count.seq_num,
                count);
            stream_cipher.key        = amf_security_context->knas_enc;
          ...
          case M5G_NAS_SECURITY_ALGORITHMS_5G_EA2: {
            if (0 == status->mac_matched) {
              OAILOG_ERROR(LOG_AMF_APP, "MAC integrity failed\n");
              OAILOG_FUNC_RETURN(LOG_AMF_APP, 0);
            }
            ...
            OAILOG_DEBUG(
                LOG_AMF_APP,
                "M5G_NAS_SECURITY_ALGORITHMS_5G_EA2 dir %s count.seq_num %u count "
                "%u\n",
                (direction == SECU_DIRECTION_UPLINK) ? "UPLINK" : "DOWNLINK",
                (direction == SECU_DIRECTION_UPLINK) ?
                    amf_security_context->ul_count.seq_num :
          ...
          default:
            OAILOG_ERROR(
                LOG_AMF_APP, "Unknown Cyphering protection algorithm %d\n",
    ...
    case SECURITY_HEADER_TYPE_INTEGRITY_PROTECTED_CYPHERED_NEW:
      switch (amf_security_context->selected_algorithms.encryption) {
        ...
        case M5G_NAS_SECURITY_ALGORITHMS_5G_EA1: {
          ...
          OAILOG_DEBUG(
              LOG_AMF_APP,
              "M5G_NAS_SECURITY_ALGORITHMS_5G_EA1 dir %s count.seq_num %u count %u\n",
              (direction == SECU_DIRECTION_UPLINK) ? "UPLINK" : "DOWNLINK",
              (direction == SECU_DIRECTION_UPLINK) ?
                  amf_security_context->ul_count.seq_num :
                  amf_security_context->dl_count.seq_num,
        ...
        case M5G_NAS_SECURITY_ALGORITHMS_5G_EA2: {
          ...
          OAILOG_DEBUG(
              LOG_AMF_APP,
              "M5G_NAS_SECURITY_ALGORITHMS_5G_EA2 dir %s count.seq_num %u count %u\n",
              (direction == SECU_DIRECTION_UPLINK) ? "UPLINK" : "DOWNLINK",
              (direction == SECU_DIRECTION_UPLINK) ?
                  amf_security_context->ul_count.seq_num :
                  amf_security_context->dl_count.seq_num,
...
static uint32_t _nas5g_message_get_mac(
  ...
  switch (amf_security_context->selected_algorithms.integrity) {
    ...
    case M5G_NAS_SECURITY_ALGORITHMS_5G_EA1: {
      ...
      OAILOG_DEBUG(
          LOG_AMF_APP,
          "M5G_NAS_SECURITY_ALGORITHMS_5G_EA1 returned MAC %x.%x.%x.%x(%u) for "
          "length "
    ...
    case M5G_NAS_SECURITY_ALGORITHMS_5G_EA2: {
      ...
      OAILOG_DEBUG(
          LOG_AMF_APP,
          "M5G_NAS_SECURITY_ALGORITHMS_5G_EA2 returned MAC %x.%x.%x.%x(%u) for "
          "length "
          "%lu direction %d, count %d\n",
          mac[0], mac[1], mac[2], mac[3], *((uint32_t*) &mac), length,
          direction, count);
    ...
    case M5G_NAS_SECURITY_ALGORITHMS_5G_EA0:
      OAILOG_DEBUG(
          LOG_AMF_APP, "M5G_NAS_SECURITY_ALGORITHMS_5G_EA0 dir %s count.seq_num %u\n",
          (direction == SECU_DIRECTION_UPLINK) ? "UPLINK" : "DOWNLINK",
          (direction == SECU_DIRECTION_UPLINK) ?
              amf_security_context->ul_count.seq_num :
              amf_security_context->dl_count.seq_num);
    ...
    default:
      OAILOG_ERROR(
          LOG_AMF_APP, "Unknown integrity protection algorithm %d\n",
