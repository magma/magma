...
int nas5g_message_decrypt(
  ...
  if (size < 0) {
    OAILOG_INFO(LOG_AMF_APP, "MESSAGE TOO SHORT\n");
    OAILOG_FUNC_RETURN(LOG_AMF_APP, TLV_BUFFER_TOO_SHORT);
  } else if (size > 1) {
    ...
    if (mac == header->message_authentication_code) {
      ...
      OAILOG_INFO(LOG_AMF_APP, "Integrity: MAC Success\n");
    } else {
      OAILOG_CRITICAL(
          LOG_AMF_APP,
    ...
    OAILOG_INFO(
        LOG_AMF_APP,
        "AMF_TEST: Decrypt the security protected NAS message starts");
    ...
    OAILOG_INFO(
        LOG_AMF_APP,
        "AMF_TEST: Decrypt the security protected NAS message completed");

    bytes = length - size;
  } else {
    OAILOG_INFO(LOG_AMF_APP, "Plain NAS message found\n");
    /*
...
int nas5g_message_decode(
  ...
  if (size > 2) {
    ...
    if (amf_security_context) {
      ...
      OAILOG_INFO(
          LOG_AMF_APP,
          "AMF_TEST: Compute the NAS message authentication code starts");
      mac = _nas5g_message_get_mac(
          buffer + offset, length - offset,
          amf_security_context->direction_decode, amf_security_context);
      OAILOG_INFO(
          LOG_AMF_APP,
          "AMF_TEST: Compute the NAS message authentication code completed");
      /*
       * Check NAS message integrity
       */
    ...
    OAILOG_INFO(
        LOG_AMF_APP,
        "AMF_TEST: Decode security protected NAS message starts for length %d",
        length - size);
    for (int i = 0; i < (length - size); i++) {
      OAILOG_INFO(
          LOG_AMF_APP, "AMF_TEST: nas_msg[%d]: %x", i, (buffer + size)[i]);
    }
    ...
    OAILOG_INFO(
        LOG_AMF_APP,
        "AMF_TEST: Decode security protected NAS message completed");
  } else {
    /*
     * Decode plain NAS message
     */
    OAILOG_INFO(LOG_AMF_APP, "AMF_TEST: Decode plain NAS message starts");
    bytes =
        _nas5g_message_plain_decode(buffer, &msg->header, &msg->plain, length);
    OAILOG_INFO(LOG_AMF_APP, "AMF_TEST: Decode plain NAS message completed");
  }

  if (bytes < 0) {
    OAILOG_INFO(LOG_AMF_APP, "AMF_TEST: Decode failed");
    OAILOG_FUNC_RETURN(LOG_AMF_APP, bytes);
  }
...
int nas5g_message_header_decode(
  ...
  // DECODE_U16(buffer, *(uint8_t*) (header), size);
  ...
  OAILOG_DEBUG(
      LOG_AMF_APP,
      "AMF_TEST: epd:%2x, security_header_type:%2x, "
  ...
  if (header->extended_protocol_discriminator ==
    ...
    if (header->security_header_type != SECURITY_HEADER_TYPE_NOT_PROTECTED) {
      ...
      if (*is_sr == false) {
        ...
        OAILOG_INFO(
            LOG_AMF_APP,
            "AMF_TEST: epd:%x, security_header_type:%x, "
            "message_authentication_code:%x, sequence_number:%x",
...
static int _nas5g_message_plain_decode(
  ...
  if (header->extended_protocol_discriminator ==
     ...
     */
    bytes = amf_msg_test.M5gNasMessageDecodeMsg(
...
static int _nas5g_message_protected_decode(
  ...
  if (plain_msg) {
    ...
    //TODO free_wrapper((void**) &plain_msg);
  }
  OAILOG_FUNC_RETURN(LOG_AMF_APP, bytes);
}
        ...
        switch (amf_security_context->selected_algorithms.encryption) {
          ...
          case M5G_NAS_SECURITY_ALGORITHMS_5G_EA0:
            OAILOG_INFO(
                ...
                amf_security_context->dl_count.seq_num);
            OAILOG_INFO(LOG_AMF_APP, "AMF_TEST: len%d %d", len);
...
static int _nas5g_message_encrypt(
  ...
  OAILOG_FUNC_IN(LOG_AMF_APP);
  OAILOG_INFO(LOG_AMF_APP, "AMF_TEST: ");
...
static uint32_t _nas5g_message_get_mac(
  ...
  switch (amf_security_context->selected_algorithms.integrity) {
    case M5G_NAS_SECURITY_ALGORITHMS_5G_IA1: {//TODO
    } break;
