...
namespace magma5g
{
   ...
   AmfMsg amf_msg_obj;
    ...
    static amf_procedures_t* _nas_new_amf_procedures(amf_context_t* const amf_context) 
    {
  ...
  nas_5g_auth_info_proc_t* nas_new_5gcn_auth_info_procedure(amf_context_t* const amf_context) {
    ...
    //auth_info_proc->cn_proc.base_proc.type = NAS_PROC_TYPE_CN;//TODO -  NEED-RECHECK
    ...
    if (wrapper) {
      ...
      //LIST_INSERT_HEAD(&amf_context->amf_procedures->cn_procs, wrapper, entries);//TODO -  NEED-RECHECK commented as list in class. recheck
...
//---------------------------------------------------------------------------------------------
    nas_amf_registration_proc_t* nas_proc::get_nas_specific_procedure_registration(const amf_context_t* ctxt) 
    {
...
//------------------------------------------------------------------------------
bool nas_proc::is_nas_specific_procedure_registration_running(const amf_context_t* ctxt) {
...
//-----------------------------------------------------------------------------
static amf_procedures_t* nas_new_amf_procedures(amf_context_t* amf_context)
 {
...
//-----------------------------------------------------------------------------
int nas_proc::nas5g_message_decode(unsigned char*  buffer, amf_nas_message_t *nas_msg , int length,
        amf_security_context_t* amf_security_context,amf_nas_message_decode_status_t* decode_status)
        {
          ...
          * Decode the header
          */
         //OAILOG_STREAM_HEX( OAILOG_LEVEL_DEBUG, LOG_NAS5G, "Incoming NAS message: ", buffer, length);
         //size = amf_msg_obj.AmfMsgDecodeHeaderMsg(&nas_msg->header, &buffer, length);
...
//------------------------------------------------------------------------------------
  ...
  //ident_proc->amf_com_proc.amf_proc.base_proc.type = NAS_PROC_TYPE_AMF;//TODO -  NEED-RECHECK
...
static int amf_identification_request(nas_amf_ident_proc_t* const proc) {
  ...
  ue_m5gmm_context_s* ue_5gmm_context;// =  amf_ue_context_exists_amf_ue_ngap_id(proc->ue_id);//TODO -  NEED-RECHECK need to define
  if (ue_5gmm_context) {
...
//-------------------------------------------------------------------------------------
      int identification::amf_proc_identification(amf_context_t* const amf_context, nas_amf_proc_t* const amf_proc,
