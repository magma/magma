...
nas_amf_registration_proc_t* nas_proc::get_nas_specific_procedure_registration(
  ...
  // OAILOG_INFO(LOG_AMF_APP, "AMF-TEST: ctxt:%p, amf_procedures:%p,
  // amf_specific_proc:%p, amf_specific_proc->type:%d, from %s\n", ctxt,
  // ctxt->amf_procedures, ctxt->amf_procedures->amf_specific_proc,
  // ctxt->amf_procedures->amf_specific_proc->type, __FUNCTION__);
  OAILOG_INFO(
  ...
  if ((ctxt) && (ctxt->amf_procedures) &&
      ...
      ((AMF_SPEC_PROC_TYPE_REGISTRATION ==
        ctxt->amf_procedures->amf_specific_proc->type))) {
    OAILOG_INFO(LOG_AMF_APP, "AMF-TEST: checks passed ,from%s\n", __FUNCTION__);
