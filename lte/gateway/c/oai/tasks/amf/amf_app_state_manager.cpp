...
constexpr char AMF_UE_ID_UE_CTXT_TABLE_NAME[] = "amf_app_amf_ue_ngap_id_ue_context_htbl";
...
constexpr char AMF_GNB_UE_ID_AMF_UE_ID_TABLE_NAME[] = "anf_app_gnb_ue_ngap_id_ue_context_htbl";
...
//Singleton class initializer which calls to create new object of AmfNasStateManager
...
amf_app_desc_t* AmfNasStateManager::get_state(bool read_from_redis) {
  //OAILOG_DEBUG(LOG_AMF_APP, "Inside get_state of AMF with read_from_db %d", read_from_redis);
...
void AmfNasStateManager::free_state() {
  ...
  clear_amf_nas_hashtables();
  //timer_remove(state_cache_p->statistic_timer_id, nullptr); //TODO -  NEED-RECHECK on timer
...
{
    //AssertFatal(is_initialized, "StateManager init() function should be called to initialize state");
    return state_ue_ht;
}
