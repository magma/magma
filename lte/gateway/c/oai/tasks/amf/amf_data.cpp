...
namespace magma5g
{
    ...
    void amf_ctx_set_attribute_valid(amf_context_t* ctxt, const int attribute_bit_pos) 
    {
    ...
    void amf_ctx_set_valid_imsi(amf_context_t* ctxt, imsi_t* imsi, const imsi64_t imsi64) {
    ctxt->_imsi   = *imsi;
    ctxt->_imsi64 = imsi64;
...
//------------------------------------------------------------------------------
void nas_amf_smc_proc_t::amf_ctx_set_security_eksi(amf_context_t* ctxt, ksi_t eksi) {
  ctxt->_security.eksi = eksi;
  OAILOG_TRACE( LOG_NAS_AMF, "ue_id=" AMF_UE_NGAP_ID_FMT " set security context eksi %d\n",
      (PARENT_STRUCT(ctxt, ue_m5gmm_context_s, amf_context))->amf_ue_ngap_id, eksi);
}

    //------------------------------------------------------------------------------
void nas_amf_smc_proc_t::amf_ctx_set_security_type(amf_context_t*  ctxt, amf_sc_type_t sc_type) {
  ctxt->_security.sc_type = sc_type;
  OAILOG_TRACE( LOG_NAS_AMF, "ue_id=" AMF_UE_NGAP_ID_FMT " set security context security type %d\n",
      (PARENT_STRUCT(ctxt, ue_m5gmm_context_s, amf_context))->amf_ue_ngap_id, sc_type);
}
...
void amf_ctx_clear_security(amf_context_t*  ctxt) {
  ...
  ctxt->_security.selected_algorithms.encryption = 0; //NAS_SECURITY_ALGORITHMS_EEA0;
  ctxt->_security.selected_algorithms.integrity  = 0; //NAS_SECURITY_ALGORITHMS_EIA0;
  ...
  OAILOG_DEBUG(LOG_NAS_AMF, "ue_id=" AMF_UE_NGAP_ID_FMT " cleared security context \n",
