  ...
  if (msg->m5gs_mobile_identity.mobile_identity.imsi.type_of_identity ==
    ...
    if(msg->m5gs_mobile_identity.mobile_identity.imsi.protect_schm_id == 
       ...
       memcpy(&supi_imsi.msin, 
          &msg->m5gs_mobile_identity.mobile_identity.imsi.scheme_output, MSIN_MAX_LENGTH);

       //Copy entire supi_imsi to param->imsi->u.value
       memcpy(&params->imsi->u.value, &supi_imsi, IMSI_BCD8_SIZE);

       OAILOG_INFO(LOG_AMF_APP, "AMF_TEST : printing SUPI/IMSI from params->imsi->u.value\n");
       //OAILOG_INFO(LOG_AMF_APP, "SUPI as IMSI derived : %02x %02x %02x %02x %02x %02x %02x %02x",
       OAILOG_INFO(LOG_AMF_APP, "SUPI as IMSI derived : %02x%02x%02x%02x%02x%02x%02x%02x \n",
...
int amf_procedure_handler::amf_handle_identity_response(
  ...
  if (msg->mobile_identity.imsi.type_of_identity ==
    ...
    if(msg->mobile_identity.imsi.protect_schm_id == 
       ...
       p_imsi            = &imsi;
       supi_imsi.plmn.mcc_digit1 =
                 msg->mobile_identity.imsi.mcc_digit1;
       supi_imsi.plmn.mcc_digit2 =
                 msg->mobile_identity.imsi.mcc_digit2;
       supi_imsi.plmn.mcc_digit3 =
                 msg->mobile_identity.imsi.mcc_digit3;
       supi_imsi.plmn.mnc_digit1 =
                 msg->mobile_identity.imsi.mnc_digit1;
       supi_imsi.plmn.mnc_digit2 =
                 msg->mobile_identity.imsi.mnc_digit2;
       supi_imsi.plmn.mnc_digit3 =
                 msg->mobile_identity.imsi.mnc_digit3;

       //copy 5 octet scheme_output to msin of supi_imsi
       memcpy(&supi_imsi.msin, 
          &msg->mobile_identity.imsi.scheme_output, MSIN_MAX_LENGTH);

       //Copy entire supi_imsi to imsi.u.value which is 8 bytes
       memcpy(&imsi.u.value, &supi_imsi, IMSI_BCD8_SIZE);
       OAILOG_INFO(LOG_AMF_APP, "AMF_TEST : Identity RSP SUPI/IMSI from imsi.u.value");
       OAILOG_INFO(LOG_AMF_APP, "SUPI as IMSI derived : %02x%02x%02x%02x%02x%02x%02x%02x",
	...
	*/
       OAILOG_ERROR(LOG_AMF_APP, "Invalid protection scheme  received "
    ...
    OAILOG_INFO(LOG_NAS_AMF, "AMF_TEST: PLMN as 0x%02x%02x%02x\n", *octet1, *octet2, *octet3);
    OAILOG_INFO(LOG_NAS_AMF, "AMF_TEST: Region ID as 0x%02x\n", octet4);
    //OAILOG_INFO(LOG_NAS_AMF, "AMF_TEST: set_id and pointer as %04" PRIu16 "\n", *set_id_pointer);
    OAILOG_INFO(LOG_NAS_AMF, "AMF_TEST: set_id and pointer as 0x%02x\n", *set_id_pointer);
    OAILOG_INFO(LOG_NAS_AMF, "AMF_TEST: 5G-TMSI as 0x%08" PRIx32 "\n", amf_guti.m_tmsi);
    //===== Only debugging purpose and will be deleted later=======
    uint8_t plmn3ch[3];
    memset(plmn3ch, 0, 3);
    memcpy(&plmn3ch, &amf_guti, 3);
    OAILOG_INFO(LOG_AMF_APP, "AMF_TEST: PLMN from GUTI generated function 3 octets  0x%02x%02x%02x",
    ...
    if(amf_supi_guti_map.size() == 0){
       //first entry.
       amf_supi_guti_map.insert(std::pair<imsi64_t, guti_and_amf_id_t>(imsi64, guti_and_amf_id));
    }
    else {
