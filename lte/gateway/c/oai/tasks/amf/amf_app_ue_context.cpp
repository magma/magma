...
namespace magma5g
{
  nas_network nas_networks_ctx; // need to asses public functions in many functions
  static void _directoryd_report_location(uint64_t imsi, uint8_t imsi_len) 
{
    ...
    amf_ue_ngap_id_t amf_app_ue_context::amf_app_ctx_get_new_ue_id(amf_ue_ngap_id_t* amf_app_ue_ngap_id_generator_p)
    {
 ...
 //------------------------------------------------------------------------------
 void amf_app_ue_context::notify_ngap_new_ue_amf_ngap_id_association(const ue_m5gmm_context_s* ue_context_p) 
 {
...
//-------------------------------------------------------------------------------------------------
    int amf_app_ue_context::amf_insert_ue_context(const amf_ue_context_t* amf_ue_context_p, 
        ...
        if (INVALID_AMF_UE_NGAP_ID != ue_context_p->amf_ue_ngap_id) {
          ...
          // filled guti
            if ((0 != ue_context_p->amf_context._m5_guti.guamfi.amf_code) ||
                ...
                (0 != ue_context_p->amf_context._m5_guti.guamfi.plmn.mcc_digit1) ||  // MCC 000 does not exist in ITU table
...
amf_context_t* amf_context_get(const amf_ue_ngap_id_t ue_id) 
{
  ...
  if (INVALID_AMF_UE_NGAP_ID != ue_id) {
    ue_m5gmm_context_s* ue_mm_context = amf_ue_context_exists_amf_ue_ngap_id(ue_id);
    if (ue_mm_context) {
  ...
  const amf_ue_ngap_id_t amf_ue_ngap_id) 
{
  ...
  hashtable_ts_get(state_imsi_ht, (const hash_key_t) amf_ue_ngap_id, 
                   (void**) &ue_context_p);
  if (ue_context_p) {
    OAILOG_TRACE(
        LOG_AMF_APP,
...
int amf_context_upsert_imsi(amf_context_t* elm) {
  hashtable_rc_t h_rc = HASH_TABLE_OK;
  amf_ue_ngap_id_t ue_id = (PARENT_STRUCT(elm, ue_m5gmm_context_s, amf_context))->amf_ue_ngap_id;
       ...
       // amf_app_ue_context_free_content(ue_context_p);
        ...
        if ((ue_context_p->amf_context._m5_guti.guamfi.amf_code) ||
            ...
            hash_rc = obj_hashtable_uint64_ts_remove( amf_ue_context_p->guti_ue_context_htbl,
                (const void* const) & ue_context_p->amf_context._m5_guti, sizeof(ue_context_p->amf_context._m5_guti));
            //if (HASH_TABLE_OK != hash_rc)
            //OAILOG_ERROR(LOG_AMF_APP,"UE Context not found!\n"
