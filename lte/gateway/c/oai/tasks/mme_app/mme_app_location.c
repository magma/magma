  ...
  struct ue_mm_context_s *const ue_context_p)
{
  ...
  OAILOG_INFO(
    TASK_MME_APP,
    "Sending S6A UPDATE LOCATION REQ to S6A, ue_id = %u\n",
  ...
  OAILOG_DEBUG(
    TASK_MME_APP,
    "S6A ULR: RAT TYPE = (%d) for (ue_id = %u)\n",
    s6a_ulr_p->rat_type,
    ue_context_p->mme_ue_s1ap_id);
  /*
  ...
  if (
    ue_context_p->emm_context.volte_params.presencemask &
  ...
  const s6a_update_location_ans_t *const ula_pP)
{
  ...
  if (message_p == NULL) {
    OAILOG_ERROR(
      LOG_MME_APP,
      "Message pointer is NULL while allocating new message for PDN Config "
      "Rsp, (ue_id = %u)\n",
      ue_mm_context->mme_ue_s1ap_id);
    unlock_ue_contexts(ue_mm_context);
    OAILOG_FUNC_RETURN(LOG_MME_APP, RETURNerror);
  ...
  */
  ...
  OAILOG_INFO(
    LOG_MME_APP,
    "Sending PDN CONFIG RSP to NAS for (ue_id = %u)\n",
  ...
  const s6a_cancel_location_req_t *const clr_pP)
{
  ...
  if (ue_context_p->ecm_state == ECM_IDLE) {
    ...
    OAILOG_INFO(
      LOG_MME_APP,
      "Sending Detach to NAS for (ue_id = %u)\n",
      ue_context_p->mme_ue_s1ap_id);
