  ...
  itti_s11_create_session_response_t *const create_sess_resp_pP)
{
  ...
  if (create_sess_resp_pP->cause.cause_value != REQUEST_ACCEPTED) {
    ...
    OAILOG_ERROR(
      LOG_MME_APP,
      "Sending NAS PDN connectivity failure for ue_id = (%u)"
      ", bearer id = (%d)"
      ", pti = (%d)\n",
      ue_context_p->mme_ue_s1ap_id,
      bearer_id,
  ...
  OAILOG_INFO(
    TASK_MME_APP,
    "Sending PDN CONNECTIVITY RESPONSE to NAS for ue_id = (%d)\n"
    " bearer ID = (%d)\n",
    ue_context_p->mme_ue_s1ap_id,
    bearer_id);
  rc = itti_send_msg_to_task(TASK_NAS_MME, message_p);
  ...
  itti_mme_app_initial_context_setup_rsp_t *const initial_ctxt_setup_rsp_pP)
{
  ...
  itti_send_msg_to_task(TASK_SPGW, message_p);
  /*
  ...
  s1ap_cn_domain_t domain_indicator)
{
  ...
  rc = itti_send_msg_to_task(TASK_S1AP, message_p);
