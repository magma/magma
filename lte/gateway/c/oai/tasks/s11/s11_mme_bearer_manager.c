  ...
  itti_s11_downlink_data_notification_acknowledge_t * ack_p)
{
  ...
  memset (&cause, 0, sizeof (gtpv2c_cause_t));     // Prepare a create bearer response to send to SGW.
  ...
  ulp_ack.u_api_info.triggeredRspInfo.teidLocal  = ack_p->local_teid;     // Set the remote TEID
  ...
  itti_s11_delete_bearer_command_t * cmd_p)
{
  ...
  ulp_req.u_api_info.initialReqInfo.edns_peer_ip = (struct sockaddr*)&cmd_p->edns_peer_ip;
  ...
  itti_s11_create_bearer_response_t * response_p)
{
  ...
  memset (&ulp_req, 0, sizeof (nw_gtpv2c_ulp_api_t));     // Prepare a create bearer response to send to SGW.
