...
int sctp_itti_send_new_association(
  ...
  /*else */ if (ppid == NGAP) {
    ...
    OAILOG_ERROR(
        LOG_SCTP, "ppid not matching in sctp_itti_send_new_association ");
    return 0;
  }
}
...
int sctp_itti_send_new_message_ind(
  ...
  if (ppid == S1AP) {
    ...
    OAILOG_ERROR(
        LOG_SCTP, "ppid not matching in sctp_itti_send_new_message_ind ");
    return 0;
  }
}
...
int sctp_itti_send_com_down_ind(
  ...
  if (ppid == S1AP) {
    ...
    OAILOG_ERROR(
        LOG_SCTP, " ppid not matching in sctp_itti_send_com_down_ind ");
    return 0;
  }
}
