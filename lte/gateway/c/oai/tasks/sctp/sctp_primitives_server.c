...
static int count =0;
  ...
  switch (ITTI_MSG_ID(received_message_p)) {
     ...
     if(count <2)
	{
      ...
      if (sctpd_init(&received_message_p->ittiMsg.sctpInit) < 0) { //sctpd_init 
		return 0;//temp to test remove later
        Fatal("ACL_TAG Failed to init sctpd\n");
      }
...
#if 1
	else if (received_message_p->ittiMsg.sctpInit.ngap_ppid == NGAP) {
	       //msg = itti_alloc_new_message(TASK_NGAP, SCTP_AMF_SERVER_INITIALIZED);
	       msg = itti_alloc_new_message(TASK_NGAP, SCTP_MME_SERVER_INITIALIZED);
    ...
    case SCTP_DATA_REQ: {
      uint32_t ppid =SCTP_DATA_REQ(received_message_p).ppid;		//check ppid here	
      ...
      if (sctpd_send_dl(ppid ,assoc_id, stream, payload) < 0) {
	      if (ppid == S1AP) {
		      ...
		      OAILOG_ERROR(LOG_SCTP, "pid not matching in sctp_itti_send_lower_layer_conf ");
	      }
      }
    } break;
...
static void* sctp_thread(__attribute__((unused)) void* args_p) {
  ...
  init_task_context(TASK_SCTP, (task_id_t[]){TASK_MME_APP, TASK_S1AP, TASK_NGAP}, 3, handle_message, &sctp_task_zmq_ctx);
...
int sctp_init(const mme_config_t* mme_config_p , const amf_config_t* amf_config_p) {
