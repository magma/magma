...
int balloc(bstring b, int olen)
{
  int len;
  if (
    b == NULL || b->data == NULL || b->slen < 0 || b->mlen <= 0 ||
...
int bcatcstr(bstring b, const char *s)
{
  ...
  if (
    b == NULL || b->data == NULL || b->slen < 0 || b->mlen < b->slen ||
    b->mlen <= 0 || s == NULL)
    return BSTR_ERR;
...
int bassign(bstring a, const_bstring b)
{
  ...
  if (b->slen != 0) {
    ...
    if (
      a == NULL || a->data == NULL || a->mlen < a->slen || a->slen < 0 ||
      a->mlen == 0)
      return BSTR_ERR;
  }
...
{
  if (
    n < 0 || b == NULL || b->data == NULL || b->mlen < b->slen || b->slen < 0 ||
    b->mlen <= 0)
    return BSTR_ERR;
  if (b->slen > n) {
    b->slen = n;
...
int btoupper(bstring b)
{
  int i, len;
  if (
    b == NULL || b->data == NULL || b->mlen < b->slen || b->slen < 0 ||
    b->mlen <= 0)
    return BSTR_ERR;
...
int bstrnicmp(const_bstring b0, const_bstring b1, int n)
{
  ...
  if (
    bdata(b0) == NULL || b0->slen < 0 || bdata(b1) == NULL || b1->slen < 0 ||
    n < 0)
    return SHRT_MIN;
  m = n;
...
{
  if (
    b0 == NULL || b1 == NULL || b0->data == NULL || b1->data == NULL ||
    b0->slen < 0 || b1->slen < 0)
    return BSTR_ERR;
  if (b0->slen != b1->slen) return BSTR_OK;
  if (b0->data == b1->data || b0->slen == 0) return 1;
...
int biseqcstrcaseless(const_bstring b, const char *s)
{
  ...
  for (i = 0; i < b->slen; i++) {
    if (
      s[i] == '\0' || (b->data[i] != (unsigned char) s[i] &&
...
int bdelete(bstring b, int pos, int len)
{
  ...
  if (
    len < 0 || b == NULL || b->data == NULL || b->slen < 0 ||
...
{
  if (
    b == NULL || b->slen < 0 || b->mlen <= 0 || b->mlen < b->slen ||
    b->data == NULL)
    return BSTR_ERR;
...
int binstr(const_bstring b1, int pos, const_bstring b2)
{
  ...
  if (
    b1 == NULL || b1->data == NULL || b1->slen < 0 || b2 == NULL ||
...
int bsetstr(bstring b0, int pos, const_bstring b1, unsigned char fill)
{
  ...
  if (
    pos < 0 || b0 == NULL || b0->slen < 0 || NULL == b0->data ||
...
int binsertblk(bstring b, int pos, const void *blk, int len, unsigned char fill)
{
  ...
  if (
    b == NULL || blk == NULL || pos < 0 || len < 0 || b->slen < 0 ||
  ...
  /* Aliasing case */
  if (
    ((size_t)((unsigned char *) blk + len)) >= ((size_t) b->data) &&
...
int breplace(bstring b1, int pos, int len, const_bstring b2, unsigned char fill)
{
  ...
  pl = pos + len;
  if (
    b1 == NULL || b2 == NULL || b1->data == NULL || b2->data == NULL ||
  ...
  /* Aliasing case */
  if (
    (pd = (ptrdiff_t)(b2->data - b1->data)) >= 0 && pd < (ptrdiff_t) b1->slen) {
  ...
  if (aux->slen != len)
    bstr__memmove(
      b1->data + pos + aux->slen, b1->data + pos + len, b1->slen - (pos + len));
  ...
  instr_fnptr instr)
{
  ...
  if (
    b == NULL || b->data == NULL || find == NULL || find->data == NULL ||
...
int binsertch(bstring b, int pos, int len, unsigned char fill)
{
  ...
  if (
    pos < 0 || b == NULL || b->slen < 0 || b->mlen < b->slen || b->mlen <= 0 ||
    len < 0)
    return BSTR_ERR;
...
int breada(bstring b, bNread readPtr, void *parm)
{
  ...
  if (
    b == NULL || b->mlen <= 0 || b->slen < 0 || b->mlen < b->slen ||
    readPtr == NULL)
    return BSTR_ERR;
...
bstring bgets(bNgetc getcPtr, void *parm, char terminator)
{
  ...
  if (
    0 > bgetsa(buff = bfromcstr(""), getcPtr, parm, terminator) ||
    0 >= buff->slen) {
    bdestroy(buff);
    buff = NULL;
  }
...
int bsreadlna(bstring r, struct bStream *s, char terminator)
{
  ...
  if (
    s == NULL || s->buff == NULL || r == NULL || r->mlen <= 0 || r->slen < 0 ||
    r->mlen < r->slen)
    return BSTR_ERR;
  l = s->buff->slen;
...
int bsreadlnsa(bstring r, struct bStream *s, const_bstring term)
{
  ...
  if (
    s == NULL || s->buff == NULL || r == NULL || term == NULL ||
  ...
  void *parm)
{
  ...
  if (
    cb == NULL || s == NULL || s->readFnPtr == NULL || splitStr == NULL ||
    splitStr->slen < 0)
    return BSTR_ERR;
...
int bstrListAlloc(struct bstrList *sl, int msz)
{
  ...
  size_t nsz;
  if (
    !sl || msz <= 0 || !sl->entry || sl->qty < 0 || sl->mlen <= 0 ||
    sl->qty > sl->mlen)
    return BSTR_ERR;
  ...
  void *parm)
{
  ...
  if (
    cb == NULL || str == NULL || pos < 0 || pos > str->slen ||
...
struct bstrList *bsplits(const_bstring str, const_bstring splitStr)
{
  ...
  if (
    str == NULL || str->slen < 0 || str->data == NULL || splitStr == NULL ||
...
int bformata(bstring b, const char *fmt, ...)
{
  ...
  if (
    b == NULL || fmt == NULL || b->data == NULL || b->mlen <= 0 ||
...
int bvcformata(bstring b, int count, const char *fmt, va_list arg)
{
  ...
  if (
    b == NULL || fmt == NULL || count <= 0 || b->data == NULL || b->mlen <= 0 ||
    b->slen < 0 || b->slen > b->mlen)
    return BSTR_ERR;
