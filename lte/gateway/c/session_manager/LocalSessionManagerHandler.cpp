...
LocalSessionManagerHandlerImpl::LocalSessionManagerHandlerImpl(
    ...
    : enforcer_(enforcer), session_store_(session_store), reporter_(reporter),
      directoryd_client_(directoryd_client),

      current_epoch_(0), reported_epoch_(0), retry_timeout_(1) {}
...
void LocalSessionManagerHandlerImpl::check_usage_for_reporting(
  ...
  reporter_->report_updates(request, [
    ...
    if (!status.ok()) {
      ...
      enforcer_->update_session_credits_and_rules(*session_map_ptr, response,
                                                  session_update);
...
copy_session_info2create_req(const LocalCreateSessionRequest &request,
...
void LocalSessionManagerHandlerImpl::CreateSession(
  ...
  enforcer_->get_event_base().runInEventBaseThread([this, context,
    ...
    if (enforcer_->session_with_imsi_exists(session_map, imsi)) {
      ...
      if (!enforcer_->session_with_apn_exists(session_map, imsi,
                                              request_cpy.apn())) {
    ...
    send_create_session(session_map,
                        copy_session_info2create_req(request_cpy, sid), imsi,
                        sid, cfg, response_callback);
  });
}
...
void LocalSessionManagerHandlerImpl::send_create_session(
    ...
    std::function<void(grpc::Status, LocalCreateSessionResponse)>
        response_callback) {
  reporter_->report_create_session(request, [
    this, imsi, sid, cfg, response_callback,
    session_map_ptr = std::make_shared<SessionMap>(std::move(session_map))
  ](Status status, CreateSessionResponse response) mutable {
    if (status.ok()) {
      bool success = enforcer_->init_session_credit(*session_map_ptr, imsi, sid,
                                                    cfg, response);
      if (!success) {
...
void LocalSessionManagerHandlerImpl::end_session(
  ...
  try {
    ...
    enforcer_->terminate_subscriber(session_map, request.sid().id(),
                                    request.apn(), update);
