...
ChargingGrant::ChargingGrant(const StoredChargingGrant& marshaled) {
  ...
  is_suspended   = marshaled.is_suspended;
}
...
StoredChargingGrant ChargingGrant::marshal() {
  ...
  marshaled.is_suspended                     = is_suspended;
  return marshaled;
}
...
CreditValidity ChargingGrant::is_valid_credit_response(
  ...
  if (!update.success()) {
    if (DiameterCodeHandler::is_permanent_failure(update.result_code())) {
      ...
      credit_validity = TRANSIET_ERROR;
...
void ChargingGrant::set_is_suspended(
    bool suspended, SessionCreditUpdateCriteria* uc) {
  if (is_suspended != suspended) {
    MLOG(MDEBUG) << "Credit suspension set to: " << suspended;
  }
  is_suspended     = suspended;
  uc->is_suspended = suspended;
}

bool ChargingGrant::get_is_suspended() {
  return is_suspended;
}
