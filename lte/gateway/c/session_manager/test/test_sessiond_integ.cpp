...
TEST_F(SessiondTest, end_to_end_success) {
  std::promise<void> end_promise;
  {
    ...
    EXPECT_CALL(
        *controller_mock,
        UpdateSession(
            testing::_, CheckSingleUpdateSession(expected_update), testing::_))
        .Times(1)
        .WillOnce(testing::DoAll(
    ...
    EXPECT_CALL(
        *controller_mock,
        TerminateSession(testing::_, CheckTerminateImsi(IMSI1), testing::_))
        .Times(1)
        .WillOnce(testing::DoAll(
...
TEST_F(SessiondTest, end_to_end_cloud_down) {
  std::promise<void> end_promise;
  {
    ...
    EXPECT_CALL(
        *controller_mock,
        UpdateSession(testing::_, CheckSingleUpdateSession(second_update), testing::_))
        .Times(1)
