...
namespace magma {
const SessionState::Config test_sstate_cfg =
...
TEST_F(SessionStateTest, test_insert_credit) {
  ...
  EXPECT_EQ(
    session_state->get_charging_pool().get_credit(1, ALLOWED_TOTAL),
    1024);

  receive_credit_from_pcrf("m1", 1024, MonitoringLevel::PCC_RULE_LEVEL);
  EXPECT_EQ(
    session_state->get_monitor_pool().get_credit("m1", ALLOWED_TOTAL),
    1024);
}
...
TEST_F(SessionStateTest, test_add_used_credit) {
  ...
  EXPECT_EQ(
    session_state->get_charging_pool().get_credit(1, USED_TX),
    2000);
  EXPECT_EQ(
    session_state->get_monitor_pool().get_credit("m1", USED_TX),
    2000);

  session_state->add_used_credit("dyn_rule1", 4000, 2000);
  EXPECT_EQ(
    session_state->get_charging_pool().get_credit(2, USED_TX),
    4000);
  EXPECT_EQ(
    session_state->get_monitor_pool().get_credit("m2", USED_TX),
    4000);
...
TEST_F(SessionStateTest, test_mixed_tracking_rules) {
  ...
  EXPECT_EQ(
    ...
    2000);
  EXPECT_EQ(
    session_state->get_monitor_pool().get_credit("m1", USED_RX),
    1000);
  ...
  EXPECT_EQ(
    ...
    4000);
  EXPECT_EQ(
    session_state->get_charging_pool().get_credit(2, USED_RX),
    2000);
  session_state->add_used_credit("dyn_rule3", 5000, 3000);
  EXPECT_EQ(
    session_state->get_charging_pool().get_credit(3, USED_TX),
    5000);
  EXPECT_EQ(
    session_state->get_monitor_pool().get_credit("m3", USED_TX),
    5000);
...
TEST_F(SessionStateTest, test_reauth_key) {
  ...
  EXPECT_EQ(
    session_state->get_charging_pool().get_credit(1, REPORTING_TX),
    1000);
  EXPECT_EQ(
    session_state->get_charging_pool().get_credit(1, REPORTING_RX),
    500);
...
TEST_F(SessionStateTest, test_reauth_new_key) {
  ...
  receive_credit_from_ocs(1, 1024);
  EXPECT_EQ(session_state->get_charging_pool().get_credit(1, ALLOWED_TOTAL), 1024);
}
