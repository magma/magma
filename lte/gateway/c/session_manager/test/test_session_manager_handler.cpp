...
TEST_F(SessionManagerHandlerTest, test_create_session_cfg) {
  ...
  const auto& sid             = id_gen_.gen_session_id(IMSI1);
  SessionConfig cfg;
  cfg.common_context =
  ...
  create_session_create_response(IMSI1, monitoring_key, static_rules, &response);
  response.mutable_static_rules()->Add()->mutable_rule_id()->assign("rule1");
  create_credit_update_response(
      IMSI1, 1, 1536, response.mutable_credits()->Add());
...
TEST_F(SessionManagerHandlerTest, test_session_recycling_lte) {
  ...
  SessionConfig cfg;
  cfg.common_context =
      build_common_context(IMSI1, "", "apn1", MSISDN, TGPP_LTE);
  ...
  create_session_create_response(
      IMSI1, monitoring_key, static_rules, &response);
  response.mutable_static_rules()->Add()->mutable_rule_id()->assign("rule1");
  create_credit_update_response(
      IMSI1, 1, 1536, response.mutable_credits()->Add());
  ...
  LocalCreateSessionRequest request;
  grpc::ServerContext create_context;
  auto common = build_common_context(IMSI1, "", "apn1", MSISDN, TGPP_LTE);
  request.mutable_common_context()->CopyFrom(common);
  ...
  common = build_common_context(IMSI1, "", "apn1", "new msisdn", TGPP_LTE);
...
TEST_F(SessionManagerHandlerTest, test_report_rule_stats) {
  ...
  auto sid           = id_gen_.gen_session_id(IMSI1);
  SessionConfig cfg  = {};
  cfg.common_context = build_common_context("", IP2, "APN", MSISDN, TGPP_LTE);
  const auto& lte_context = build_lte_context(
      IP1, "imei", "plmn_id", "imsi_plmn_id", "user_loc", 1, nullptr);
  cfg.rat_specific_context.mutable_lte_context()->CopyFrom(lte_context);
  SessionRead req  = {IMSI1};
  auto session_map = session_store->read_sessions(req);
  ...
  EXPECT_CALL(
      *reporter, report_updates(CheckUpdateSessionRequestNumber(1), testing::_))
      .Times(1);
