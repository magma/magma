...
TEST_F(LocalEnforcerTest, test_sync_sessions_on_restart_revalidation_timer) {
  ...
  session_map[IMSI1].push_back(std::unique_ptr<SessionState>(session_state));
...
TEST_F(LocalEnforcerTest, test_update_with_transient_error) {
  ...
  EXPECT_TRUE(
      session_uc[IMSI1][SESSION_ID_1].charging_credit_map[1].is_suspended);
  EXPECT_EQ(
      session_uc[IMSI1][SESSION_ID_1].charging_credit_map[1].service_state,
      SESSION_ACTIVE);
  EXPECT_FALSE(
      session_uc[IMSI1][SESSION_ID_1].charging_credit_map[2].is_suspended);
  EXPECT_EQ(
...
TEST_F(LocalEnforcerTest, test_reauth_with_redirected_suspended_credit) {
  ...
  EXPECT_EQ(
      ...
      SERVICE_REDIRECTED);
  EXPECT_TRUE(update[IMSI1][SESSION_ID_1].charging_credit_map[1].is_suspended);
  ...
  local_enforcer->update_session_credits_and_rules(
      session_map, update_response, update);
  EXPECT_FALSE(update[IMSI1][SESSION_ID_1].charging_credit_map[1].is_suspended);
}
