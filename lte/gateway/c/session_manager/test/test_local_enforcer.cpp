...
TEST_F(LocalEnforcerTest, test_aggregate_records_mixed_ips) {
  ...
  // ipv4 usage
  create_rule_record(
      IMSI1, teids1.enb_teid(), "rule1", 10, 20, record_list->Add());
  // ipv6 usage for the same charging key and subscriber
  create_rule_record(
      IMSI1, teids1.enb_teid(), "rule2", 5, 15, record_list->Add());
  create_rule_record(
      IMSI1, teids1.enb_teid(), "rule3", 100, 150, record_list->Add());
...
TEST_F(LocalEnforcerTest, test_update_session_credits_and_rules) {
  ...
  auto record_list = table.mutable_records();
  create_rule_record(
      IMSI1, teids1.enb_teid(), "rule1", 1024, 1024, record_list->Add());
  auto update = SessionStore::get_default_session_update(session_map);
  local_enforcer->aggregate_records(session_map, table, update);
...
TEST_F(LocalEnforcerTest, test_terminate_credit) {
  ...
  create_rule_record(
      IMSI1, teids1.enb_teid(), "internal_default_drop_flow_rule", 0, 0,
      record_list->Add());
...
TEST_F(LocalEnforcerTest, test_final_unit_activation_and_canceling) {
  ...
  insert_static_rule(1, "", "rule1");
  insert_static_rule(1, "", "rule3");
  auto& ip_addr      = cfg1.common_context.ue_ipv4();
  auto& ipv6_addr    = cfg1.common_context.ue_ipv6();
  auto teids         = cfg1.common_context.teids();
  const auto& msisdn = cfg1.common_context.msisdn();
  // The activation for the static rules (rule1,rule3) and dynamic rule (rule2)
  EXPECT_CALL(
      *pipelined_client, activate_flows_for_rules(
                             IMSI1, ip_addr, ipv6_addr, CheckTeids(teids),
  ...
  create_rule_record(
      IMSI1, teids1.agw_teid(), "rule2", 1024, 2048, record_list->Add());
  create_rule_record(
      IMSI1, teids1.agw_teid(), "rule3", 1024, 2048, record_list->Add());
  auto update = SessionStore::get_default_session_update(session_map);
  local_enforcer->aggregate_records(session_map, table, update);
...
TEST_F(LocalEnforcerTest, test_final_unit_action_no_update) {
  ...
  auto record_list = table.mutable_records();
  create_rule_record(
      IMSI1, teids1.enb_teid(), "static_rule1", 1023, 0, record_list->Add());
  auto update = SessionStore::get_default_session_update(session_map);
  local_enforcer->aggregate_records(session_map, table, update);
  ...
  create_rule_record(
      IMSI1, teids1.enb_teid(), "static_rule1", 1024, 0, record_list->Add());
  update = SessionStore::get_default_session_update(session_map);
  local_enforcer->aggregate_records(session_map, table, update);
