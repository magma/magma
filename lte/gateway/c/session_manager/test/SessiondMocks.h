...
MOCK_METHOD3(AddRule, Status(grpc::ServerContext*,
  const PolicyRule*,
  Void*));
MOCK_METHOD3(ActivateFlows, Status(grpc::ServerContext*,
  const ActivateFlowsRequest*,
  ActivateFlowsResult*));
MOCK_METHOD3(DeactivateFlows, Status(grpc::ServerContext*,
...
public:
  ...
  MOCK_METHOD3(deactivate_flows_for_rules, bool(
    const std::string& imsi,
    const std::vector<std::string>& rule_ids,
    const std::vector<PolicyRule>& dynamic_rules));
  MOCK_METHOD4(activate_flows_for_rules, bool(
    const std::string& imsi,
 ...
 * Mock handler to mock actual request handling and just test server
 */
class MockCentralController final
  : public CentralSessionController::Service {
public:
  MOCK_METHOD3(CreateSession, Status(grpc::ServerContext*,
    const CreateSessionRequest*,
    CreateSessionResponse*));

  MOCK_METHOD3(UpdateSession, Status(grpc::ServerContext*,
    const UpdateSessionRequest*,
    UpdateSessionResponse*));

  MOCK_METHOD3(TerminateSession, Status(grpc::ServerContext*,
 ...
 * Mock handler to mock actual request handling and just test server
 */
class MockSessionHandler final
  : public LocalSessionManagerHandler {
public:
  ~MockSessionHandler() {}

  MOCK_METHOD3(ReportRuleStats, void(grpc::ServerContext*,
    const RuleRecordTable*,
    std::function<void(Status, Void)>));

  MOCK_METHOD3(CreateSession, void(grpc::ServerContext*,
    const LocalCreateSessionRequest*,
    std::function<void(Status, LocalCreateSessionResponse)>));

  MOCK_METHOD3(EndSession, void(grpc::ServerContext*,
    const SubscriberID*,
