...
bool SessionState::suspend_service_if_necesary_for_credit(
    CreditKey ckey, SessionStateUpdateCriteria& update_criteria) {
  uint suspended = 0;

  auto it = credit_map_.find(ckey);
  if (it == credit_map_.end()) {
    MLOG(MDEBUG) << "Could not find RG " << ckey << " Not suspending serivce";
    return false;
  }
  for (const auto& credit : credit_map_) {
    if (credit.second->is_suspended) {
      suspended++;
    }
  }
  if (credit_map_.size() > 0 && suspended == credit_map_.size()) {
...
bool SessionState::receive_charging_credit(
  ...
  if (credit_validity == TRANSIET_ERROR) {
...
void SessionState::set_suspend_credit(
    const CreditKey& charging_key, bool is_suspended,
    SessionStateUpdateCriteria& update_criteria) {
  auto it = credit_map_.find(charging_key);
  if (it != credit_map_.end()) {
    auto credit_uc = get_credit_uc(charging_key, update_criteria);
    auto& grant    = it->second;
    grant->set_is_suspended(is_suspended, credit_uc);
  }
}
...
void SessionState::get_rules_per_credit_key(
  ...
  std::vector<std::string> static_rules_out;
  std::vector<PolicyRule> dynamic_rules_out;

  static_rules_.get_rule_ids_for_charging_key(charging_key, static_rules_out);
  dynamic_rules_.get_rule_definitions_for_charging_key(
      charging_key, dynamic_rules_out);

  rulesToProcess.static_rules  = static_rules_out;
  rulesToProcess.dynamic_rules = dynamic_rules_out;
}
...
void SessionState::apply_charging_credit_update(
  ...
  charging_grant->is_suspended      = credit_uc.is_suspended;
}
