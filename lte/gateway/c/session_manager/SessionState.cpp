...
void SessionState::add_rule_usage(
  ...
  if (is_dynamic_rule_installed(rule_id) || is_static_rule_installed(rule_id)) {
    update_data_metrics(used_tx, used_rx);
  }
}
...
void SessionState::update_data_metrics(uint64_t bytes_tx, uint64_t bytes_rx) {
  ...
  increment_counter(
      UE_USED_COUNTER_NAME, bytes_tx, size_t(4), LABEL_IMSI, sid.c_str(),
      LABEL_APN, apn.c_str(), LABEL_MSISDN, msisdn.c_str(), LABEL_DIRECTION,
      DIRECTION_UP);
  increment_counter(
      UE_USED_COUNTER_NAME, bytes_rx, size_t(4), LABEL_IMSI, sid.c_str(),
      LABEL_APN, apn.c_str(), LABEL_MSISDN, msisdn.c_str(), LABEL_DIRECTION,
      DIRECTION_DOWN);
}
