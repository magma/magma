...
struct SessionActionOrStatus {
  ...
  static SessionActionOrStatus
  create_new_session_action(const std::string session_id) {
...
public:
   ...
   */
  virtual void
  BindPolicy2Bearer(ServerContext *context,
   ...
   */
  virtual void
  UpdateTunnelIds(ServerContext *context, UpdateTunnelIdsRequest *request,
   ...
   */
  virtual void
  SetSessionRules(ServerContext *context, const SessionRules *request,
                  std::function<void(Status, Void)> response_callback) = 0;
};
...
private:
   ...
   */
  SessionActionOrStatus
  handle_create_session_cwf(SessionMap &session_map,
                            const std::string &session_id,
   ...
   */
  SessionActionOrStatus
  recycle_cwf_session(std::unique_ptr<SessionState> &session,
                      const SessionConfig &cfg, SessionMap &session_map) const;
  /**
