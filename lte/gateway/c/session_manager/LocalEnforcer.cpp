...
void LocalEnforcer::update_charging_credits(
  ...
  for (const auto& credit_update_resp : response.responses()) {
    ...
    // handle credit level failures
    if (!credit_update_resp.success() &&
        !handle_credit_update_failure(credit_update_resp, &actions)) {
      continue;
    }
    ...
    if (!should_activate) {
      // this credit still doesn't have quota at this point
      // if FUA is now terminate, we can handle this here
      // if (credit_update_resp.credit().is_final() &&
      //     credit_update_resp.credit().final_action() ==
      //         ChargingCredit_FinalAction_TERMINATE) {
      //   actions.sessions_to_terminate.insert(
      //       ImsiAndSessionID(imsi, session_id));
      // }
      continue;
    }
