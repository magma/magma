  ...
  switch (fua_type) {
    case REDIRECT: {
      ...
      if (!session->is_gy_dynamic_rule_installed(rule.id())) {
        ...
        uint32_t version =
            session->insert_gy_rule(rule, DYNAMIC, lifetime, session_uc);
    ...
    case RESTRICT_ACCESS: {
      RulesToProcess to_process;
      const std::vector<PolicyRule> restrict_rules =
          action->get_restrict_rules();
      for (PolicyRule rule : restrict_rules) {
        uint32_t version =
            session->insert_gy_rule(rule, STATIC, lifetime, session_uc);
        to_process.append_versioned_policy(rule, version);
      }
      pipelined_client_->add_gy_final_action_flow(
          imsi, ip_addr, ipv6_addr, teids, msisdn, to_process);
