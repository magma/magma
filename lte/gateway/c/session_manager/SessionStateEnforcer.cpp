...
void call_back_upf(grpc::Status, magma::UPFSessionContextState response) {
  ...
  conv_session_enforcer->get_event_base().runInEventBaseThread(
      [imsi, fteid, version]() {
        ...
        conv_session_enforcer->m5g_update_session_state_message_from_upf(
...
void SessionStateEnforcer::handle_session_init_rule_updates(
  ...
  for (auto itr = itp.first; itr != itp.second; itr++) {
    ...
    // Get new TEID
    session_state.set_local_teid(pipelined_client_->get_next_teid());
    rule.mutable_pdi()->set_local_f_teid(session_state.get_local_teid());
  }
  ...
  MLOG(MINFO) << "State of session changed to "
              << session_fsm_state_to_str(session_state.get_state())
              << " Current version is " << session_state.get_current_version()
              << "  Teid " << session_state.get_local_teid();
...
void SessionStateEnforcer::m5g_start_session_termination(
   ...
   * Message to UPF need to be filled from session i.e. of session's
   * current_version_
   * current_state_ and
   * dnn can directly be used.
   */
  m5g_remove_rules_from_session_for_termination(imsi, session, dnn, session_uc);
...
void SessionStateEnforcer::m5g_remove_rules_from_session_for_termination(
    const std::string& imsi, const std::unique_ptr<SessionState>& session,
    const std::string& dnn, SessionStateUpdateCriteria& uc) {
  // remove all rules;
  MLOG(MINFO) << "Will be removing all associated rules of IMSI " << imsi;
 ...
 * upon receving message or notification from UPF
 * Defined to handle ACTIVE or INACTIVE state of session
 * based on UPF message to SMF
 ...
 */
void SessionStateEnforcer::m5g_update_session_state_message_from_upf(
  ...
  MLOG(MWARNING) << "te id (" << teid << ") found ..updating to AMF";
  auto& session       = **session_it;
  auto session_update = SessionStore::get_default_session_update(session_map);
  ...
  session->set_current_version(++cur_version, session_uc);
  MLOG(MINFO) << "state of session changed to "
              << session_fsm_state_to_str(new_state) << " current version is "
              << cur_version;
  bool update_success = session_store_.update_sessions(session_update);
  if (update_success) {
...
bool SessionStateEnforcer::static_rule_init() {
  ...
  reqpdr1.mutable_pdi()->set_src_interface(3);
  reqpdr1.mutable_pdi()->set_local_f_teid(100);
  reqpdr1.mutable_pdi()->set_net_instance("downlink");
  reqpdr1.mutable_pdi()->set_ue_ip_adr("192.168.128.11");
  reqpdr1.set_o_h_remo_desc(6);
  reqpdr1.mutable_set_gr_far()->set_far_id(1);
  // Fill the  action apply
  reqpdr1.mutable_set_gr_far()->add_far_action_to_apply(Value);
  reqpdr1.mutable_set_gr_far()->mutable_fwd_parm()->set_dest_iface(2);
  reqpdr1.mutable_set_gr_far()->mutable_fwd_parm()->set_net_instance(
      "Instance");
