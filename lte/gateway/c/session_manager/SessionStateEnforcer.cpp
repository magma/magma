...
bool SessionStateEnforcer::m5g_update_session_context(
  ...
  gnode_teid                             = update_session_rules(
      imsi, session_state, session_uc, gnb_teid_get, upf_teid_get);
  if (!gnode_teid) return false;
  ...
  for (auto itr = itp.first; itr != itp.second; itr++) {
    ...
    switch (rule.pdi().src_interface()) {
      case ACCESS:
        // Get new UPF TEID
        if (upf_teid_get) {
          upf_teid = insert_pdr_from_access(session_state, session_uc, rule);
        }
        break;
      case CORE:
        if (gnb_teid_get) {
          if (!insert_pdr_from_core(imsi, session_state, session_uc, rule))
            return 0;
        }
        break;
    }
...
bool SessionStateEnforcer::m5g_release_session(
  ...
  m5g_start_session_termination(session_map, imsi, session, pdu_id, session_uc);
  return true;
}
...
void SessionStateEnforcer::m5g_move_to_inactive_state(
  ...
  m5g_execute_state_change_action(session, INACTIVE, session_uc);
  /* Call for all rules to be de-associated from session
   * and inform to UPF
   */
...
void SessionStateEnforcer::m5g_execute_state_change_action(
  ...
  MLOG(MDEBUG) << "During " << stateStr << " state of session"
               << "of imsi: " << session->get_subscriber_id()
...
void SessionStateEnforcer::prepare_response_to_access(
  ...
  rsp->set_downlink_unit_type(Kbps_64);
  rsp->set_uplink_unit_type(Kbps_64);
  ...
  uint32_t teid_value = session_state.get_upf_local_teid();
  teid_value          = htonl(teid_value);
  char teidar[5];
  memset(teidar, '\0', 5);
...
bool SessionStateEnforcer::set_upf_node(
  ...
  MLOG(MINFO) << "Set_upf_node_id: " << upf_node_id_;
  MLOG(MINFO) << "Set_upf_n3_addr: " << upf_node_ip_addr_;
  return true;
}

std::string SessionStateEnforcer::get_upf_n3_addr() {
  return upf_node_ip_addr_;
}

std::string SessionStateEnforcer ::get_upf_node_id() {
  return upf_node_id_;
}
