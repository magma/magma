...
bool SessionStateEnforcer::m5g_update_session_context(
  ...
  gnode_teid = update_session_rules(session_state, get_gnb_teid, get_upf_teid,
                                    &session_uc);
  if (!gnode_teid) {
    return false;
  }
...
bool SessionStateEnforcer::handle_session_init_rule_updates(
   ...
   * be used by GnodeB
   */
  prepare_response_to_access(*session_state,
                             magma::lte::M5GSMCause::OPERATION_SUCCESS, upf_ip,
                             upf_teid);
  return true;
}
