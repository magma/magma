...
std::vector<uint8_t> PDUGenerator::get_conditional_attr(void) {
    ...
    std::vector<uint8_t> conditional_attr((uint8_t*)&attributes, (uint8_t*)&attributes + sizeof(struct conditional_attributes));
...
bool PDUGenerator::send_packet(const struct pcap_pkthdr* phdr, const u_char* pdata) {
  ...
  pdu.version = 2;
  pdu.pdu_type = 2;
  pdu.header_length = sizeof(struct pdu_info) + sizeof(struct conditional_attributes);
  //pdu.header_length = BASE_HEADER_LEN + sizeof(struct conditional_attributes);
