...
if [ "$MAGMA_INSTALLED" != "$SUCCESS_MESSAGE" ]; then
  ...
  su - $MAGMA_USER -c "ansible-playbook -e \"MAGMA_ROOT='/home/$MAGMA_USER/magma' OUTPUT_DIR='/tmp'\" -i $DEPLOY_PATH/agw_hosts -e \"use_master=True\" $DEPLOY_PATH/magma_deploy.yml \"MAGMA_PACKAGE=\"$MAGMA_PACKAGE\"\""
