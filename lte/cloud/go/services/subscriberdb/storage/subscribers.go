...
// InitiateUpdate prepares the db tables for a batch update.
func (l *SubStore) InitiateUpdate() error {
	txFn := func(tx *sql.Tx) (interface{}, error) {
		_, err := l.builder.
...
func (l *SubStore) InsertMany(network string, subProtos []*lte_protos.SubscriberData) error {
	// NOTE: Caller of this function should guard against the case where
	// len(subProtos) > maxEntitiesLoadSize of configurator
	if len(subProtos) == 0 {
		return nil
	}
...
// ApplyUpdate applies all subscriber data updates onto the db table and completes the batch update.
...
// GetSubscribersPage gets a page of subscriber protos based on the page token and size, and also returns
...
func (l *SubStore) GetSubscribers(network string, sids []string) ([]*lte_protos.SubscriberData, error) {
	// NOTE: Caller of this function should guard against the case where
	// len(sids) > maxProtosLoadSize of subscriberdb
	txFn := func(tx *sql.Tx) (interface{}, error) {
		rows, err := l.builder.
			Select(subscriberProtoCol).
			From(subscriberTableName).
...
// NOTE: The configurator_storage.EntityPageToken is used for simplicity & ease of transition between
// loading from configurator to loading from this cache. However, this generated token is unrelated
// to the configurator page tokens.
