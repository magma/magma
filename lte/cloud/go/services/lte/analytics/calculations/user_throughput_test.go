...
func TestUserThroughput(t *testing.T) {
	...
	for _, result := range results {
		...
		if labels[metrics.NetworkLabelName] == "testNetwork1" && labels[lte_calculations.QuantileLabel] == "0.5" {
			assert.InEpsilon(t, expTestNetwork1P50, result.GetValue(), 1e-8)
		}
		if labels[metrics.NetworkLabelName] == "testNetwork1" && labels[lte_calculations.QuantileLabel] == "0.95" {
			assert.InEpsilon(t, expTestNetwork1P95, result.GetValue(), 1e-8)
		}
		if labels[metrics.NetworkLabelName] == "testNetwork2" && labels[lte_calculations.QuantileLabel] == "0.5" {
			assert.InEpsilon(t, expTestNetwork2P50, result.GetValue(), 1e-8)
		}
		if labels[metrics.NetworkLabelName] == "testNetwork2" && labels[lte_calculations.QuantileLabel] == "0.95" {
