...
// UserThroughputCalculation computes the average and p95 quantile throughput
// for user upload and download throughput
...
// Calculate calculates the average and p95 quantile for user upload/download throughput
func (x *UserThroughputCalculation) Calculate(prometheusClient query_api.PrometheusAPI) ([]*protos.CalculationResult, error) {
	...
	for networkID, networkThroughputVec := range networkThroughputVector {
		...
		glog.Infof("Network ID %s p50th %f p95 %f direction %s", networkID, p50thValue, p95thValue, string(x.Direction))
		results = append(results, calculations.NewResult(
			p50thValue,
			SubscriberThroughputMetric,
			calculations.CombineLabels(labels, map[string]string{QuantileLabel: "0.5"})))
		results = append(results, calculations.NewResult(
			p95thValue,
			SubscriberThroughputMetric,
			calculations.CombineLabels(labels, map[string]string{QuantileLabel: "0.95"})))
	}
