...
const (
	...
	ManageNetworkNetworkProbeTasksPath       = ManageNetworkPath + obsidian.UrlSep + NetworkProbeTasks
	...
	ManageNetworkNetworkProbeDestinationsPath       = ManageNetworkPath + obsidian.UrlSep + NetworkProbeDestinations
...
func getNetworkProbeTask(c echo.Context) error {
	ent, _, _, err := getX1EntityAndIDs(c, lte.NetworkProbeTaskEntityType)
	switch {
	case err == merrors.ErrNotFound:
		return echo.ErrNotFound
	case err != nil:
	...
	ret := (&lte_models.NetworkProbeTask{}).FromBackendModels(ent)
	return c.JSON(http.StatusOK, ret)
}

func deleteNetworkProbeTask(c echo.Context) error {
	ent, networkID, _, err := getX1EntityAndIDs(c, lte.NetworkProbeTaskEntityType)
	switch {
	case err == merrors.ErrNotFound:
		return echo.ErrNotFound
	case err != nil:
...
func getNetworkProbeDestination(c echo.Context) error {
	ent, _, _, err := getX1EntityAndIDs(c, lte.NetworkProbeDestinationEntityType)
	switch {
	case err == merrors.ErrNotFound:
		return echo.ErrNotFound
	case err != nil:
	...
	ret := (&lte_models.NetworkProbeDestination{}).FromBackendModels(ent)
	return c.JSON(http.StatusOK, ret)
}

func deleteNetworkProbeDestination(c echo.Context) error {
	ent, networkID, _, err := getX1EntityAndIDs(c, lte.NetworkProbeDestinationEntityType)
	switch {
	case err == merrors.ErrNotFound:
		return echo.ErrNotFound
	case err != nil:
...
func updateNetworkProbeTask(c echo.Context) error {
	_, networkID, taskID, err := getX1EntityAndIDs(c, lte.NetworkProbeTaskEntityType)
	switch {
	case err == merrors.ErrNotFound:
		return echo.ErrNotFound
	case err != nil:
	...
	payload := &lte_models.NetworkProbeTask{}
	if err := c.Bind(payload); err != nil {
		return obsidian.HttpError(err, http.StatusBadRequest)
	}
	if err := payload.ValidateModel(); err != nil {
...
func updateNetworkProbeDestination(c echo.Context) error {
	_, networkID, taskID, err := getX1EntityAndIDs(c, lte.NetworkProbeDestinationEntityType)
	switch {
	case err == merrors.ErrNotFound:
		return echo.ErrNotFound
	case err != nil:
	...
	payload := &lte_models.NetworkProbeDestination{}
	if err := c.Bind(payload); err != nil {
		return obsidian.HttpError(err, http.StatusBadRequest)
	}
	if err := payload.ValidateModel(); err != nil {
...
func listNetworkProbeTasks(c echo.Context) error {
	...
	ret := make(map[string]*lte_models.NetworkProbeTask, len(ents))
	for _, ent := range ents {
		ret[ent.Key] = (&lte_models.NetworkProbeTask{}).FromBackendModels(ent)
	}
	return c.JSON(http.StatusOK, ret)
}
...
func listNetworkProbeDestinations(c echo.Context) error {
	...
	ret := make(map[string]*lte_models.NetworkProbeDestination, len(ents))
	for _, ent := range ents {
		ret[ent.Key] = (&lte_models.NetworkProbeDestination{}).FromBackendModels(ent)
	}
	return c.JSON(http.StatusOK, ret)
}
