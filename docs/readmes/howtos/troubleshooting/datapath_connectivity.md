...
5. From here onwards you are going to debug OVS datapath, so you need to select
   ...
   - Capturing packets on gtpu_sys_2152 device: `tcpdump -eni gtpu_sys_2152 host $UE_IP`. If you do not see any packet, it means that packets are not reaching GTP tunnel. Check S1 connectivity to debug further.
   - *NATed datapath*: Capture packets on gtp_br0 `tcpdump -eni gtp_br0 host $UE_IP`. If you dont see any packets, try debugging with `dp_probe_cli.py` utility. This utility would show which OVS table is dropping the packet.
   ...
   - *Non-NAT datapath*: You also need to check if packet are egressing on
     the SGi port. You can do so by running tcpdump on SGi port
