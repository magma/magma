...
16. First check GTP table entries using `sudo ovs-ofctl -O OpenFlow13 dump-flows gtp_br0 table=0`
17. Check whether GTP port `2152` is listening or not by using `sudo netstat -a|grep 2152`
    If it is not listening, check the `OVS` status using `sudo ovs-vsctl show`
18. If the below error is observed by running `sudo ovs-vsctl show`
...
```
   ...
   Then GTP tunnel type needs to change from `gtp` to `gtpu` in `/etc/network/interfaces.d/gtp` file.
19. OVS debug logging can be dynamically enabled by ```sudo ovs-appctl vlog/set dbg```.
...
20. To debug the traffic issues in fastpath, enable the OVS debug logging and check the logs using ```sudo dmesg```.
21. Stop and start the `OVS Service` using below commands:
