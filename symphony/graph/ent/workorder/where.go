...
func HasLinksWith(preds ...predicate.Link) predicate.WorkOrder {
	return predicate.WorkOrder(
		func(s *sql.Selector) {
			builder := sql.Dialect(s.Dialect())
			t1 := s.Table()
			t2 := builder.Select(LinksColumn).From(builder.Table(LinksTable))
			for _, p := range preds {
				p(t2)
			}
			s.Where(sql.In(t1.C(FieldID), t2))
...
func HasCommentsWith(preds ...predicate.Comment) predicate.WorkOrder {
	return predicate.WorkOrder(
		func(s *sql.Selector) {
			builder := sql.Dialect(s.Dialect())
			t1 := s.Table()
			t2 := builder.Select(CommentsColumn).From(builder.Table(CommentsTable))
			for _, p := range preds {
				p(t2)
			}
			s.Where(sql.In(t1.C(FieldID), t2))
...
func HasCheckListItemsWith(preds ...predicate.CheckListItem) predicate.WorkOrder {
	return predicate.WorkOrder(
		func(s *sql.Selector) {
			builder := sql.Dialect(s.Dialect())
			t1 := s.Table()
			t2 := builder.Select(CheckListItemsColumn).From(builder.Table(CheckListItemsTable))
			for _, p := range preds {
				p(t2)
			}
			s.Where(sql.In(t1.C(FieldID), t2))
...
func HasTechnicianWith(preds ...predicate.Technician) predicate.WorkOrder {
	return predicate.WorkOrder(
		func(s *sql.Selector) {
			builder := sql.Dialect(s.Dialect())
			t1 := s.Table()
			t2 := builder.Select(FieldID).From(builder.Table(TechnicianInverseTable))
			for _, p := range preds {
				p(t2)
			}
			s.Where(sql.In(t1.C(TechnicianColumn), t2))
		},
	)
}
