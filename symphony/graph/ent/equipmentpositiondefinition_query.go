...
func (epdq *EquipmentPositionDefinitionQuery) QueryPositions() *EquipmentPositionQuery {
	...
	t2 := epdq.sqlQuery()
	t2.Select(t2.C(equipmentpositiondefinition.FieldID))
	query.sql = builder.Select().
		From(t1).
		Join(t2).
		On(t1.C(equipmentpositiondefinition.PositionsColumn), t2.C(equipmentpositiondefinition.FieldID))
	return query
}
...
func (epdq *EquipmentPositionDefinitionQuery) QueryEquipmentType() *EquipmentTypeQuery {
	...
	t2 := epdq.sqlQuery()
	t2.Select(t2.C(equipmentpositiondefinition.EquipmentTypeColumn))
	query.sql = builder.Select(t1.Columns(equipmenttype.Columns...)...).
		From(t1).
		Join(t2).
		On(t1.C(equipmenttype.FieldID), t2.C(equipmentpositiondefinition.EquipmentTypeColumn))
	return query
}
...
func (epdq *EquipmentPositionDefinitionQuery) sqlAll(ctx context.Context) ([]*EquipmentPositionDefinition, error) {
	...
	defer rows.Close()
	var epds EquipmentPositionDefinitions
...
func (epdq *EquipmentPositionDefinitionQuery) sqlCount(ctx context.Context) (int, error) {
	rows := &sql.Rows{}
	selector := epdq.sqlQuery()
	unique := []string{equipmentpositiondefinition.FieldID}
...
func (epds *EquipmentPositionDefinitionSelect) sqlQuery() sql.Querier {
	view := "equipmentpositiondefinition_view"
