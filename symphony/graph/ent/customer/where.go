...
func HasServicesWith(preds ...predicate.Service) predicate.Customer {
	return predicate.Customer(
		func(s *sql.Selector) {
			...
			t1 := s.Table()
			t2 := builder.Table(ServicesInverseTable)
			t3 := builder.Table(ServicesTable)
			t4 := builder.Select(t3.C(ServicesPrimaryKey[1])).
				From(t3).
				Join(t2).
				On(t3.C(ServicesPrimaryKey[0]), t2.C(FieldID))
