...
func HasSurveyWith(preds ...predicate.Survey) predicate.SurveyQuestion {
	return predicate.SurveyQuestion(
		func(s *sql.Selector) {
			...
			t1 := s.Table()
			t2 := builder.Select(FieldID).From(builder.Table(SurveyInverseTable))
			for _, p := range preds {
				p(t2)
			}
			s.Where(sql.In(t1.C(SurveyColumn), t2))
		},
	)
}
...
func HasPhotoDataWith(preds ...predicate.File) predicate.SurveyQuestion {
	return predicate.SurveyQuestion(
		func(s *sql.Selector) {
			builder := sql.Dialect(s.Dialect())
			t1 := s.Table()
			t2 := builder.Select(PhotoDataColumn).From(builder.Table(PhotoDataTable))
			for _, p := range preds {
				p(t2)
			}
			s.Where(sql.In(t1.C(FieldID), t2))
