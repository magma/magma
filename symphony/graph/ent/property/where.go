...
func HasEquipmentWith(preds ...predicate.Equipment) predicate.Property {
	return predicate.Property(
		func(s *sql.Selector) {
			builder := sql.Dialect(s.Dialect())
			t1 := s.Table()
			t2 := builder.Select(FieldID).From(builder.Table(EquipmentInverseTable))
			for _, p := range preds {
				p(t2)
			}
			s.Where(sql.In(t1.C(EquipmentColumn), t2))
		},
	)
}
...
func HasServiceWith(preds ...predicate.Service) predicate.Property {
	return predicate.Property(
		func(s *sql.Selector) {
			builder := sql.Dialect(s.Dialect())
			t1 := s.Table()
			t2 := builder.Select(FieldID).From(builder.Table(ServiceInverseTable))
			for _, p := range preds {
				p(t2)
			}
			s.Where(sql.In(t1.C(ServiceColumn), t2))
		},
	)
}
...
func HasEquipmentPortWith(preds ...predicate.EquipmentPort) predicate.Property {
	return predicate.Property(
		func(s *sql.Selector) {
			builder := sql.Dialect(s.Dialect())
			t1 := s.Table()
			t2 := builder.Select(FieldID).From(builder.Table(EquipmentPortInverseTable))
			for _, p := range preds {
				p(t2)
			}
			s.Where(sql.In(t1.C(EquipmentPortColumn), t2))
		},
	)
}
...
func HasProjectWith(preds ...predicate.Project) predicate.Property {
	return predicate.Property(
		func(s *sql.Selector) {
			builder := sql.Dialect(s.Dialect())
			t1 := s.Table()
			t2 := builder.Select(FieldID).From(builder.Table(ProjectInverseTable))
			for _, p := range preds {
				p(t2)
			}
			s.Where(sql.In(t1.C(ProjectColumn), t2))
		},
	)
}
...
func HasEquipmentValueWith(preds ...predicate.Equipment) predicate.Property {
	return predicate.Property(
		func(s *sql.Selector) {
			builder := sql.Dialect(s.Dialect())
			t1 := s.Table()
			t2 := builder.Select(FieldID).From(builder.Table(EquipmentValueInverseTable))
			for _, p := range preds {
				p(t2)
			}
			s.Where(sql.In(t1.C(EquipmentValueColumn), t2))
		},
	)
}
...
func HasLocationValueWith(preds ...predicate.Location) predicate.Property {
	return predicate.Property(
		func(s *sql.Selector) {
			builder := sql.Dialect(s.Dialect())
			t1 := s.Table()
			t2 := builder.Select(FieldID).From(builder.Table(LocationValueInverseTable))
			for _, p := range preds {
				p(t2)
			}
			s.Where(sql.In(t1.C(LocationValueColumn), t2))
		},
	)
}
