	...
	for fileName := range r.MultipartForm.File {
		...
		for row := 1; ; row++ {
			...
			func() {
				...
				equip, _ := m.getOrCreateEquipment(ctx, mr, equipName, equipType, site, nil, equipProps)
				...
				position := equip.
					...
					OnlyX(ctx)
				card, _ := m.getOrCreateEquipment(ctx, mr, cardName, card16Type, nil, position, cardProps)
				if card == nil {
				...
				splitter, _ := m.getOrCreateEquipment(ctx, mr, splitterName, splitterType, manhole, nil, nil)
...
func (m *importer) getOrCreateBuildingInPort(ctx context.Context, bldg *ent.Location, sPSize int, name string) *ent.EquipmentPort {
	...
	bldgPortEquipment, _ := m.getOrCreateEquipment(ctx, mr, name, bldgPortType, bldg, nil, nil)
