	...
	return predicate.Token(
		func(s *sql.Selector) {
			step := sql.NewStep(
				...
				sql.To(UserTable, FieldID),
				sql.Edge(sql.M2O, true, UserTable, UserColumn),
			)
...
func HasUserWith(preds ...predicate.User) predicate.Token {
	return predicate.Token(
		func(s *sql.Selector) {
			...
			t1 := s.Table()
			t2 := builder.Select(FieldID).From(builder.Table(UserInverseTable))
			for _, p := range preds {
				p(t2)
			}
			s.Where(sql.In(t1.C(UserColumn), t2))
		},
	)
}
