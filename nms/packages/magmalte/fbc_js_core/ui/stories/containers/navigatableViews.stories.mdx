import Button from '../../components/design-system/Button';
import NavigatableViews from '../../components/design-system/View/NavigatableViews';
import symphony from '../../theme/symphony';
import {Meta, Preview, Story} from '@storybook/addon-docs/blocks';
import {STORY_CATEGORIES} from '../storybookUtils';
import {makeStyles} from '@material-ui/styles';

export const useStyles = makeStyles(() => ({
  root: {
    display: 'flex',
    width: '100%',
    minHeight: '400px',
    maxHeight: 'calc(100vh - 16px)',
  },
  childView: {
    display: 'flex',
    height: '100%',
    backgroundColor: symphony.palette.white,
    height: '2000px',
  },
}));

<Meta title={`${STORY_CATEGORIES.CONTAINERS}/NavigatableViews`} />

# NavigatableViews

```js
import NavigatableViews from '../../../ui/components/design-system/View/NavigatableViews';
```

Use `NavigatableViews` to display a side bar, where clicking each item shows a different view.

<Preview>
  <Story name="NavigatableViews">
    {() => {
      const classes = useStyles();
      const views = [
        {
          menuItem: {
            label: 'Home',
            tooltip: 'Go to home page',
          },
          component: {
            header: {
              title: 'Home',
              subtitle:
                'The Company is a secret group of multinational corporate alliances known only by those who work for them or oppose them. Its influence and power over individuals stretches to the White House, controlling every decision the country makes.',
              actionButtons: [
                <Button
                  onClick={() =>
                    console.log(
                      'If this is home, we probably need to be able to open the door...',
                    )
                  }>
                  Open Door
                </Button>,
              ],
            },
            children: <div className={classes.childView} />,
          },
        },
        {
          menuItem: {
            label: 'Products',
            tooltip: 'See what we offer',
          },
          component: {
            header: {
              title: 'Products',
              subtitle: 'Ever heard of Sona..?',
              actionButtons: [
                <Button onClick={() => console.log('I want to buy things!!')}>
                  Purchase
                </Button>,
              ],
            },
            children: <div className={classes.childView} />,
          },
        },
        {
          menuItem: {
            label: 'Downloads',
            tooltip: 'Drivers, Guids and stuff',
          },
          component: {
            header: {
              title: 'Downloads',
              actionButtons: [
                <Button onClick={() => console.log('sh...')}>
                  Go search!
                </Button>,
              ],
            },
            children: <div className={classes.childView} />,
          },
        },
        {
          menuItem: {
            label: 'About',
            tooltip: 'Who are we',
          },
          component: {
            header: {
              title: 'About',
              actionButtons: [
                <Button onClick={() => console.log('Send some mail..')}>
                  Contact
                </Button>,
              ],
            },
            children: <div className={classes.childView} />,
          },
        },
      ];
      return (
        <div className={classes.root}>
          <NavigatableViews header="The Company" views={views} />
        </div>
      );
    }}
  </Story>
</Preview>
