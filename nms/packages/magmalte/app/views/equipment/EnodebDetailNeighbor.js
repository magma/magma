...
type FreqProps = {
  type?: null,
  list?: Array<neighbor_frequency>,
  onSave: neighbor_frequency => void,
};
type NeighborFreqKey = $Keys<neighbor_frequency>;
export function FreqFormDialog(props: FreqProps) {
  const type = props.type || 'add',
    row: neighbor_frequency = {
      enable: true,
      index: 0,
      q_offset_range: -22,
      t_reselection_eutra_sf_medium: 25,
    },
    classes = useStyles();
  let has = false;
  if (props.list) {
    const idxes = props.list.map(function (m) {
        return parseInt(m.index) - 0;
      }),
  ...
  const handleChange = (key: NeighborFreqKey, val) => {
          ...
          <Box
            ...
            <FormControl className={classes.formControl}>
              ...
              <Select
                ...
                variant="standard"
                name="q_offset_range"
                value={parseInt(form.q_offset_range)}
                onChange={({target}) =>
            ...
            <FormControl className={classes.formControl}>
              ...
              <Select
                ...
                value={parseInt(form.t_reselection_eutra_sf_medium)}
                onChange={({target}) =>
...
type CellProps = {
  type?: null,
  list?: Array<neighbor_cell>,
  onSave: neighbor_cell => void,
};
type NeighborCellKey = $Keys<neighbor_cell>;
export function CellFormDialog(props: CellProps) {
  const type = props.type || 'add',
    row: neighbor_cell = {
      enable: true,
      index: 0,
  ...
  const handleChange = (key: NeighborCellKey, val) => {
    setCellForm({...cellForm, [(key: string)]: val});
  };
          ...
          <Box
            ...
            <FormControl className={classes.formControl}>
              ...
              <Select
                ...
                variant="standard"
                name="q_offset"
                value={parseInt(cellForm.q_offset)}
...
export function NeighborFreqConfig() {
  ...
  const [dataList, dispatch] = useReducer(
    reducer,
    enbInfo.enb?.enodeb_config?.managed_config?.neighbor_frequency_list ?? [],
  );
                  ...
                  try {
                    remove(o, function (p) {
                      ...
                      if (
                        ...
                        enbInfo.enb.enodeb_config.managed_config.neighbor_frequency_list = list;
                      }
                      if (enbInfo.enb.config) {
                        enbInfo.enb.config.neighbor_frequency_list = list;
                      }
    ...
    <div>
      ...
      <div style={{width: '100%'}}>
        <ActionTable
          data={dataList}
          columns={columns}
          options={{actionsColumnIndex: 0, pageSizeOptions: [5, 10, 50]}}
        />
      </div>
    </div>
  );
...
export function NeighborCellConfig() {
  ...
  const [cells, dispatch] = useReducer(
    cellReducer,
    enbInfo.enb?.enodeb_config?.managed_config?.neighbor_cell_list ?? [],
  );
  const enqueueSnackbar = useEnqueueSnackbar();
                  ...
                  try {
                    remove(o, function (p) {
                      ...
                      if (
                        ...
                        enbInfo.enb.enodeb_config.managed_config.neighbor_cell_list = list;
                      }
                      if (enbInfo.enb.config) {
                        enbInfo.enb.config.neighbor_cell_list = list;
                      }
                      ctx.setState(enbInfo.enb.serial, {
                        ...enbInfo,
                        enb: enbInfo.enb,
