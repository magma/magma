...
func GetServiceAddress(service string) (string, error) {
	return platform_registry.GetServiceAddress(service, protos2.ServiceType_SOUTHBOUND)
}
...
func GetConnection(service string) (*grpc.ClientConn, error) {
	return platform_registry.GetConnection(service, protos2.ServiceType_SOUTHBOUND)
}
...
func GetConnectionWithTimeout(service string, timeout time.Duration) (*grpc.ClientConn, error) {
	return platform_registry.GetConnectionWithTimeout(service, timeout, protos2.ServiceType_SOUTHBOUND)
}
