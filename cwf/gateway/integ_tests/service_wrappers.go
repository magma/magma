...
func sendPolicyReAuthRequest(instanceName string, target *fegprotos.PolicyReAuthTarget) (*fegprotos.PolicyReAuthAnswer, error) {
...
func sendPolicyAbortSession(instanceName string, target *fegprotos.PolicyAbortSessionRequest) (*fegprotos.PolicyAbortSessionResponse, error) {
	cli, err := getPCRFClient(instanceName)
	if err != nil {
		return nil, err
	}
...
// Input: The subscriber data which will be added.
func addSubscriberToPCRF(instanceName string, sub *lteprotos.SubscriberID) error {
	cli, err := getPCRFClient(instanceName)
	if err != nil {
		return err
	}
...
func clearSubscribersFromPCRF(instanceName string) error {
	cli, err := getPCRFClient(instanceName)
	if err != nil {
		return err
	}
...
func addPCRFRules(instanceName string, rules *fegprotos.AccountRules) error {
	cli, err := getPCRFClient(instanceName)
	if err != nil {
		return err
	}
...
func addPCRFUsageMonitors(instanceName string, monitorInfo *fegprotos.UsageMonitorConfiguration) error {
	cli, err := getPCRFClient(instanceName)
	if err != nil {
		return err
	}
...
func usePCRFMockDriver(instanceName string) error {
	cli, err := getPCRFClient(instanceName)
	if err != nil {
		return err
	}
...
func clearPCRFMockDriver(instanceName string) error {
	cli, err := getPCRFClient(instanceName)
	if err != nil {
		return err
	}
...
func setPCRFExpectations(instanceName string, expectations []*fegprotos.GxCreditControlExpectation, defaultAnswer *fegprotos.GxCreditControlAnswer) error {
	cli, err := getPCRFClient(instanceName)
	if err != nil {
		return err
	}
...
func getAssertExpectationsResult(instanceName string, ) ([]*fegprotos.ExpectationResult, []*fegprotos.ErrorByIndex, error) {
	cli, err := getPCRFClient(instanceName)
	if err != nil {
		return nil, nil, nil
	}
...
func setNewOCSConfig(instanceName string, ocsConfig *fegprotos.OCSConfig) error {
...
// Input: The subscriber data which will be added.
func addSubscriberToOCS(instanceName string, sub *lteprotos.SubscriberID) error {
	cli, err := getOCSClient(instanceName)
	if err != nil {
		return err
	}
...
func clearSubscribersFromOCS(instanceName string, ) error {
	cli, err := getOCSClient(instanceName)
	if err != nil {
		return err
	}
...
func setCreditOnOCS(instanceName string, creditInfo *fegprotos.CreditInfo) error {
	cli, err := getOCSClient(instanceName)
	if err != nil {
		return err
	}
...
func sendChargingReAuthRequest(instanceName string, imsi string, ratingGroup uint32) (*fegprotos.ChargingReAuthAnswer, error) {
	cli, err := getOCSClient(instanceName)
	if err != nil {
		return nil, err
	}
