...
func TestDiameterIdentity(t *testing.T) {
	s := DiameterIdentity("hello")
	...
	if s.Type() != DiameterIdentityType {
		t.Fatalf("Unexpected type. Want %d, have %d",
			DiameterIdentityType, s.Type())
	}
	if len(s.String()) == 0 {
...
func TestDecodeDiameterIdentity(t *testing.T) {
	b := []byte{
		0x68, 0x65, 0x6c, 0x6c, 0x6f, 0x2c,
		0x20, 0x77, 0x6f, 0x72, 0x6c, 0x64,
	}
	s, err := DecodeDiameterIdentity(b)
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal([]byte(s.(DiameterIdentity)), b) {
	...
	if v := string(s.(DiameterIdentity)); v != "hello, world" {
