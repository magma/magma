...
func startServerWithExpectations(
	...
	expectations []*fegprotos.Expectation,
	failureBehavior fegprotos.SetExpectationsRequest_UnexpectedRequestBehavior,
	...
	go func() {
		...
		pcrf.SetExpectations(ctx, &fegprotos.SetExpectationsRequest{
...
func TestPCRFExpectations(t *testing.T) {
	...
	expectedReqHappy := &fegprotos.GxCreditControlRequest{
		Imsi:          test.IMSI1,
		RequestType:   fegprotos.CCRequestType_INITIAL,
		RequestNumber: 1,
	}
	answerHappy := &fegprotos.GxCreditControlAnswer{
		RequestType:   fegprotos.CCRequestType_INITIAL,
		RequestNumber: 1,
		RuleInstalls: &fegprotos.RuleInstalls{
			RuleNames:     []string{"rule1", "rule2"},
			RuleBaseNames: []string{"base1", "base2"},
		...
		UsageMonitoringInfos: []*fegprotos.UsageMonitoringInformation{
			{
	...
	happyExpectation := fegprotos.NewGxCreditControlExpectation().Expect(expectedReqHappy).Return(answerHappy)

	// E/A that will not be met
	expectedReqNotMet := &fegprotos.GxCreditControlRequest{
		Imsi:          test.IMSI1,
		RequestType:   fegprotos.CCRequestType_UPDATE,
		RequestNumber: 2,
	}
	answerNotMet := &fegprotos.GxCreditControlAnswer{
		RequestType:   fegprotos.CCRequestType_UPDATE,
		RequestNumber: 2,
	...
	pcrf := startServerWithExpectations(
		...
		[]*fegprotos.Expectation{happyExpectation, expectationNotMet},
		fegprotos.SetExpectationsRequest_CONTINUE_WITH_DEFAULT_ANSWER,
		defaultCCA)
	...
	assertCCAIsEqualToExpectation(t, actualAnswer, answerHappy)
	...
	assertCCAIsEqualToExpectation(t, unexpectedAnswer, defaultCCA)

	res, err := pcrf.AssertExpectations(context.Background(), &orcprotos.Void{})
	assert.NoError(t, err)
	assert.Empty(t, res.GyUnexpectedRequests)
	assertCCAIsEqualToExpectation(t, unexpectedAnswer, defaultCCA)
	expectedResult := []*fegprotos.ExpectationResult{
		{
			ExpectationMet: true,
			Expectation:    happyExpectation,
		},
		{
			ExpectationMet: false,
			Expectation:    expectationNotMet,
		},
	}
	assert.ElementsMatch(t, expectedResult, res.Results)
}

func assertCCAIsEqualToExpectation(t *testing.T, actual *gx.CreditControlAnswer, expectation *fegprotos.GxCreditControlAnswer) {
