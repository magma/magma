...
class HtmlReport():
    ...
    def generateHeader(self):
        ...
        self.file.write(
            '  <title>MAGMA/OAI Core Network Build Results for ' +
            self.job_name +
            ' job build #' +
            self.job_id +
            '</title>\n')
        ...
        self.file.write('     </td>\n')
        self.file.write(
            '     <td align = "center" style="border-collapse: collapse; border: none; vertical-align: center;">\n')
        self.file.write(
    ...
    def analyze_compile_log(self, nfType):
        ...
        if os.path.isfile(cwd + '/archives/' + logFileName):
            ...
            with open(cwd + '/archives/' + logFileName, 'r') as logfile:
                for line in logfile:
                    result = re.search(section_start_pattern, line)
                    if (result is not None) and not section_done and (
                            re.search('cmake', line) is not None):
                        section_status = True
                    result = re.search(section_end_pattern, line)
