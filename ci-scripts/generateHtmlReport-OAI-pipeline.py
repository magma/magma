...
class HtmlReport():
    ...
    def generateHeader(self):
        ...
        header += '  <title>MAGMA/OAI Core Network Build Results for ' +
        ...
        header += '       <b><font size = "6">Job Summary -- Job: ' + self.job_name +
                        ' -- Build-ID: <a href="' + self.job_url + '">' + self.job_id + '</a></font></b>\n'
        header += '     </td>\n'
    ...
    def generateBuildSummary(self):
        ...
        returnString += '      <td><a href="' + \
            self.git_url + '">' + self.git_url + '</a></td>\n'
        returnString += '    </tr>\n'
        if self.git_merge_request:
            returnString += '    <tr>\n'
    ...
    def buildSummaryHeader(self):
        ...
        if os.path.isfile(cwd + '/archives/build_magma_mme.log'):
            self.file.write(
                '       <th>MAGMA - OAI MME cNF (Ubuntu-18)</th>\n')
    ...
    def buildCompileRows(self):
        self.file.write('    <tr>\n')
        self.file.write(
            '      <td rowspan=2 bgcolor="lightcyan" ><b>magma-common</b> Compile / Build</td>\n')
        ...
        self.file.write('    <tr>\n')
        self.file.write(
            '      <td rowspan=2 bgcolor="lightcyan" ><b>magma-oai-mme</b> Compile / Build</td>\n')
        ...
        self.file.write('    <tr>\n')
        self.file.write(
            '      <td rowspan=2 bgcolor="lightcyan" ><b>magma-sctpd</b> Compile / Build</td>\n')
                ...
                with open(cwd + '/archives/' + logFileName, 'r') as logfile:
                    for line in logfile:
                        ...
                        if section_status:
                            if nfType == 'OAI-COMMON':
                                result = re.search(
                                    'Linking CXX static library eventd/libEVENTD.a', line)
                            if nfType == 'OAI-MME':
                                result = re.search(
                                    'Linking CXX executable oai_mme/mme', line)
                            if nfType == 'SCTPD':
                                result = re.search(
                                    'Linking CXX executable sctpd', line)
                            if result is not None:
                                status = True
                    logfile.close()
                if status:
        ...
        for logFileName in logFileNames:
            ...
            if os.path.isfile(cwd + '/archives/' + logFileName):
                ...
                if nb_warnings == 0 and nb_errors == 0:
                    ...
                    cell_msg = '       <td bgcolor="Tomato"><pre style="border:none; background-color:Tomato"><b>'
                if nb_errors > 0:
                    cell_msg += str(nb_errors) + \
                        ' errors found in compile log\n'
                cell_msg += str(nb_warnings) + \
                    ' warnings found in compile log\n'
                if nb_notes > 0:
...
while len(argvs) > 1:
    ...
    if re.match('^\-\-help$', myArgv, re.IGNORECASE):
        ...
        # Not Mandatory
        matchReg = re.match('^\-\-git_src_commit_msg=(.+)$',
        ...
        matchReg = re.match('^\-\-git_merge_request=(.+)$',
                            myArgv, re.IGNORECASE)
