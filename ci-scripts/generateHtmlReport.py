...
class HtmlReport():
            ...
            if int(nb_fail.strip()) == 0:
                ...
                if self.git_merge_request:
                    self.file.write(
                        '    <strong>' +
                        nb_fail.strip() +
                        ' modified files in Merge-Request DO NOT follow OAI rules. <span class="glyphicon glyphicon-warning-sign"></span> -> (' +
                        nb_total.strip() +
                        ' were checked)</strong>\n')
                else:
                    self.file.write(
                        '    <strong>' +
                        nb_fail.strip() +
                        ' files in repository DO NOT follow OAI rules. <span class="glyphicon glyphicon-warning-sign"></span> -> (' +
                        nb_total.strip() +
                        ' were checked)</strong>\n')
                self.file.write('  </div>\n')
    ...
    def analyze_sca_log(self):
        ...
        if os.path.isfile(cwd + '/archives/cppcheck.xml'):
            ...
            if (nb_errors > 0) or (nb_warnings > 0):
                ...
                self.file.write(
                    '   <td>' +
                    str(nb_bufferAccessOutOfBounds) +
                    '</td>\n')
                self.file.write('   <td>N/A</td>\n')
                self.file.write('   </tr>\n')
                self.file.write('   <tr>\n')
                ...
                nb_others = nb_uninitvar + nb_uninitStructMember + nb_memleak + nb_doubleFree + nb_resourceLeak + nb_nullPointer + \
                    nb_arrayIndexOutOfBounds + nb_arrayIndexOutOfBounds + nb_bufferAccessOutOfBounds + nb_unknownEvaluationOrder
    ...
    def analyze_vagrant_make_run_log(self, vmType, kind):
        ...
        if kind == 'agw1-no-s11':
            logFileName = vmType.lower().replace(
                'magma', 'magma_vagrant') + '_make_run.log'
        if kind == 'agw1-with-s11':
            logFileName = vmType.lower().replace(
                'magma', 'magma_vagrant') + '_make_run2.log'
    ...
    def s1apTesterTable(self, kind):
        ...
        self.file.write(
            ...
            button_ref +
            '">More details on S1AP Tester checks</button>\n')
        ...
        if kind == 'agw1-no-s11' and os.path.isfile(
                cwd + '/archives/magma_vagrant_make_coverage_oai.log'):
