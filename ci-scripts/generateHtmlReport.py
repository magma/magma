...
class HtmlReport():
    ...
    def generateHeader(self):
        ...
        self.file.write(
            '  <title>MAGMA/OAI Core Network Test Results for '
            + self.job_name + ' job build #' + self.job_id + '</title>\n',
        )
        self.file.write('</head>\n')
        ...
        self.file.write(
            '       <b><font size = "6">Job Summary -- Job: ' + self.job_name
            + ' -- Build-ID: <a href="' + self.job_url
            + '">' + self.job_id + '</a></font></b>\n',
        )
        self.file.write('     </td>\n')
        self.file.write('   </tr>\n')
    ...
    def coding_formatting_log(self):
        cwd = os.getcwd()
        self.file.write(
            '  <h2>OAI Coding / Formatting Guidelines Check</h2>\n',
        )
        if os.path.isfile(cwd + '/oai_rules_result.txt'):
            cmd = 'grep NB_FILES_FAILING_CHECK ' + cwd + \
                '/oai_rules_result.txt | sed -e "s#NB_FILES_FAILING_CHECK=##"'
            nb_fail = subprocess.check_output(
                cmd, shell=True, universal_newlines=True,
            )
            cmd = 'grep NB_FILES_CHECKED ' + cwd + \
                '/oai_rules_result.txt | sed -e "s#NB_FILES_CHECKED=##"'
            nb_total = subprocess.check_output(
                cmd, shell=True, universal_newlines=True,
            )
            if int(nb_fail.strip()) == 0:
                ...
                if self.git_merge_request:
                    self.file.write(
                        '    <strong>All modified files in Merge-Request follow OAI rules. <span class="glyphicon glyphicon-ok-circle"></span> -> ('
                        + nb_total.strip() + ' were checked)</strong>\n',
                    )
                else:
                    self.file.write(
                        '    <strong>All files in repository follow OAI rules. <span class="glyphicon glyphicon-ok-circle"></span> -> ('
                        + nb_total.strip() + ' were checked)</strong>\n',
                    )
                self.file.write('  </div>\n')
            else:
                ...
                if self.git_merge_request:
                    self.file.write(
                        '    <strong>' + nb_fail.strip(
                        ) + ' modified files in Merge-Request DO NOT follow OAI rules. <span class="glyphicon glyphicon-warning-sign"></span> -> (' + nb_total.strip() + ' were checked)</strong>\n',
                    )
                else:
                    self.file.write(
                        '    <strong>' + nb_fail.strip(
                        ) + ' files in repository DO NOT follow OAI rules. <span class="glyphicon glyphicon-warning-sign"></span> -> (' + nb_total.strip() + ' were checked)</strong>\n',
                    )
                self.file.write('  </div>\n')

                if os.path.isfile(cwd + '/oai_rules_result_list.txt'):
                    self.file.write(
                        '  <button data-toggle="collapse" data-target="#oai-formatting-details">More details on formatting check</button>\n',
                    )
                    self.file.write(
                        '  <div id="oai-formatting-details" class="collapse">\n',
                    )
                    self.file.write(
                        '  <p>Please apply the following command to this(ese) file(s): </p>\n',
                    )
                    self.file.write(
                        '  <p style="margin-left: 30px"><strong><code>clang-format -i filename(s)</code></strong></p>\n',
                    )
                    self.file.write(
                        '  <table class="table-bordered" width = "60%" align = "center" border = 1>\n',
                    )
                    self.file.write(
                        '    <tr><th bgcolor = "lightcyan" >Filename</th></tr>\n',
                    )
            ...
            self.file.write(
                '   <strong>Was NOT performed (with CLANG-FORMAT tool). <span class="glyphicon glyphicon-ban-circle"></span></strong>\n',
            )
            self.file.write('  </div>\n')
    ...
    def analyze_sca_log(self):
        ...
        if os.path.isfile(cwd + '/archives/cppcheck.xml'):
            ...
            if (nb_errors == 0) and (nb_warnings == 0):
                self.file.write('   <div class="alert alert-success">\n')
                self.file.write(
                    '   <strong>CPPCHECK found NO error and NO warning <span class="glyphicon glyphicon-ok-circle"></span></strong>\n',
                )
                self.file.write('   </div>\n')
            elif (nb_errors == 0):
                self.file.write('   <div class="alert alert-warning">\n')
                self.file.write(
                    ...
                    + str(nb_warnings)
                    + ' warnings <span class="glyphicon glyphicon-warning-sign"></span></strong>\n',
                )
                self.file.write('   </div>\n')
            else:
                self.file.write('   <div class="alert alert-danger">\n')
                self.file.write(
                    '   <strong>CPPCHECK found ' + str(nb_errors) + ' errors and ' + str(
                        nb_warnings,
                    ) + ' warnings <span class="glyphicon glyphicon-ban-circle"></span></strong>\n',
                )
                self.file.write('   </div>\n')
            if (nb_errors > 0) or (nb_warnings > 0):
                self.file.write(
                    '   <button data-toggle="collapse" data-target="#oai-cppcheck-details">More details on CPPCHECK results</button>\n',
                )
                self.file.write(
                    '   <div id="oai-cppcheck-details" class="collapse">\n',
                )
                self.file.write('   <br>\n')
                self.file.write(
                    '   <table class="table-bordered" width = "80%" align = "center" border = "1">\n',
                ...
                self.file.write(
                    '   <td>' + str(nb_uninitStructMember) + '</td>\n',
                )
                self.file.write('   <td>N/A</td>\n')
                self.file.write('   </tr>\n')
                self.file.write('   <tr>\n')
                ...
                self.file.write(
                    '   <td>' + str(nb_arrayIndexOutOfBounds) + '</td>\n',
                )
                self.file.write('   <td>N/A</td>\n')
                self.file.write('   </tr>\n')
                self.file.write('   <tr>\n')
                self.file.write(
                    '   <td>Buffer is accessed out of bounds</td>\n',
                )
                self.file.write(
                    '   <td>' + str(nb_bufferAccessOutOfBounds) + '</td>\n',
                )
                self.file.write('   <td>N/A</td>\n')
                self.file.write('   </tr>\n')
                self.file.write('   <tr>\n')
                self.file.write(
                    '   <td>Expression depends on order of evaluation of side effects</td>\n',
                )
                self.file.write(
                    '   <td>' + str(nb_unknownEvaluationOrder) + '</td>\n',
                )
                self.file.write('   <td>N/A</td>\n')
                self.file.write('   </tr>\n')
                self.file.write('   <tr>\n')
                self.file.write('   <td>Others</td>\n')
                nb_others = nb_uninitvar + nb_uninitStructMember + nb_memleak + nb_doubleFree + nb_resourceLeak + nb_nullPointer + \
                    nb_arrayIndexOutOfBounds + nb_arrayIndexOutOfBounds + \
                    nb_bufferAccessOutOfBounds + nb_unknownEvaluationOrder
                ...
                self.file.write('   <br>\n')
                self.file.write(
                    '   <p>Full details in artifact (cppcheck.xml) </p>\n',
                )
                self.file.write(
                    '   <p style="margin-left: 30px">Graphical Interface tool : <strong><code>cppcheck-gui -l cppcheck.xml</code></strong></p>\n',
                )
                self.file.write('   <br>\n')
                self.file.write('   </div>\n')
        else:
            self.file.write('  <div class="alert alert-danger">\n')
            self.file.write(
                '   <strong>Was NOT performed (with CPPCHECK tool). <span class="glyphicon glyphicon-ban-circle"></span></strong>\n',
            )
            self.file.write('  </div>\n')
    ...
    def addPagination(self):
        ...
        if agw1_noS11_status:
            self.file.write(
                '     <li class="page-item"><a class="page-link" href="#agw1-no-s11-details">AGW1-no-S11 <font color = "Green"><span class="glyphicon glyphicon-ok-sign"></span></font></a></li>\n',
            )
        else:
            self.file.write(
                '     <li class="page-item"><a class="page-link" href="#agw1-no-s11-details">AGW1-no-S11 <font color = "Red"><span class="glyphicon glyphicon-remove-sign"></span></font></a></li>\n',
            )
        ...
        if agw1_wiS11_status:
            self.file.write(
                '     <li class="page-item"><a class="page-link" href="#agw1-wi-s11-details">AGW1-with-S11 <font color = "Green"><span class="glyphicon glyphicon-ok-sign"></span></font></a></li>\n',
            )
        else:
            self.file.write(
                '     <li class="page-item"><a class="page-link" href="#agw1-wi-s11-details">AGW1-with-S11 <font color = "Red"><span class="glyphicon glyphicon-remove-sign"></span></font></a></li>\n',
            )
        self.file.write('  </ul>\n')
        self.file.write('\n')
    ...
    def analyze_vagrant_up_log(self, vmType):
        ...
        if os.path.isfile(cwd + '/archives/' + logFileName):
            ...
            cell_msg = '      <td bgcolor="Tomato"><pre style="border:none; background-color:Tomato"><b>'
            cell_msg += 'KO: logfile (' + logFileName + \
                ') not found</b></pre></td>\n'
    ...
    def makeRunRow(self, kind):
        self.file.write('    <tr>\n')
        self.file.write(
            '      <td bgcolor="lightcyan" >Make Run Results</td>\n',
        )
    ...
    def statusCheckRow(self, kind):
        self.file.write('    <tr>\n')
        self.file.write(
            '      <td bgcolor="lightcyan" >Magma Services Status</td>\n',
        )
    ...
    def s1apTesterTable(self, kind):
        ...
        if os.path.isfile(cwd + '/archives/' + s1ap_logfile):
            ...
            if int(found) > 0:
                self.file.write(
                    '  <h3>S1AP Tester Summary -- STATUS is <font color = "Green"><span class="glyphicon glyphicon-ok-sign"></span></font></h3>\n',
                )
            else:
                self.file.write(
                    '  <h3>S1AP Tester Summary -- STATUS is <font color = "Red"><span class="glyphicon glyphicon-remove-sign"></span></font></h3>\n',
                )
        else:
            self.file.write(
                '  <h3>S1AP Tester Summary -- STATUS is <font color = "Gray"><span class="glyphicon glyphicon-question-sign"></span></font></h3>\n',
            )

        self.file.write('  <br>\n')
        self.file.write(
            '  <button data-toggle="collapse" data-target="#'
            + button_ref + '">More details on S1AP Tester checks</button>\n',
        )
        ...
        if os.path.isfile(cwd + '/archives/' + s1ap_logfile):
            ...
            with open(cwd + '/archives/' + s1ap_logfile, 'r') as logfile:
                for line in logfile:
                    ...
                    if result is not None:
                        ...
                        global_test_duration += float(
                            test_duration.replace('s', ''),
                        )
                    result = re.search(
                        '^OK$|^FAILED|^Killed by signal 15', line,
                    )
                    if result is not None:
                        listTests = '    <tr>\n'
            ...
            self.file.write('     <tr>\n')
            self.file.write(
                '       <td bgcolor = "LightBlue" ><b>FINAL STATUS</b></td>\n',
            )
            if status:
                self.file.write(
                    '       <td align = "left" bgcolor = "Green"><b><font color = "white">OK</font></b></td>\n',
                )
                self.file.write(
                    '       <td align = "left" bgcolor = "Green"><b><font color = "white">'
                    + str(global_test_duration) + 's</font></b></td>\n',
                )
            else:
                self.file.write(
                    '       <td align = "left" bgcolor = "Red"><b><font color = "white">FAIL</font></b></td>\n',
                )
                self.file.write(
                    '       <td align = "left" bgcolor = "Red"><b><font color = "white">'
                    + str(global_test_duration) + 's</font></b></td>\n',
                )
            self.file.write('     </tr>\n')
        else:
            ...
            self.file.write(
                '       <td colspan=3><b>KO: logfile ('
                + s1ap_logfile + ') not found</b></td>\n',
            )
            self.file.write('     </tr>\n')
            ...
            with open(cwd + '/archives/magma_vagrant_make_coverage_oai.log', 'r') as logfile:
                ...
                for line in logfile:
                    ...
                    if result is not None:
                        ...
                        self.file.write('     <tr>\n')
                        self.file.write(
                            '       <td colspan = 3 bgcolor = "DarkSlateGrey"><b><font color = "white">Overall coverage rate</font></b></td>\n',
                        )
                        self.file.write('     </tr>\n')
                    if cov_global_rate:
                        result = re.search(
                            ' lines.*: (.+)% \(([0-9]+) of ([0-9]+) lines', line,
                        )
                        if result is not None:
                            ...
                            self.file.write(
                                '       <td bgcolor = "DarkSlateGrey"><b><font color = "white">'
                                + percentage + '%</font></b></td>\n',
                            )
                            self.file.write(
                                '       <td bgcolor = "DarkSlateGrey"><b><font color = "white">'
                                + reached_line_nb + ' over ' + total_line_nb + '</font></b></td>\n',
                            )
                            self.file.write('     </tr>\n')
                        result = re.search(
                            ' functions.*: (.+)% \(([0-9]+) of ([0-9]+) functions', line,
                        )
                        if result is not None:
                            percentage = result.group(1)
                            ...
                            self.file.write(
                                '       <td bgcolor = "DarkSlateGrey"><b><font color = "white">'
                                + reached_function_nb + ' over ' + total_function_nb + '</font></b></td>\n',
                            )
                            self.file.write('     </tr>\n')
                        ...
                        if result is not None:
                            self.file.write('     <tr>\n')
                            self.file.write(
                                '       <td colspan = 3>More details in artifact magma_logs.zip (code_coverage.zip)</td>\n',
                            )
                            self.file.write('     </tr>\n')
            logfile.close()
