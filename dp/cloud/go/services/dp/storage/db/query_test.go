...
func (s *QueryTestSuite) TestCreate() {
	...
	for _, tt := range testCases {
		s.Run(tt.name, s.inTransaction(func() {
			id := s.whenModelIsInserted(tt.input)
...
func (s *QueryTestSuite) TestUpdate() {
	err := s.resourceManager.InTransaction(func() {
		id := s.whenModelIsInserted(getSomeModel())
...
func (s *QueryTestSuite) whenModelIsInserted(model db.Model) int64 {
	id, err := db.NewQuery().
		WithBuilder(s.resourceManager.GetBuilder()).
		From(model).
		Select(db.NewExcludeMask()).
...
func (s *someModel) Fields() map[string]db.Field {
	return map[string]db.Field{
		"id":    db.IntField{X: &s.id},
		"value": db.FloatField{X: &s.value},
		"name":  db.StringField{X: &s.name},
		"flag":  db.BoolField{X: &s.flag},
		"date":  db.TimeField{X: &s.date},
	}
}
...
func (o *otherModel) Fields() map[string]db.Field {
	return map[string]db.Field{
		"id":      db.IntField{X: &o.id},
		"some_id": db.NullIntField{X: &o.someId},
		"value":   db.NullFloatField{X: &o.value},
		"name":    db.NullStringField{X: &o.name},
		"flag":    db.NullBoolField{X: &o.flag},
		"date":    db.NullTimeField{X: &o.date},
	}
}
...
func (a *anotherModel) Fields() map[string]db.Field {
	return map[string]db.Field{
		"id":       db.IntField{X: &a.id},
		"other_id": db.NullIntField{X: &a.otherId},
	}
}
