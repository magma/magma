...
class DefaultResponseDBProcessorTestCase(LocalDBTestCase):
    ...
    def test_processor_splits_sas_response_into_separate_db_objects_and_links_them_with_requests(
        ...
        # When
        self._process_response(request_type_name=request_type_name, response=response, db_requests=db_requests)
    ...
    def test_grant_state_after_response(
        ...
        # Given
        db_requests = self._create_db_requests(request_type_name, requests_fixtures)
        response = self._prepare_response_from_db_requests(db_requests, response_code=response_code)
    ...
    def test_last_used_eirp_is_reset(
        ...
        # Given
        db_requests = self._create_db_requests(request_type_name, requests_fixtures)
        granted_state = self.session.query(DBGrantState).filter(DBGrantState.name == GrantStates.GRANTED.value).one()

        response = self._prepare_response_from_db_requests(db_requests, response_code=response_code)
        response_payload = response.json()[request_response[request_type_name]]

        for resp in response_payload:
            cbsd = self.session.query(DBCbsd).filter(DBCbsd.cbsd_id == resp["cbsdId"]).first()
            channel = self._create_channel(cbsd, 1, 2, last_used_max_eirp=last_used_eirp_value)
            self._create_grant(resp["grantId"], channel=channel, cbsd=cbsd, state=granted_state)

        grants_query = self.session.query(DBGrant)

        for grant in grants_query.all():
            self.assertEqual(last_used_eirp_value, grant.channel.last_used_max_eirp)
        ...
        # Then
        for grant in grants_query.all():
            self.assertEqual(final_last_used_eirp_value, grant.channel.last_used_max_eirp)
    ...
    def test_cbsd_state_after_registration_response(self, sas_response_code, expected_cbsd_state):
        # Given
        db_requests = self._create_db_requests(REGISTRATION_REQ, registration_requests)
        response = self._prepare_response_from_db_requests(db_requests, sas_response_code)

        # When
        self._process_response(request_type_name=REGISTRATION_REQ, response=response, db_requests=db_requests)
    ...
    def test_cbsd_state_after_deregistration_response(self, sas_response_code, expected_cbsd_state):
        # Given
        db_requests = self._create_db_requests(DEREGISTRATION_REQ, deregistration_requests)
        self._set_cbsds_to_state(CbsdStates.REGISTERED.value)
        response = self._prepare_response_from_db_requests(db_requests, sas_response_code)

        # When
        self._process_response(request_type_name=DEREGISTRATION_REQ, response=response, db_requests=db_requests)
        states = [req.cbsd.state for req in db_requests]
    ...
    def test_channels_created_after_spectrum_inquiry_response(self, response_fixture_payload, expected_channels_count):
        ...
        response = self._prepare_response_from_payload(SPECTRUM_INQ_REQ, response_fixture_payload)
    ...
    def test_old_channels_deleted_after_spectrum_inquiry_response(self):
        ...
        response = self._prepare_response_from_payload(SPECTRUM_INQ_REQ, zero_channels_for_one_cbsd)
    ...
    def _verify_requests_number_and_state(self, db_requests, nr_of_requests, desired_state="processed"):
        ...
        self.assertListEqual([desired_state] * nr_of_requests,
    ...
    def _setup_channel_and_process_grant_response(self, max_eirp=None):
        ...
        self._process_response(request_type_name=GRANT_REQ, db_requests=db_requests, response=response)
    ...
    def _prepare_response_from_payload(req_type, response_payload, response_code=None):
        ...
        return requests.get(any_url)  # url and method don't matter, I'm just crafting a qualified response here
    ...
    def _create_db_requests_from_fixture(self, request_state, request_type, fixture, cbsd_state):
        ...
        for reqs in fixture:
            for req in reqs[request_type]:
                db_requests.append(
                    DBRequest(
                        cbsd=self._generate_cbsd_from_request_json(req, self._get_db_enum(DBCbsdState, cbsd_state)),
