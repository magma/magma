...
class DPTestCase(LocalDBTestCase):
    ...
    def test_update_last_seen_on_request(self):
        cbsd = self._build_cbsd()
        self.session.add_all([cbsd])
        self.session.commit()

        request = self._build_request()
        self.dp_service.GetCBSDState(request, None)

        self.assertEqual(SOME_TIMESTAMP, self._get_last_seen_timestamp(request))
    ...
    def _get_last_seen_timestamp(self, request: CBSDStateRequest) -> int:
        return self._query_active_cbsd(request).first().cbsd.last_seen.timestamp()

    def _query_active_cbsd(self, request: CBSDStateRequest):
