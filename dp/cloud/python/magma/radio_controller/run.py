...
def run():
    ...
    add_DPServiceServicer_to_server(DPService(session_manager=session_manager, now=datetime.now), server)
...
def get_config() -> Config:
    ...
    config_module = importlib.import_module('.'.join(f"magma.radio_controller.config.{app_config}".split('.')[:-1]))
