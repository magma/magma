...
from dp.cloud.python.protocol_controller.plugins.cbsd_sas.validators.grant_request import (
    GrantRequestSchema,
)
from dp.cloud.python.protocol_controller.plugins.cbsd_sas.validators.heartbeat_request import (
    HeartbeatRequestSchema,
)
from dp.cloud.python.protocol_controller.plugins.cbsd_sas.validators.registration_request import (
...
from dp.cloud.python.protocol_controller.plugins.cbsd_sas.validators.spectrum_inquiry_request import (
...
class RequestValidationTestCase(TestCase):
    ...
    def test_request_with_more_than_one_key_fails_validation(self):
        # Given
        request_json = registration_requests[0].update({"deregistrationRequest": [{"cbsdId": "foo"}]})
    ...
    @parameterized.expand([
        ({"bad_registrationRequest": [{"fccId": "foo", "cbsdSerialNumber": "bar"}]}, RegistrationRequestSchema),
        ({"registrationRequest": [{"bad_fccId": "foo", "cbsdSerialNumber": "bar"}]}, RegistrationRequestSchema),
        ({"registrationRequest": [{"fccId": "foo", "bad_cbsdSerialNumber": "bar"}]}, RegistrationRequestSchema),
        ({"bad_deregistrationRequest": [{"cbsdId": "foo"}]}, DeregistrationRequestSchema),
        ({"deregistrationRequest": [{"bad_cbsdId": "foo"}]}, DeregistrationRequestSchema),
        ({"bad_relinquishmentRequest": [{"cbsdId": "foo"}]}, RelinquishmentRequestSchema),
        ({"relinquishmentRequest": [{"bad_cbsdId": "foo"}]}, RelinquishmentRequestSchema),
        ({"bad_grantRequest": [{"cbsdId": "foo"}]}, GrantRequestSchema),
        ({"grantRequest": [{"bad_cbsdId": "foo"}]}, GrantRequestSchema),
        ({"bad_heartbeatRequest": [{"cbsdId": "foo", "grantId": "bar"}]}, HeartbeatRequestSchema),
        ({"heartbeatRequest": [{"bad_cbsdId": "foo", "grantId": "bar"}]}, HeartbeatRequestSchema),
        ({"heartbeatRequest": [{"cbsdId": "foo", "bad_grantId": "bar"}]}, HeartbeatRequestSchema),
        ({"bad_spectrumInquiryRequest": [{"cbsdId": "foo"}]}, SpectrumInquiryRequestSchema),
        ({"spectrumInquiryRequest": [{"bad_cbsdId": "foo"}]}, SpectrumInquiryRequestSchema),
    ])
